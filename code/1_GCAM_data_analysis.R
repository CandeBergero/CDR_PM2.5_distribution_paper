...
# This script is designed to access the database for scenarios run in GCAM-USA for the just transition paper
# We want to create the .dat file with necessary output data
# Here we have one Reference scenario, and then two net-zero scenarios. One net-zero scenario has unlimited CDR (as modeled in GCAM). The other net-zero scenario has expensive CDR, so there is less of it. Both scenarios have modified emission factors for CCS electricity technologies.
# Created: June-July 2023
...
#### Load scenarios ####
  #Connect to the database
    #This code is used to create the .dat file. Can be skipped because file was already created
    # conn <- localDBConn('/Volumes/MODELS/GCAM/gcam-v6.0-release-package-Just-transitions/output', 'database_basexdb_FINAL')
  
  #Load the scenarios
  # prj <- addScenario(conn, 'dat/GCAM_analysis_july162025.dat', 'GCAM-USA_REF', 'queries/queries.xml') #Reference
  # prj <- addScenario(conn, 'dat/GCAM_analysis_july162025.dat', 'GCAM-USA_DAC_NZUSA2050ghg_NZROW2060co2_newEFV2_allEF', 'queries/queries.xml') #Net-zero + high CDR
  # prj <- addScenario(conn, 'dat/GCAM_analysis_july162025.dat', 'GCAM-USA_withDAC_NZUSA2050ghg_NZROW2060co2_forest_CCSV3_allEF', 'queries/queries.xml') #Net-zero + lowCDR
  
  #Load the project
  prj <- loadProject('data/GCAM/GCAM_analysis_july162025.dat') #dat file has additional queries
  listScenarios(prj) #These are our three scenarios
  listQueries(prj) #These are the output queries
...
#### Load mapping files ####
    ...
    air_pollution_sector_mapping_detailed <- read_csv("mappings/air_pollution_sectors_mapping_detailed.csv")
...
##### 1. Carbon dioxide #####
      ...
      CO2_emissions_region %>%
        filter(year > 2010,
               year < 2055) %>%
        mutate(Region = (if_else(region %in% USA_all, "USA", region)))%>%
        group_by(Region, scenario, Units, year) %>%
        summarise(region_total = sum(value)) %>%
        ungroup() %>%
        # convert MTC to GtCO2 (only graph in Giga, since it has ROW)
        mutate(region_total = (region_total*emissions.CONV_C_CO2)/1000,
               Units = "GtCO2")%>%
        left_join(scenario_mapping, by = "scenario") %>%
        mutate(Scenario = factor(Scenario, levels = scenario_short_order)) -> CO2_emissions_region_final_32

      # Reorder Region factor by cumulative emissions (descending)
      CO2_emissions_region_final_32 <- CO2_emissions_region_final_32 %>%
        group_by(Region) %>%
        mutate(total_emissions = sum(region_total)) %>%
        ungroup() %>%
        mutate(Region = fct_reorder(Region, total_emissions, .desc = TRUE))

      p <- ggplot() + geom_line(data=CO2_emissions_region_final_32, aes(x=year, y=region_total, colour = Scenario, group = Scenario), size = 1) +
      ...
      ggsave("figures/GCAM_analysis/1b.CO2_emissions_region_32.png", dpi=600/2, width=6000/300, height=3000/300)
      ...
      #Table from outputs
      carbon_storage_costs <- read.csv("data/GCAM/resource_curves_C_storage.csv") #add_units("MtC and 1990USDtC") %>%
...
##### 4.Air pollution ####
      ...
      ####PM2.5####
      #Figure out where is PM2.5 coming from in US, then each state
      #1. USA aggregate
      air_pollution_sources_complete %>%
        filter(ghg == "PM2.5") %>%
        group_by(Units, ghg, year, Sector, Scenario, BECCS, DAC) %>%
      ...
      #2. Sector in each state
      air_pollution_sources_complete %>%
        filter(ghg == "PM2.5") %>%
        group_by(Units, region, ghg, year, Sector, Scenario, BECCS, DAC) %>%
        summarise(sum = sum(value)) %>%
        ungroup() %>%
        mutate(Scenario = factor(Scenario, levels = scenario_short_order))-> air_pollution_sources_complete_PM2p5_states

      region_order <- air_pollution_sources_complete %>%
        filter(ghg == "PM2.5") %>%
        group_by(region) %>%
        summarise(total = sum(value, na.rm = TRUE)) %>%
        arrange(desc(total)) %>%
        mutate(rank = row_number(),
               group = ceiling(rank / 5))

      air_pollution_sources_complete_PM2p5_states_order <- air_pollution_sources_complete %>%
        filter(ghg == "PM2.5") %>%
        group_by(Units, region, ghg, year, Sector, Scenario, BECCS, DAC) %>%
        summarise(sum = sum(value), .groups = "drop") %>%
        left_join(region_order %>% select(region, group), by = "region") %>%
        mutate(region = fct_relevel(region, region_order$region),
               Scenario = factor(Scenario, levels = scenario_short_order))

      for (i in 1:10) {
        data_subset <- air_pollution_sources_complete_PM2p5_states_order %>%
          filter(group == i)

        p <- ggplot(data_subset) +
          geom_col(aes(x = year, y = sum, fill = Sector)) +
          scale_fill_manual(values = pollution_BC_color) +
          scale_x_continuous(breaks = seq(2010, 2100, 10)) +
          facet_grid(region ~ Scenario, switch = "y") +
          theme(panel.spacing = unit(2, "lines")) +
          ggtitle(paste("PM2.5 Air Pollution by Source in the US (States", (i-1)*5+1, "to", i*5, ")")) +
          xlab("Year") +
          ylab("Tg") +
          my_theme

        ggsave(filename = paste0("figures/GCAM_analysis/4d.air_pollution_source_PM2p5_states_group", i, ".png"), plot = p, dpi = 600/2,
          width = 6000/300, height = 3000/300)
      }

      #Now just plot the difference in each state (High CDR vs Low CDR)
      air_pollution_sources_complete_PM2p5_states_order %>%
        filter(Scenario != "Reference") %>%
        select(-BECCS, -DAC) %>%
        spread(Scenario, sum) %>%
        mutate(difference = `Net-zero, High BECCS, High DAC` - `Net-zero, Low BECCS, Low DAC`)-> air_pollution_sources_complete_PM2p5_states_difference

      air_pollution_sources_complete_PM2p5_states_difference %>%
        filter(group < 5,
               year == 2050) -> air_pollution_sources_complete_PM2p5_states_difference_1

      p <- ggplot() + geom_col(data=air_pollution_sources_complete_PM2p5_states_difference_1, aes(x=year, y=difference, fill= Sector)) +
        scale_fill_manual(values=pollution_BC_color) +
        scale_x_continuous(breaks=seq(2010,2100,10)) +
        # facet_grid(DAC ~ BECCS, switch="y") +
        facet_wrap(region ~ ., ncol = 5, as.table = F) +
        theme(panel.spacing = unit(2, "lines")) +
        ggtitle( "PM2.5 Air Pollution by Source in the US in 2050 (difference High - Low)" ) +
        theme(plot.title = element_text(face="bold", size=14, hjust = 0.5)) +
        xlab("Year") +
        ylab("Tg") +
        my_theme +
        theme(legend.position = "bottom")
      ggsave("figures/GCAM_analysis/4d.air_pollution_source_PM2p5_difference_1.png", dpi=600/2, width=6000/300, height=3000/300)

      air_pollution_sources_complete_PM2p5_states_difference %>%
        filter(group > 4 & group < 9,
               year == 2050) -> air_pollution_sources_complete_PM2p5_states_difference_2

      p <- ggplot() + geom_col(data=air_pollution_sources_complete_PM2p5_states_difference_2, aes(x=year, y=difference, fill= Sector)) +
        scale_fill_manual(values=pollution_BC_color) +
        scale_x_continuous(breaks=seq(2010,2100,10)) +
        # facet_grid(DAC ~ BECCS, switch="y") +
        facet_wrap(region ~ ., ncol = 5, as.table = F) +
        ...
        my_theme +
        theme(legend.position = "bottom")
      ggsave("figures/GCAM_analysis/4d.air_pollution_source_PM2p5_difference_2.png", dpi=600/2, width=6000/300, height=3000/300)

      air_pollution_sources_complete_PM2p5_states_difference %>%
        filter(group > 8,
               year == 2050) -> air_pollution_sources_complete_PM2p5_states_difference_3

      p <- ggplot() + geom_col(data=air_pollution_sources_complete_PM2p5_states_difference_3, aes(x=year, y=difference, fill= Sector)) +
        scale_fill_manual(values=pollution_BC_color) +
        scale_x_continuous(breaks=seq(2010,2100,10)) +
        # facet_grid(DAC ~ BECCS, switch="y") +
        facet_wrap(region ~ ., ncol = 5, as.table = F) +
        ...
        my_theme +
        theme(legend.position = "bottom")
      ggsave("figures/GCAM_analysis/4d.air_pollution_source_PM2p5_difference_3.png", dpi=600/2, width=6000/300, height=3000/300)

      ### 2.Get more detail in sectors
      scenario_order <- c("Reference","Net-zero, High BECCS, High DAC",  "Net-zero, Low BECCS, Low DAC")

      air_pollution_source_all %>%
        filter(year > 2010 & year < 2055,
               region %in% USA_states) %>% #Get rid of USA national level air pollution (land and resource production)
        left_join(air_pollution_sector_mapping_detailed, by = c("sector", "subsector", "technology")) %>%
        left_join(scenario_mapping, by = "scenario") -> air_pollution_sources_complete_detailed

      air_pollution_sources_complete_detailed %>%
        filter(ghg == "PM2.5") %>%
        group_by(Units, ghg, year, Sector_detailed, Scenario) %>%
        summarise(sum = sum(value)) %>%
        ungroup() %>%
        mutate(Scenario = factor(Scenario, levels = scenario_order))-> air_pollution_sources_complete_PM2p5_detailed

      library(RColorBrewer)

      # Choose a diverging palette with enough distinct colors
      num_sectors <- length(unique(air_pollution_sources_complete_PM2p5_detailed$Sector_detailed))

      # Spectral supports up to 11 values. For more, use `colorRampPalette`
      colors <- colorRampPalette(brewer.pal(11, "Spectral"))(num_sectors)

      p <- ggplot() +
        geom_col(data = air_pollution_sources_complete_PM2p5_detailed,
                 aes(x = year, y = sum, fill = Sector_detailed)) +
        ...
        ylab("Tg") +
        my_theme
      ggsave("figures/GCAM_analysis/4d.air_pollution_source_PM2p5_DETAILED.png", dpi=600/2, width=6000/300, height=3000/300)
      ...
      air_pollution_sources_complete %>% #Excludes national USA level air pollution (resource production / ag and land)
        group_by(Units, year, ghg, Sector, BECCS, DAC, Scenario) %>%
        summarise(sum = sum(value)) %>%
...
#####5. Primary Energy consumption #####
      ...
      primary_energy_consumption_states %>%
        ...
        summarise(sum = round(sum(value),1)) %>%
        ungroup() %>%
...
##### 8. Final energy industry by fuel #####
      ...
      #### b. Industry final energy by fuel and tech ####
      colors <- rev(brewer.pal(n = 11, name = "RdYlBu"))

      industry_tech_fuel <- getQuery(prj, "industry final energy by tech and fuel" )
      ...
      industry_tech_fuel %>%
        ...
        mutate(Scenario = factor(Scenario, levels = scenario_short_facet_order))%>%
        left_join(FE_fuel_mapping, by = "input") %>%
        mutate(Fuel = factor(Fuel, levels = final_energy_fuel_order)) %>%
        group_by(Units, Scenario, sector, subsector, technology, Fuel, year) %>%
        summarise(total_USA= sum(value)) %>%
        ungroup() %>%
        mutate(Scenario = factor(Scenario, levels = scenario_short_order))-> industry_tech_fuel_USA

      #Sector
      industry_tech_fuel_USA %>%
        group_by(Units, Scenario, sector, year) %>%
        summarise(total_USA_sector = sum(total_USA)) %>%
        ungroup() -> industry_tech_fuel_USA_sector

      p <- ggplot() + geom_col(data=industry_tech_fuel_USA_sector, aes(x=year, y=total_USA_sector, fill= sector)) +
        scale_fill_manual(values = colors) +  # << use custom color palette
        facet_wrap( ~ Scenario, ncol = 2, as.table = FALSE) +
        scale_x_continuous(breaks=seq(2010,2100,10)) +
        theme(panel.spacing = unit(2, "lines")) +
        ggtitle( "Final Energy in Industry USA by Sector" ) +
        theme(plot.title = element_text(face="bold", size=14, hjust = 0.5)) +
        xlab("Year") +
        ylab("EJ") +
        my_theme 
      ggsave("figures/GCAM_analysis/8b.final_energy_industry_USA_sector.png", dpi=600/2, width=6000/300, height=3000/300)

      #Tech
      industry_tech_fuel_USA %>%
        group_by(Units, Scenario, sector, technology, year) %>%
        summarise(total_USA_sector = sum(total_USA)) %>%
        ungroup() %>%
        mutate(Scenario = factor(Scenario, levels = scenario_short_order))-> industry_tech_fuel_USA_tech

      p <- ggplot() + geom_col(data=industry_tech_fuel_USA_tech, aes(x=year, y=total_USA_sector, fill= technology)) +
        #scale_fill_manual(values=final_energy_fuel_color) +
        scale_fill_viridis_d(option = "magma") +
        facet_grid( sector~ Scenario, scales = "free_y") +
        scale_x_continuous(breaks=seq(2010,2100,10)) +
        theme(panel.spacing = unit(2, "lines")) +
        ggtitle( "Final Energy in Industry USA by Technology" ) +
        theme(plot.title = element_text(face="bold", size=14, hjust = 0.5)) +
        xlab("Year") +
        ylab("EJ") +
        my_theme
      ...
      sectors <- unique(industry_tech_fuel_USA$sector)
      ...
      for (s in sectors) {
        ...
        data_subset <- industry_tech_fuel_USA %>% filter(sector == s)

        p <- ggplot() +
          geom_col(data = data_subset, aes(x = year, y = total_USA, fill = Fuel)) +
          scale_fill_viridis_d(option = "magma") +
          facet_grid(technology ~ Scenario, scales = "free_y") +
          scale_x_continuous(breaks = seq(2010, 2100, 10)) +
          ggtitle(paste("Final Energy in Industry USA by Technology -", s)) +
          xlab("Year") +
          ylab("EJ") +
          my_theme
...
##### 10. Electricity generation #####
    ...
    elec_gen <- getQuery(prj, "elec gen by region (no CHP)")
      ...
      #Percentages
      # Calculate percentage share of each Source per Scenario and year
      elec_percent <- elec_gen_subsector_fuel %>%
        # Keep only the 'Scenario' column with capital S
        filter(!is.na(Scenario)) %>%
        group_by(Scenario, year) %>%
        # Compute total generation per Scenario-year
        mutate(total_gen = sum(sum, na.rm = TRUE)) %>%
        # Compute percent contribution of each Source
        mutate(percent = 100 * sum / total_gen) %>%
        ungroup() %>%
        select(Scenario, year, Source, sum, percent)
      
      #total
      elec_gen_subsector_fuel %>%
        group_by(Scenario, year) %>%
        # Compute total generation per Scenario-year
        summarise(total_gen = sum(sum, na.rm = TRUE)) %>%
        ungroup() -> total
...
##### 12. Biomass #####
      ...
      ggsave("figures/GCAM_analysis/12b.biomass_consumption_MAP.png", dpi=600/2, width=6000/300, height=3000/300)
        ...
        #### c. "MSW production" ####
        MSW_produciton <- getQuery(prj, "MSW production")
...
##### 13. Land allocation #####
      ...
      food_demand %>%
        filter(year > 2010 & year < 2055,
               region == "USA") %>%
        left_join(scenario_mapping, by = "scenario") -> food_demand_USA

      #### food consumption ####
      food_consumption <- getQuery(prj, "food consumption by type (general)")

      food_consumption %>%
        filter(year > 2010 & year < 2055,
               region == "USA") %>%
        left_join(scenario_mapping, by = "scenario") -> food_consumption_USA
...
##### 16. Electricity prices ####
  ...
  ##### 17. Water ####
      water_demand <- getQuery(prj, "irrigation water withdrawals by crop type" )

      water_demand %>%
        filter(year > 2010 & year < 2055,
               region == "USA") %>%
        left_join(scenario_mapping, by = "scenario") -> water_demand_USA

#================================ GCAM SENSITIVITY SSPs ==============================================================
      #Sensitivity to different SSPs in GCAM assumptions
      #Create .dat file
      #Connect to the database
      # conn <- localDBConn('/Users/mariacandelariabergero/Documents/gcam-v6.0-Mac-Release-Package/output', 'database_basexdb_FINAL_SSPs')
      # conn2 <- localDBConn('/Users/mariacandelariabergero/Documents/gcam-v6.0-Mac-Release-Package/output', 'database_basexdb_FINAL_SSPs_2')
      # conn3 <- localDBConn('/Users/mariacandelariabergero/Documents/gcam-v6.0-Mac-Release-Package/output', 'database_basexdb_FINAL')

      #REF
      # prj <- addScenario(conn2, 'dat/GCAM_analysis_October152025.dat', 'GCAM-USA_REF_SSP1', 'queries/queries.xml')
      # prj <- addScenario(conn3, 'dat/GCAM_analysis_October152025.dat', 'GCAM-USA_REF', 'queries/queries.xml')
      # prj <- addScenario(conn2, 'dat/GCAM_analysis_October152025.dat', 'GCAM-USA_REF_SSP3', 'queries/queries.xml')
      # prj <- addScenario(conn2, 'dat/GCAM_analysis_October152025.dat', 'GCAM-USA_REF_SSP4', 'queries/queries.xml')
      # prj <- addScenario(conn2, 'dat/GCAM_analysis_October152025.dat', 'GCAM-USA_REF_SSP5', 'queries/queries.xml')

      #HIGH CDR
      prj <- addScenario(conn, 'dat/GCAM_analysis_October152025.dat', 'GCAM-USA_DAC_NZUSA2050ghg_NZROW2060co2_newEFV2_allEF_SSP1', 'queries/queries.xml')
      prj <- addScenario(conn2, 'dat/GCAM_analysis_October152025.dat', 'GCAM-USA_DAC_NZUSA2050ghg_NZROW2060co2_newEFV2_allEF_SSP2', 'queries/queries.xml')
      prj <- addScenario(conn2, 'dat/GCAM_analysis_October152025.dat', 'GCAM-USA_DAC_NZUSA2050ghg_NZROW2060co2_newEFV2_allEF_SSP3', 'queries/queries.xml')
      prj <- addScenario(conn2, 'dat/GCAM_analysis_October152025.dat', 'GCAM-USA_DAC_NZUSA2050ghg_NZROW2060co2_newEFV2_allEF_SSP4', 'queries/queries.xml')
      prj <- addScenario(conn, 'dat/GCAM_analysis_October152025.dat', 'GCAM-USA_DAC_NZUSA2050ghg_NZROW2060co2_newEFV2_allEF_SSP5', 'queries/queries.xml')


      #LOW CDR
      # prj <- addScenario(conn, 'dat/GCAM_analysis_October152025.dat', 'GCAM-USA_withDAC_NZUSA2050ghg_NZROW2060co2_forest_CCSV3_allEF_SSP1', 'queries/queries.xml')
      # prj <- addScenario(conn, 'dat/GCAM_analysis_October152025.dat', 'GCAM-USA_withDAC_NZUSA2050ghg_NZROW2060co2_forest_CCSV3_allEF_ssp2', 'queries/queries.xml')
      # prj <- addScenario(conn, 'dat/GCAM_analysis_October152025.dat', 'GCAM-USA_withDAC_NZUSA2050ghg_NZROW2060co2_forest_CCSV3_allEF_ssp3', 'queries/queries.xml')
      # prj <- addScenario(conn, 'dat/GCAM_analysis_October152025.dat', 'GCAM-USA_withDAC_NZUSA2050ghg_NZROW2060co2_forest_CCSV3_allEF_ssp4', 'queries/queries.xml')
      # prj <- addScenario(conn, 'dat/GCAM_analysis_October152025.dat', 'GCAM-USA_withDAC_NZUSA2050ghg_NZROW2060co2_forest_CCSV3_allEF_ssp5', 'queries/queries.xml')

      #Load the project
      prj <- loadProject('data/GCAM/GCAM_analysis_October152025.dat')
      listScenarios(prj)
      listQueries(prj)
      ...
      GHG_emissions_region %>%
        ...
        filter(year > 2010,
               year < 2055) %>%
        mutate(
          Region = if_else(region %in% USA_all, "USA", "ROW"),
          scenario = case_when(
            scenario == "GCAM-USA_withDAC_NZUSA2050ghg_NZROW2060co2_forest_CCSV3_allEF_SSP1" ~ "LOW-CDR SSP1",
            scenario == "GCAM-USA_withDAC_NZUSA2050ghg_NZROW2060co2_forest_CCSV3_allEF_ssp2" ~ "LOW-CDR SSP2",
            scenario == "GCAM-USA_withDAC_NZUSA2050ghg_NZROW2060co2_forest_CCSV3_allEF_ssp3" ~ "LOW-CDR SSP3",
            scenario == "GCAM-USA_withDAC_NZUSA2050ghg_NZROW2060co2_forest_CCSV3_allEF_ssp4" ~ "LOW-CDR SSP4",
            scenario == "GCAM-USA_withDAC_NZUSA2050ghg_NZROW2060co2_forest_CCSV3_allEF_ssp5" ~ "LOW-CDR SSP5",
            scenario == "GCAM-USA_DAC_NZUSA2050ghg_NZROW2060co2_newEFV2_allEF_SSP1" ~ "HIGH-CDR SSP1",
            scenario == "GCAM-USA_DAC_NZUSA2050ghg_NZROW2060co2_newEFV2_allEF_SSP2" ~ "HIGH-CDR SSP2",
            scenario == "GCAM-USA_DAC_NZUSA2050ghg_NZROW2060co2_newEFV2_allEF_SSP3" ~ "HIGH-CDR SSP3",
            scenario == "GCAM-USA_DAC_NZUSA2050ghg_NZROW2060co2_newEFV2_allEF_SSP4" ~ "HIGH-CDR SSP4",
            scenario == "GCAM-USA_DAC_NZUSA2050ghg_NZROW2060co2_newEFV2_allEF_SSP5" ~ "HIGH-CDR SSP5",
            scenario == "GCAM-USA_REF_SSP1" ~ "BAU SSP1",
            scenario == "GCAM-USA_REF" ~ "BAU SSP2",
            scenario == "GCAM-USA_REF_SSP3" ~ "BAU SSP3",
            scenario == "GCAM-USA_REF_SSP4" ~ "BAU SSP4",
            scenario == "GCAM-USA_REF_SSP5" ~ "BAU SSP5",
            TRUE ~ scenario # keep any others unchanged
          )) %>%
        separate(scenario, c("Scenario", "SSP"), sep = " ", remove = FALSE)-> GHG_emissions_region_complete
      ...
      air_pollutants_complete %>%
        filter(year < 2055) %>%
        group_by(Units, scenario, ghg, Region, year, Scenario, SSP) %>%
        summarise(sum = sum(value)) %>%
        ungroup() -> air_pollutants_USA
      ...
      p <- ggplot() + geom_line(data=air_pollutants_USA, aes(x=year, y=sum, colour = Scenario, group = Scenario), size = 1.5) +
        ...
        expand_limits(y = 0) +
        facet_grid(ghg ~ SSP, scales = "free_y") +
        #facet_wrap(facets = ~fct_reorder(ghg, sum, .desc = TRUE), scales = "free_y") +
        #facet_wrap(ghg ~ ., scales = "free_y") +
        theme(strip.text.x = element_text(size = 22)) +
        theme(strip.text.y = element_text(size = 22)) +
        ...
        my_theme +
        theme(legend.position = "bottom")
      ggsave("figures/GCAM_analysis/SENSITIVITY.air_pollution_energy.png", dpi=600/2, width=6000/300, height=6000/300)

      # Create bar plot
      p <- ggplot() +
        geom_bar(data = air_pollutants_USA %>% filter (year == 2050),
          aes(x = Scenario, y = sum, fill = Scenario),
          stat = "identity",
          position = "dodge",
          width = 0.7) +
        scale_fill_viridis_d(option = "magma") +
        facet_grid(ghg ~ SSP, scales = "free_y") +
        #facet_wrap(~ fct_reorder(ghg, sum, .desc = TRUE), scales = "free_y") +
        expand_limits(y = 0) +
        ggtitle("Air Pollutants in the US (Energy Sector, Year 2050)") +
        xlab("Scenario") +
        ylab("Tg") +
        my_theme
      ggsave("figures/GCAM_analysis/SENSITIVITY.air_pollution_energy_2050.png", dpi=600/2, width=6000/300, height=3000/300)

      air_pollutants_USA_round <- air_pollutants_USA %>%
        mutate(sum = round(sum, 2))

      # Filter to 2050
      df_plot <- air_pollutants_USA %>% filter(year == 2050)

      # Create the plot
      p_dots <- ggplot(df_plot, aes(x = Scenario, y = sum, color = SSP)) +
        geom_point(position = position_dodge(width = 0.6), size = 3) +  # dodge to separate SSPs
        facet_wrap(~ ghg, scales = "free_y") +
        scale_color_viridis_d(option = "magma") +
        expand_limits(y = 0) +
        labs(
          title = "Air Pollutants in the US (Energy Sector, Year 2050)",
          y = "Tg",
          x = "Scenario",
          color = "SSP"
        ) +
        theme_minimal() +
        theme( legend.position = "right")
      ggsave("figures/GCAM_analysis/SENSITIVITY.air_pollutants_USA_2050_dots.png", plot = p_dots, width = 12, height = 8, dpi = 300)
      ggsave("figures/GCAM_analysis/SENSITIVITY.air_pollutants_USA_2050_dots.svg", plot = p_dots, width = 12, height = 8, dpi = 300)

      ### STATES
      #Here we do a map with all air pollution in 2050 per state (all pollutants)
      air_pollutants_complete %>%
        filter(year == 2050) %>%
        # group_by(Units, scenario, ghg, region, year) %>%
        # summarise(sum = sum(value)) %>%
        # ungroup() %>%
        rename(state = region) -> air_pollutants_USA_MAP

      p <- plot_usmap(region = c("states"),
                      data = air_pollutants_USA_MAP %>% filter(ghg == "PM2.5"),
                      values = "value") +
        ggtitle( "2050 PM2.5 Air Pollution in the US from the energy sector in Tg" ) +
        facet_wrap(scenario ~ ., ncol = 2, as.table = F) +
        #facet_grid(DAC ~ BECCS, switch="y") +
        scale_fill_gradient2(low = "#06BA63", mid = "#FAF0CA", high = "#E63946", midpoint = 0) +
        my_theme + 
        theme(legend.position = "right")
      ggsave("figures/GCAM_analysis/SENSITIVITY.PM25_air_pollution_energy_column_MAPS_2050.png", dpi=600/2, width=6000/300, height=3000/300)

      # Define batch size (e.g., 12 states per plot)
      batch_size <- 12

      # Get unique states
      states <- unique(air_pollutants_USA_MAP$state)

      # Split states into batches
      state_batches <- split(states, ceiling(seq_along(states) / batch_size))

      # Loop over batches
      for (i in seq_along(state_batches)) {

        batch_states <- state_batches[[i]]

        # Filter to only states in this batch
        plot_data <- air_pollutants_USA_MAP %>%
          filter(
            year == 2050,
            ghg == "PM2.5",
            state %in% batch_states
          )
        ...
        p <- ggplot() +
          ...
          scale_fill_viridis_d(option = "magma") +
          facet_grid(state ~ SSP) +
          expand_limits(y = 0) +
          ggtitle(paste("PM2.5 Air Pollution in the US (Energy Sector, Year 2050) — Batch", i)) +
          xlab("Scenario") +
          ylab("Tg") +
          theme_minimal() +
          theme(
        ...
        ggsave(filename = paste0("figures/GCAM_analysis/SENSITIVITY_PM25_batch_", i, ".png"),
               plot = p, dpi = 300, width = 12,  height = 8 )
      }

      ### Now dodge dots
      # Filter and rename
      air_pollutants_USA_MAP <- air_pollutants_complete %>%
        filter(year == 2050) %>%
        rename(state = region)
      ...
      for (i in seq_along(state_batches)) {
        ...
        # Filter to states in this batch and PM2.5 only
        plot_data <- air_pollutants_USA_MAP %>%
          filter(
            ghg == "PM2.5",
            state %in% batch_states
        ...
        # Create dodged dot plot
        p <- ggplot(plot_data, aes(x = Scenario, y = value, color = SSP)) +
          geom_point(position = position_dodge(width = 0.6), size = 3) +
          facet_wrap(~ state) +
          scale_color_viridis_d(option = "magma") +
          expand_limits(y = 0) +
          scale_y_continuous(limits = c(0, 0.11)) +  # set max to 0.08
          labs(
            title = paste("PM2.5 Air Pollution in the US (Energy Sector, Year 2050) — Batch", i),
            y = "Tg",
            x = "Scenario",
            color = "SSP"
          ) +
        ...
        # Save
        ggsave(
          filename = paste0("figures/GCAM_analysis/SENSITIVITY_PM25_dots_batch_", i, ".png"),
          plot = p,
          dpi = 300,
          width = 14,
          height = 10
        ...
        ggsave(
          filename = paste0("figures/GCAM_analysis/SENSITIVITY_PM25_dots_batch_", i, ".svg"),
          plot = p,
          dpi = 300,
          width = 14,
          height = 10

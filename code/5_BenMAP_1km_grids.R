# BenMAP_1km_grids.R
# Code for re-scaling variables at 1 km grid: population and mortality, and for getting mean for PM2.5
...
# Sensitivities date: October-December 2025
...
#### 1. Load census data ####
    ...
    input_population_data <- read.csv("data/BenMAP/ACS/ACSDT5Y2019.B01003_2024-02-28T212808/ACSDT5Y2019.B01003-Data.csv") #From census data table B01003 on total population at the census block level. Note we rename column to match census block metadata
    
    input_race_data <- read.csv("data/BenMAP/ACS/ACSDT5Y2019.B02001_2024-02-28T213117/ACSDT5Y2019.B02001-Data.csv")  #From census data table B02001 on race at the census block level. Note we rename column to match census block metadata
    
    input_gender_age_data <- read.csv("data/BenMAP/ACS/ACSDT5Y2019.B01001_2024-02-28T213030/ACSDT5Y2019.B01001-Data.csv")  #From census data table B01001 on race at the census block level. Note we rename column to match census block metadata
    
    input_income_data <-read.csv("data/BenMAP/ACS/ACSDT5Y2019.B19013_2024-02-28T213319/ACSDT5Y2019.B19013-Data.csv")  #From census data table B19013 on race at the census block level. Note we rename column to match census block metadata
...
#### 1. SEATTLE ####
  ...
  #### 1.5. Outputs ####
    ...
    partitions_seattle_area_pixel_border <- partitions_seattle_area_pixel$pixel_ID[partitions_seattle_area_pixel$sum_area_ratio_pixel < 0.99] #1167
      ...
      #write.csv(partitions_seattle_list, "output_data/Partitions/partitions_seattle.csv")
...
#### 2. LOS ANGELES ####
  ...
  #### 2.5. Outputs ####
    ...
    ### 2.5.2. Reset pixel area when needed###
        ...
        #write.csv(partitions_LA_list, "output_data/Partitions/partitions_LA.csv")
...
#### 3. RIVERSIDE ####
  ...
  write.csv(partitions_riverside_list, "output_data/Partitions/partitions_riverside.csv")
...
#### 4. PHOENIX ####
  ...
  #### 4.5. Outputs ####
    ...
    ### 4.5.2. Reset pixel area when needed###
      ...
      write.csv(partitions_phoenix_list, "output_data/Partitions/partitions_phoenix.csv")
...
#### 5. DALLAS ####
  ...
  #### 5.5. Outputs ####
    ...
    ### 5.5.2. Reset pixel area when needed###
      ...
      write.csv(partitions_dallas_list, "output_data/Partitions/partitions_dallas.csv")
...
#### 6. HOUSTON ####
  ...
  #### 6.5. Outputs ####
    ...
    ### 6.5.2. Reset pixel area when needed###
      ...
      write.csv(partitions_houston_list, "output_data/Partitions/partitions_houston.csv")
...
#### 7. MIAMI ####
  ...
  #### 7.5. Outputs ####
    ...
    ### 7.5.2. Reset pixel area when needed###
      ...
      write.csv(partitions_miami_list, "output_data/Partitions/partitions_miami.csv")
...
#### 8. ATLANTA ####
  ...
  #### 8.5. Outputs ####
    ...
    ### 8.5.2. Reset pixel area when needed###
      ...
      write.csv(partitions_atl_list, "output_data/Partitions/partitions_Atlanta.csv")
...
#### 9. CHICAGO ####
  ...
  #### 9.5. Outputs ####
    ...
    ### 9.5.2. Reset pixel area when needed###
      ...
      write.csv(partitions_chicago_list, "output_data/Partitions/partitions_Chicago.csv")
...
#### 10. WASHINGTON DC ####
  ...
  #### 10.5. Outputs ####
    ...
    ### 10.5.2. Reset pixel area when needed###
      ...
      write.csv(partitions_wash_list, "output_data/Partitions/partitions_DC.csv")
...
#### 11. PHILADELPHIA ####
  ...
  #### 11.5. Outputs ####
    ...
    ### 11.5.2. Reset pixel area when needed###
      ...
      write.csv(partitions_phil_list, "output_data/Partitions/partitions_phil.csv")
...
#### 12. NEW YORK ####
  ...
  #### 12.5. Outputs ####
    ...
    ### 12.5.2. Reset pixel area when needed###
      ...
      write.csv(partitions_NY_list, "output_data/Partitions/partitions_NY.csv")
...
#### 13. BOSTON ####
  ...
  #### 13.5. Outputs ####
    ...
    ### 13.5.2. Reset pixel area when needed###
      ...
      write.csv(partitions_boston_list, "output_data/Partitions/partitions_boston.csv")
...
#### 14. DETROIT ####
  ...
  #### 14.5. Outputs ####
    ...
    ### 14.5.2. Reset pixel area when needed###
      ...
      write.csv(partitions_detroit_list, "output_data/Partitions/partitions_detroit.csv")
...
#### 15. SAN FRANCISCO ####
  ...
  #### 15.5. Outputs ####
    ...
    ### 15.5.2. Reset pixel area when needed###
      ...
      write.csv(partitions_SF_list, "output_data/Partitions/partitions_SF.csv")
...
#WRF-CMAQ files from Jing for daily average PM2.5
  #WE ARE USING THE ADJUSTED VALUES (based on a high-resolution and high-quality ground-level PM2.5 dataset (GlobalHighPM2.5)
    #From Wei, J. et al. First close insight into global daily gapless 1 km PM2.5 pollution, variability, and health impact. 
    #Nat. Commun. 14, 8349 (2023).
#### 1. SEATTLE ####
  #### 1.1. 2019 ####
...
#------------------------------------------------- SECTION 5: 1 KM MORTALITY BENMAP RESULTS -------------------------------------------------
#### 1. SEATTLE ####
...
#------------------------------------------------- SITING SENSITIVITY: 1. PREPARING POLLUTION FILES ---------------------------------------------------------------------------
      #Here we want to test sensitivity to assumptions for siting future power infrastructure. 
      #Currently we retire following eia schedule, and then older generator first
      #Then we add by increasing capacity factor of existing generators (per state, fuel and technology) to 85%.
        #And then by allocating generation (by state fuel, and technology) proportionally to the size of the power plants
        #(larger power plants get more generation. Smaller power plants get less generation)
      
      #Now, for three cities (LA, Detroit, Philadelphia) we instead assume that we retire and site future electricity considering
        #1. minority people living within 5km-buffer
        #2. total people living within 5km-buffer
        #3. lower-income people living within 5km-buffer
      
      #After we consider this, we calculate emissions, and rerun WRF-CMAQ, BenMAP, and distribution assessment
##### 2. LOS ANGELES #####   
      #Read in original pollution files for diff plots
      grid_data_PM25_southwest_LA_NAD_2050_high <-
        st_read("output_data/shapefiles/1km_V2/LA/1_km_PM25_2050_high_LA.shp")
      
      grid_data_PM25_southwest_LA_NAD_2050_low <-
        st_read("output_data/shapefiles/1km_V2/LA/1_km_PM25_2050_low_LA.shp")
    
    #### 2.1. Minority ####
      ##### 2.1.1. High CDR ####
      # 2.1.1 Get mean value 
      southwest_nc_1km_stack_2050_minority_highCDR <- stack("data/WRF-CMAQ/sensitivity/2_southwest/Minority/Dailymean.1km.HighCCS_Minority.2050.PM25_AVG.nc", varname = "PM25_AVG")
      southwest_mean_raster_2050_minority_highCDR <- calc(southwest_nc_1km_stack_2050_minority_highCDR, fun = mean, na.rm = TRUE, na.rm.args = list(na.rm = TRUE))
      plot(southwest_mean_raster_2050_minority_highCDR, col = rev(magma(20)), 
           main = " Southwest 2050 Yearly Mean PM2.5 Concentrations (ug/m3) Minority Sensitivity High-CDR")
      ...
      # 1. Check the CRS
      st_crs(grid_data_south_west)
      crs(southwest_mean_raster_2050_minority_highCDR)
      #2. Print extent information
      print(extent(southwest_mean_raster_2050_minority_highCDR))
      print(st_bbox(grid_data_south_west))
      #3. Check resolution
      res(southwest_mean_raster_2050_minority_highCDR)
      bbox <- st_bbox(grid_data_south_west)
      resolution <- c((bbox$xmax - bbox$xmin) / num_cols,
                      (bbox$ymax - bbox$ymin) / num_rows) 
      print(resolution)
      # 4. Set the CRS for mean_raster_2019
      crs(southwest_mean_raster_2050_minority_highCDR) <- st_crs(grid_data_south_west)$proj4string
      # 5. Set the extent of mean_raster_aligned to match grid_data
      extent(southwest_mean_raster_2050_minority_highCDR) <- extent(grid_data_south_west)
      #6. Filter pixels 
      grid_data_south_west %>%
        filter(pixel_ID %in% LA_pixels) -> grid_data_south_west_LA
      ...
      # 2.1.3. Get values from raster and assign them to my grid 
      # 1. Extract values using st_extract
      system.time({southwest_values <- raster::extract(southwest_mean_raster_2050_minority_highCDR, grid_data_south_west_LA)}) #391 s = ~6.5 min
      grid_data_PM25_southwest_LA_2050_minority_highCDR <- grid_data_south_west_LA
      # 2. Assign the values to the PM25_AVG column
      grid_data_PM25_southwest_LA_2050_minority_highCDR$PM25_AVG <- southwest_values
      grid_data_PM25_southwest_LA_2050_minority_highCDR$PM25_AVG <- as.numeric(gsub("[^0-9.-]", "", grid_data_PM25_southwest_LA_2050_minority_highCDR$PM25_AVG))
      
      # 2.1.4. Prepare output table
      # 1. Convert to NAD83
      grid_data_PM25_southwest_LA_NAD_2050_minority_highCDR <- st_transform(grid_data_PM25_southwest_LA_2050_minority_highCDR, 
                                                                            crs = st_crs("+proj=longlat +datum=NAD83"))
      
      #Fix one pixel with NA (this may be because the intersection fraction is very small). We set pollution form neighboring pixel
      grid_data_PM25_southwest_LA_NAD_2050_minority_highCDR %>%
        filter(!is.na(PM25_AVG)) -> grid_data_PM25_southwest_LA_NAD_2050_minority_highCDR
      ...
      grid_data_PM25_southwest_LA_NAD_2050_minority_highCDR %>%
        ...
        mutate(Metric = "D24HourMean",
               `Seasonal Metric` = "QuarterlyMean",
               `Annual Metric` = "Mean") -> grid_data_PM25_southwest_LA_NAD_2050_minority_highCDR_final
      
      # 2. Save file
      write.csv(grid_data_PM25_southwest_LA_NAD_2050_minority_highCDR_final, row.names = FALSE, 
                file = paste("output_data/BenMAP_files/sensitivity_siting/LA/Date_", today_date,"PM25_2050_minority_highCDR_LA.csv"))
      
      st_write(grid_data_PM25_southwest_LA_NAD_2050_minority_highCDR, 
               "output_data/shapefiles/sensitivity_siting/LA/1_km_PM25_2050_minority_highCDR_LA.shp")
      grid_data_PM25_southwest_LA_NAD_2050_minority_highCDR <- st_read("output_data/shapefiles/sensitivity_siting/LA/1_km_PM25_2050_minority_highCDR_LA.shp")
      ...
      grid_data_PM25_southwest_LA_NAD_2050_minority_highCDR_final %>%
        left_join(LA_pop_total, by = c("Row", "Column")) %>%
        filter(!is.na(Values))-> LA_pop_PM25_total
      ...
      total_population <- sum(LA_pop_PM25_total$population)
      # Calculate population-weighted average pollution
      LA_mean_BASE_2050_minority_highCDR <- sum_products / total_population
      mean(LA_pop_PM25_total$Values)
      
      #Set values > 9 to 9 for graphing purposes
      #Based on the EPA -> https://www.epa.gov/pm-pollution/national-ambient-air-quality-standards-naaqs-pm
      grid_data_PM25_southwest_LA_NAD_2050_minority_highCDR %>%
        mutate(PM25_AVG_graph = if_else(PM25_AVG >= 9, 9, PM25_AVG)) -> grid_data_PM25_southwest_LA_NAD_2050_minority_highCDR_graph
      
      # Perform intersection to retain only pixels within the city boundary
      grid_data_PM25_southwest_LA_NAD_2050_minority_highCDR_graph_crop <- st_intersection(grid_data_PM25_southwest_LA_NAD_2050_minority_highCDR_graph, cbsa_shapefile_LA)
      
      p <- ggplot() +
        geom_sf(data = grid_data_PM25_southwest_LA_NAD_2050_minority_highCDR_graph_crop, aes(fill = PM25_AVG_graph), color = NA, size = 0) +
        scale_fill_gradientn(colors = colors,
                             values = scales::rescale(c(0, 5, 10)),  # Set the midpoint to 5
                             limits = c(0, 10),  # Adjust limits to match your data range 
                             name = "PM2.5 (ug/m3)") +
        ggtitle("2050 High-CDR LA Mean 1km Yearly PM2.5 (ug/m3). Sensitivity minority") +
        geom_text(aes(label = paste("Population weighted mean:", round(LA_mean_BASE_2050_minority_highCDR, 2), "ug/m3")),
                  x = Inf, y = -Inf, hjust = 1, vjust = -0.6, size = 5)+
        annotation_scale() +
        geom_sf(data = cbsa_shapefile_LA, color = "black", fill = NA)+
        theme_void() +
        geom_point(data = NULL, aes(x = -118.24271554610264,  y =  34.05368499865087), shape = 1, color = "#95C623", size = 6, stroke = 2) #LA city hall
      ggsave("figures/BenMAP/sensitivity_siting/pollution_V2/1km_PM25_2050_minority_highCDR_LA.png", p, width = 16, height = 12, units = "in", dpi = 600)
      ggsave("figures/BenMAP/sensitivity_siting/pollution_V2/1km_PM25_2050_minority_highCDR_LA.svg", p, width = 16, height = 12, units = "in", dpi = 600)
      print(p)
      
      ##### 2.1.2. Low CDR ####
      # 2.1.1 Get mean value 
      southwest_nc_1km_stack_2050_minority_lowCDR <- stack("data/WRF-CMAQ/sensitivity/2_southwest/Minority/Dailymean.1km.LowCCS_Minority.2050.PM25_AVG.nc", varname = "PM25_AVG")
      southwest_mean_raster_2050_minority_lowCDR <- calc(southwest_nc_1km_stack_2050_minority_lowCDR, fun = mean, na.rm = TRUE, na.rm.args = list(na.rm = TRUE))
      plot(southwest_mean_raster_2050_minority_lowCDR, col = rev(magma(20)), 
           main = " Southwest 2050 Yearly Mean PM2.5 Concentrations (ug/m3) Minority Sensitivity low-CDR")
      ...
      # 1. Check the CRS
      st_crs(grid_data_south_west)
      crs(southwest_mean_raster_2050_minority_lowCDR)
      #2. Print extent information
      print(extent(southwest_mean_raster_2050_minority_lowCDR))
      print(st_bbox(grid_data_south_west))
      #3. Check resolution
      res(southwest_mean_raster_2050_minority_lowCDR)
      bbox <- st_bbox(grid_data_south_west)
      resolution <- c((bbox$xmax - bbox$xmin) / num_cols,
                      (bbox$ymax - bbox$ymin) / num_rows) 
      print(resolution)
      # 4. Set the CRS for mean_raster_2019
      crs(southwest_mean_raster_2050_minority_lowCDR) <- st_crs(grid_data_south_west)$proj4string
      # 5. Set the extent of mean_raster_aligned to match grid_data
      extent(southwest_mean_raster_2050_minority_lowCDR) <- extent(grid_data_south_west)
      #6. Filter pixels 
      grid_data_south_west %>%
        filter(pixel_ID %in% LA_pixels) -> grid_data_south_west_LA
      ...
      # 2.1.3. Get values from raster and assign them to my grid 
      # 1. Extract values using st_extract
      system.time({southwest_values <- raster::extract(southwest_mean_raster_2050_minority_lowCDR, grid_data_south_west_LA)}) #391 s = ~6.5 min
      grid_data_PM25_southwest_LA_2050_minority_lowCDR <- grid_data_south_west_LA
      # 2. Assign the values to the PM25_AVG column
      grid_data_PM25_southwest_LA_2050_minority_lowCDR$PM25_AVG <- southwest_values
      grid_data_PM25_southwest_LA_2050_minority_lowCDR$PM25_AVG <- as.numeric(gsub("[^0-9.-]", "", grid_data_PM25_southwest_LA_2050_minority_lowCDR$PM25_AVG))
      
      # 2.1.4. Prepare output table
      # 1. Convert to NAD83
      grid_data_PM25_southwest_LA_NAD_2050_minority_lowCDR <- st_transform(grid_data_PM25_southwest_LA_2050_minority_lowCDR, 
                                                                            crs = st_crs("+proj=longlat +datum=NAD83"))
      
      #Fix one pixel with NA (this may be because the intersection fraction is very small). We set pollution form neighboring pixel
      grid_data_PM25_southwest_LA_NAD_2050_minority_lowCDR %>%
        filter(!is.na(PM25_AVG)) -> grid_data_PM25_southwest_LA_NAD_2050_minority_lowCDR
      ...
      grid_data_PM25_southwest_LA_NAD_2050_minority_lowCDR %>%
        ...
        mutate(Metric = "D24HourMean",
               `Seasonal Metric` = "QuarterlyMean",
               `Annual Metric` = "Mean") -> grid_data_PM25_southwest_LA_NAD_2050_minority_lowCDR_final
      
      # 2. Save file
      write.csv(grid_data_PM25_southwest_LA_NAD_2050_minority_lowCDR_final, row.names = FALSE, 
                file = paste("output_data/BenMAP_files/sensitivity_siting/LA/Date_", today_date,"PM25_2050_minority_lowCDR_LA.csv"))
      
      st_write(grid_data_PM25_southwest_LA_NAD_2050_minority_lowCDR, 
               "output_data/shapefiles/sensitivity_siting/LA/1_km_PM25_2050_minority_lowCDR_LA.shp")
      grid_data_PM25_southwest_LA_NAD_2050_minority_lowCDR <- st_read("output_data/shapefiles/sensitivity_siting/LA/1_km_PM25_2050_minority_lowCDR_LA.shp")
      # 2.1.5. Prepare figure
      #Get national mean (population weighted)
      population_2019_data_pixel_age_final_LA %>%
        group_by(Row, Column) %>%
      ...
      grid_data_PM25_southwest_LA_NAD_2050_minority_lowCDR_final %>%
        left_join(LA_pop_total, by = c("Row", "Column")) %>%
        filter(!is.na(Values))-> LA_pop_PM25_total
      ...
      total_population <- sum(LA_pop_PM25_total$population)
      # Calculate population-weighted average pollution
      LA_mean_BASE_2050_minority_lowCDR <- sum_products / total_population
      mean(LA_pop_PM25_total$Values)
      
      #Set values > 9 to 9 for graphing purposes
      #Based on the EPA -> https://www.epa.gov/pm-pollution/national-ambient-air-quality-standards-naaqs-pm
      grid_data_PM25_southwest_LA_NAD_2050_minority_lowCDR %>%
        mutate(PM25_AVG_graph = if_else(PM25_AVG >= 9, 9, PM25_AVG)) -> grid_data_PM25_southwest_LA_NAD_2050_minority_lowCDR_graph
      
      # Perform intersection to retain only pixels within the city boundary
      grid_data_PM25_southwest_LA_NAD_2050_minority_lowCDR_graph_crop <- st_intersection(grid_data_PM25_southwest_LA_NAD_2050_minority_lowCDR_graph, cbsa_shapefile_LA)
      
      p <- ggplot() +
        geom_sf(data = grid_data_PM25_southwest_LA_NAD_2050_minority_lowCDR_graph_crop, aes(fill = PM25_AVG_graph), color = NA, size = 0) +
        scale_fill_gradientn(colors = colors,
                             values = scales::rescale(c(0, 5, 10)),  # Set the midpoint to 5
                             limits = c(0, 10),  # Adjust limits to match your data range 
                             name = "PM2.5 (ug/m3)") +
        ggtitle("2050 low-CDR LA Mean 1km Yearly PM2.5 (ug/m3). Sensitivity minority") +
        geom_text(aes(label = paste("Population weighted mean:", round(LA_mean_BASE_2050_minority_lowCDR, 2), "ug/m3")),
                  x = Inf, y = -Inf, hjust = 1, vjust = -0.6, size = 5)+
        annotation_scale() +
        geom_sf(data = cbsa_shapefile_LA, color = "black", fill = NA)+
        theme_void() +
        geom_point(data = NULL, aes(x = -118.24271554610264,  y =  34.05368499865087), shape = 1, color = "#95C623", size = 6, stroke = 2) #LA city hall
      ggsave("figures/BenMAP/sensitivity_siting/pollution_V2/1km_PM25_2050_minority_lowCDR_LA.png", p, width = 16, height = 12, units = "in", dpi = 600)
      ggsave("figures/BenMAP/sensitivity_siting/pollution_V2/1km_PM25_2050_minority_lowCDR_LA.svg", p, width = 16, height = 12, units = "in", dpi = 600)
      print(p)
      
    #### 2.2. Total people ####
      ##### 2.2.1. High CDR ####
      # 2.1.1 Get mean value 
      southwest_nc_1km_stack_2050_population_highCDR <- stack("data/WRF-CMAQ/sensitivity/2_southwest/Population/Dailymean.1km.HighCCS_Population.2050.PM25_AVG.nc", varname = "PM25_AVG")
      southwest_mean_raster_2050_population_highCDR <- calc(southwest_nc_1km_stack_2050_population_highCDR, fun = mean, na.rm = TRUE, na.rm.args = list(na.rm = TRUE))
      plot(southwest_mean_raster_2050_population_highCDR, col = rev(magma(20)), 
           main = " Southwest 2050 Yearly Mean PM2.5 Concentrations (ug/m3) Population Sensitivity High-CDR")
      ...
      # 1. Check the CRS
      st_crs(grid_data_south_west)
      crs(southwest_mean_raster_2050_population_highCDR)
      #2. Print extent information
      print(extent(southwest_mean_raster_2050_population_highCDR))
      print(st_bbox(grid_data_south_west))
      #3. Check resolution
      res(southwest_mean_raster_2050_population_highCDR)
      bbox <- st_bbox(grid_data_south_west)
      resolution <- c((bbox$xmax - bbox$xmin) / num_cols,
                      (bbox$ymax - bbox$ymin) / num_rows) 
      print(resolution)
      # 4. Set the CRS for mean_raster_2019
      crs(southwest_mean_raster_2050_population_highCDR) <- st_crs(grid_data_south_west)$proj4string
      # 5. Set the extent of mean_raster_aligned to match grid_data
      extent(southwest_mean_raster_2050_population_highCDR) <- extent(grid_data_south_west)
      #6. Filter pixels 
      grid_data_south_west %>%
        filter(pixel_ID %in% LA_pixels) -> grid_data_south_west_LA
      ...
      # 2.1.3. Get values from raster and assign them to my grid 
      # 1. Extract values using st_extract
      system.time({southwest_values <- raster::extract(southwest_mean_raster_2050_population_highCDR, grid_data_south_west_LA)}) #391 s = ~6.5 min
      grid_data_PM25_southwest_LA_2050_population_highCDR <- grid_data_south_west_LA
      # 2. Assign the values to the PM25_AVG column
      grid_data_PM25_southwest_LA_2050_population_highCDR$PM25_AVG <- southwest_values
      grid_data_PM25_southwest_LA_2050_population_highCDR$PM25_AVG <- as.numeric(gsub("[^0-9.-]", "", grid_data_PM25_southwest_LA_2050_population_highCDR$PM25_AVG))
      
      # 2.1.4. Prepare output table
      # 1. Convert to NAD83
      grid_data_PM25_southwest_LA_NAD_2050_population_highCDR <- st_transform(grid_data_PM25_southwest_LA_2050_population_highCDR, 
                                                                            crs = st_crs("+proj=longlat +datum=NAD83"))
      
      #Fix one pixel with NA (this may be because the intersection fraction is very small). We set pollution form neighboring pixel
      grid_data_PM25_southwest_LA_NAD_2050_population_highCDR %>%
        filter(!is.na(PM25_AVG)) -> grid_data_PM25_southwest_LA_NAD_2050_population_highCDR
      ...
      grid_data_PM25_southwest_LA_NAD_2050_population_highCDR %>%
        ...
        mutate(Metric = "D24HourMean",
               `Seasonal Metric` = "QuarterlyMean",
               `Annual Metric` = "Mean") -> grid_data_PM25_southwest_LA_NAD_2050_population_highCDR_final
      
      # 2. Save file
      write.csv(grid_data_PM25_southwest_LA_NAD_2050_population_highCDR_final, row.names = FALSE, 
                file = paste("output_data/BenMAP_files/sensitivity_siting/LA/Date_", today_date,"PM25_2050_population_highCDR_LA.csv"))
      
      st_write(grid_data_PM25_southwest_LA_NAD_2050_population_highCDR, 
               "output_data/shapefiles/sensitivity_siting/LA/1_km_PM25_2050_population_highCDR_LA.shp")
      grid_data_PM25_southwest_LA_NAD_2050_population_highCDR <- st_read("output_data/shapefiles/sensitivity_siting/LA/1_km_PM25_2050_population_highCDR_LA.shp")
      ...
      grid_data_PM25_southwest_LA_NAD_2050_population_highCDR_final %>%
        left_join(LA_pop_total, by = c("Row", "Column")) %>%
        filter(!is.na(Values))-> LA_pop_PM25_total
      ...
      total_population <- sum(LA_pop_PM25_total$population)
      # Calculate population-weighted average pollution
      LA_mean_BASE_2050_population_highCDR <- sum_products / total_population
      mean(LA_pop_PM25_total$Values)
      
      #Set values > 9 to 9 for graphing purposes
      #Based on the EPA -> https://www.epa.gov/pm-pollution/national-ambient-air-quality-standards-naaqs-pm
      grid_data_PM25_southwest_LA_NAD_2050_population_highCDR %>%
        mutate(PM25_AVG_graph = if_else(PM25_AVG >= 9, 9, PM25_AVG)) -> grid_data_PM25_southwest_LA_NAD_2050_population_highCDR_graph
      
      # Perform intersection to retain only pixels within the city boundary
      grid_data_PM25_southwest_LA_NAD_2050_population_highCDR_graph_crop <- st_intersection(grid_data_PM25_southwest_LA_NAD_2050_population_highCDR_graph, cbsa_shapefile_LA)
      
      p <- ggplot() +
        geom_sf(data = grid_data_PM25_southwest_LA_NAD_2050_population_highCDR_graph_crop, aes(fill = PM25_AVG_graph), color = NA, size = 0) +
        scale_fill_gradientn(colors = colors,
                             values = scales::rescale(c(0, 5, 10)),  # Set the midpoint to 5
                             limits = c(0, 10),  # Adjust limits to match your data range 
                             name = "PM2.5 (ug/m3)") +
        ggtitle("2050 High-CDR LA Mean 1km Yearly PM2.5 (ug/m3). Sensitivity population") +
        geom_text(aes(label = paste("Population weighted mean:", round(LA_mean_BASE_2050_population_highCDR, 2), "ug/m3")),
                  x = Inf, y = -Inf, hjust = 1, vjust = -0.6, size = 5)+
        annotation_scale() +
        geom_sf(data = cbsa_shapefile_LA, color = "black", fill = NA)+
        theme_void() +
        geom_point(data = NULL, aes(x = -118.24271554610264,  y =  34.05368499865087), shape = 1, color = "#95C623", size = 6, stroke = 2) #LA city hall
      ggsave("figures/BenMAP/sensitivity_siting/pollution_V2/1km_PM25_2050_population_highCDR_LA.png", p, width = 16, height = 12, units = "in", dpi = 600)
      ggsave("figures/BenMAP/sensitivity_siting/pollution_V2/1km_PM25_2050_population_highCDR_LA.svg", p, width = 16, height = 12, units = "in", dpi = 600)
      print(p)
      
      ##### 2.2.2. Low CDR ####
      # 2.1.1 Get mean value 
      southwest_nc_1km_stack_2050_population_lowCDR <- stack("data/WRF-CMAQ/sensitivity/2_southwest/population/Dailymean.1km.LowCCS_Population.2050.PM25_AVG.nc", varname = "PM25_AVG")
      southwest_mean_raster_2050_population_lowCDR <- calc(southwest_nc_1km_stack_2050_population_lowCDR, fun = mean, na.rm = TRUE, na.rm.args = list(na.rm = TRUE))
      plot(southwest_mean_raster_2050_population_lowCDR, col = rev(magma(20)), 
           main = " Southwest 2050 Yearly Mean PM2.5 Concentrations (ug/m3) population Sensitivity low-CDR")
      ...
      # 1. Check the CRS
      st_crs(grid_data_south_west)
      crs(southwest_mean_raster_2050_population_lowCDR)
      #2. Print extent information
      print(extent(southwest_mean_raster_2050_population_lowCDR))
      print(st_bbox(grid_data_south_west))
      #3. Check resolution
      res(southwest_mean_raster_2050_population_lowCDR)
      bbox <- st_bbox(grid_data_south_west)
      resolution <- c((bbox$xmax - bbox$xmin) / num_cols,
                      (bbox$ymax - bbox$ymin) / num_rows) 
      print(resolution)
      # 4. Set the CRS for mean_raster_2019
      crs(southwest_mean_raster_2050_population_lowCDR) <- st_crs(grid_data_south_west)$proj4string
      # 5. Set the extent of mean_raster_aligned to match grid_data
      extent(southwest_mean_raster_2050_population_lowCDR) <- extent(grid_data_south_west)
      #6. Filter pixels 
      grid_data_south_west %>%
        filter(pixel_ID %in% LA_pixels) -> grid_data_south_west_LA
      ...
      # 2.1.3. Get values from raster and assign them to my grid 
      # 1. Extract values using st_extract
      system.time({southwest_values <- raster::extract(southwest_mean_raster_2050_population_lowCDR, grid_data_south_west_LA)}) #391 s = ~6.5 min
      grid_data_PM25_southwest_LA_2050_population_lowCDR <- grid_data_south_west_LA
      # 2. Assign the values to the PM25_AVG column
      grid_data_PM25_southwest_LA_2050_population_lowCDR$PM25_AVG <- southwest_values
      grid_data_PM25_southwest_LA_2050_population_lowCDR$PM25_AVG <- as.numeric(gsub("[^0-9.-]", "", grid_data_PM25_southwest_LA_2050_population_lowCDR$PM25_AVG))
      
      # 2.1.4. Prepare output table
      # 1. Convert to NAD83
      grid_data_PM25_southwest_LA_NAD_2050_population_lowCDR <- st_transform(grid_data_PM25_southwest_LA_2050_population_lowCDR, 
                                                                           crs = st_crs("+proj=longlat +datum=NAD83"))
      
      #Fix one pixel with NA (this may be because the intersection fraction is very small). We set pollution form neighboring pixel
      grid_data_PM25_southwest_LA_NAD_2050_population_lowCDR %>%
        filter(!is.na(PM25_AVG)) -> grid_data_PM25_southwest_LA_NAD_2050_population_lowCDR
      ...
      grid_data_PM25_southwest_LA_NAD_2050_population_lowCDR %>%
        ...
        mutate(Metric = "D24HourMean",
               `Seasonal Metric` = "QuarterlyMean",
               `Annual Metric` = "Mean") -> grid_data_PM25_southwest_LA_NAD_2050_population_lowCDR_final
      
      # 2. Save file
      write.csv(grid_data_PM25_southwest_LA_NAD_2050_population_lowCDR_final, row.names = FALSE, 
                file = paste("output_data/BenMAP_files/sensitivity_siting/LA/Date_", today_date,"PM25_2050_population_lowCDR_LA.csv"))
      
      st_write(grid_data_PM25_southwest_LA_NAD_2050_population_lowCDR, 
               "output_data/shapefiles/sensitivity_siting/LA/1_km_PM25_2050_population_lowCDR_LA.shp")
      grid_data_PM25_southwest_LA_NAD_2050_population_lowCDR <- st_read("output_data/shapefiles/sensitivity_siting/LA/1_km_PM25_2050_population_lowCDR_LA.shp")
      ...
      grid_data_PM25_southwest_LA_NAD_2050_population_lowCDR_final %>%
        left_join(LA_pop_total, by = c("Row", "Column")) %>%
        filter(!is.na(Values))-> LA_pop_PM25_total
      ...
      total_population <- sum(LA_pop_PM25_total$population)
      # Calculate population-weighted average pollution
      LA_mean_BASE_2050_population_lowCDR <- sum_products / total_population
      mean(LA_pop_PM25_total$Values)
      
      #Set values > 9 to 9 for graphing purposes
      #Based on the EPA -> https://www.epa.gov/pm-pollution/national-ambient-air-quality-standards-naaqs-pm
      grid_data_PM25_southwest_LA_NAD_2050_population_lowCDR %>%
        mutate(PM25_AVG_graph = if_else(PM25_AVG >= 9, 9, PM25_AVG)) -> grid_data_PM25_southwest_LA_NAD_2050_population_lowCDR_graph
      
      # Perform intersection to retain only pixels within the city boundary
      grid_data_PM25_southwest_LA_NAD_2050_population_lowCDR_graph_crop <- st_intersection(grid_data_PM25_southwest_LA_NAD_2050_population_lowCDR_graph, cbsa_shapefile_LA)
      
      p <- ggplot() +
        geom_sf(data = grid_data_PM25_southwest_LA_NAD_2050_population_lowCDR_graph_crop, aes(fill = PM25_AVG_graph), color = NA, size = 0) +
        scale_fill_gradientn(colors = colors,
                             values = scales::rescale(c(0, 5, 10)),  # Set the midpoint to 5
                             limits = c(0, 10),  # Adjust limits to match your data range 
                             name = "PM2.5 (ug/m3)") +
        ggtitle("2050 low-CDR LA Mean 1km Yearly PM2.5 (ug/m3). Sensitivity population") +
        geom_text(aes(label = paste("Population weighted mean:", round(LA_mean_BASE_2050_population_lowCDR, 2), "ug/m3")),
                  x = Inf, y = -Inf, hjust = 1, vjust = -0.6, size = 5)+
        annotation_scale() +
        geom_sf(data = cbsa_shapefile_LA, color = "black", fill = NA)+
        theme_void() +
        geom_point(data = NULL, aes(x = -118.24271554610264,  y =  34.05368499865087), shape = 1, color = "#95C623", size = 6, stroke = 2) #LA city hall
      ggsave("figures/BenMAP/sensitivity_siting/pollution_V2/1km_PM25_2050_population_lowCDR_LA.png", p, width = 16, height = 12, units = "in", dpi = 600)
      ggsave("figures/BenMAP/sensitivity_siting/pollution_V2/1km_PM25_2050_population_lowCDR_LA.svg", p, width = 16, height = 12, units = "in", dpi = 600)
      print(p)
    #### 2.3. Income ####
      ##### 2.3.1. High CDR ####
      # 2.1.1 Get mean value 
      southwest_nc_1km_stack_2050_income_highCDR <- stack("data/WRF-CMAQ/sensitivity/2_southwest/Income/Dailymean.1km.HighCCS_Income.2050.PM25_AVG.nc", varname = "PM25_AVG")
      southwest_mean_raster_2050_income_highCDR <- calc(southwest_nc_1km_stack_2050_income_highCDR, fun = mean, na.rm = TRUE, na.rm.args = list(na.rm = TRUE))
      plot(southwest_mean_raster_2050_income_highCDR, col = rev(magma(20)), 
           main = " Southwest 2050 Yearly Mean PM2.5 Concentrations (ug/m3) income Sensitivity High-CDR")
      ...
      # 1. Check the CRS
      st_crs(grid_data_south_west)
      crs(southwest_mean_raster_2050_income_highCDR)
      #2. Print extent information
      print(extent(southwest_mean_raster_2050_income_highCDR))
      print(st_bbox(grid_data_south_west))
      #3. Check resolution
      res(southwest_mean_raster_2050_income_highCDR)
      bbox <- st_bbox(grid_data_south_west)
      resolution <- c((bbox$xmax - bbox$xmin) / num_cols,
                      (bbox$ymax - bbox$ymin) / num_rows) 
      print(resolution)
      # 4. Set the CRS for mean_raster_2019
      crs(southwest_mean_raster_2050_income_highCDR) <- st_crs(grid_data_south_west)$proj4string
      # 5. Set the extent of mean_raster_aligned to match grid_data
      extent(southwest_mean_raster_2050_income_highCDR) <- extent(grid_data_south_west)
      #6. Filter pixels 
      grid_data_south_west %>%
        filter(pixel_ID %in% LA_pixels) -> grid_data_south_west_LA
      ...
      # 2.1.3. Get values from raster and assign them to my grid 
      # 1. Extract values using st_extract
      system.time({southwest_values <- raster::extract(southwest_mean_raster_2050_income_highCDR, grid_data_south_west_LA)}) #391 s = ~6.5 min
      grid_data_PM25_southwest_LA_2050_income_highCDR <- grid_data_south_west_LA
      # 2. Assign the values to the PM25_AVG column
      grid_data_PM25_southwest_LA_2050_income_highCDR$PM25_AVG <- southwest_values
      grid_data_PM25_southwest_LA_2050_income_highCDR$PM25_AVG <- as.numeric(gsub("[^0-9.-]", "", grid_data_PM25_southwest_LA_2050_income_highCDR$PM25_AVG))
      
      # 2.1.4. Prepare output table
      # 1. Convert to NAD83
      grid_data_PM25_southwest_LA_NAD_2050_income_highCDR <- st_transform(grid_data_PM25_southwest_LA_2050_income_highCDR, 
                                                                              crs = st_crs("+proj=longlat +datum=NAD83"))
      
      #Fix one pixel with NA (this may be because the intersection fraction is very small). We set pollution form neighboring pixel
      grid_data_PM25_southwest_LA_NAD_2050_income_highCDR %>%
        filter(!is.na(PM25_AVG)) -> grid_data_PM25_southwest_LA_NAD_2050_income_highCDR
      ...
      grid_data_PM25_southwest_LA_NAD_2050_income_highCDR %>%
        ...
        mutate(Metric = "D24HourMean",
               `Seasonal Metric` = "QuarterlyMean",
               `Annual Metric` = "Mean") -> grid_data_PM25_southwest_LA_NAD_2050_income_highCDR_final
      
      # 2. Save file
      write.csv(grid_data_PM25_southwest_LA_NAD_2050_income_highCDR_final, row.names = FALSE, 
                file = paste("output_data/BenMAP_files/sensitivity_siting/LA/Date_", today_date,"PM25_2050_income_highCDR_LA.csv"))
      
      st_write(grid_data_PM25_southwest_LA_NAD_2050_income_highCDR, 
               "output_data/shapefiles/sensitivity_siting/LA/1_km_PM25_2050_income_highCDR_LA.shp")
      grid_data_PM25_southwest_LA_NAD_2050_income_highCDR <- st_read("output_data/shapefiles/sensitivity_siting/LA/1_km_PM25_2050_income_highCDR_LA.shp")
      ...
      grid_data_PM25_southwest_LA_NAD_2050_income_highCDR_final %>%
        left_join(LA_pop_total, by = c("Row", "Column")) %>%
        filter(!is.na(Values))-> LA_pop_PM25_total
      ...
      total_population <- sum(LA_pop_PM25_total$population)
      # Calculate population-weighted average pollution
      LA_mean_BASE_2050_income_highCDR <- sum_products / total_population
      mean(LA_pop_PM25_total$Values)
      
      #Set values > 9 to 9 for graphing purposes
      #Based on the EPA -> https://www.epa.gov/pm-pollution/national-ambient-air-quality-standards-naaqs-pm
      grid_data_PM25_southwest_LA_NAD_2050_income_highCDR %>%
        mutate(PM25_AVG_graph = if_else(PM25_AVG >= 9, 9, PM25_AVG)) -> grid_data_PM25_southwest_LA_NAD_2050_income_highCDR_graph
      
      # Perform intersection to retain only pixels within the city boundary
      grid_data_PM25_southwest_LA_NAD_2050_income_highCDR_graph_crop <- st_intersection(grid_data_PM25_southwest_LA_NAD_2050_income_highCDR_graph, cbsa_shapefile_LA)
      
      p <- ggplot() +
        geom_sf(data = grid_data_PM25_southwest_LA_NAD_2050_income_highCDR_graph_crop, aes(fill = PM25_AVG_graph), color = NA, size = 0) +
        scale_fill_gradientn(colors = colors,
                             values = scales::rescale(c(0, 5, 10)),  # Set the midpoint to 5
                             limits = c(0, 10),  # Adjust limits to match your data range 
                             name = "PM2.5 (ug/m3)") +
        ggtitle("2050 High-CDR LA Mean 1km Yearly PM2.5 (ug/m3). Sensitivity income") +
        geom_text(aes(label = paste("Population weighted mean:", round(LA_mean_BASE_2050_income_highCDR, 2), "ug/m3")),
                  x = Inf, y = -Inf, hjust = 1, vjust = -0.6, size = 5)+
        annotation_scale() +
        geom_sf(data = cbsa_shapefile_LA, color = "black", fill = NA)+
        theme_void() +
        geom_point(data = NULL, aes(x = -118.24271554610264,  y =  34.05368499865087), shape = 1, color = "#95C623", size = 6, stroke = 2) #LA city hall
      ggsave("figures/BenMAP/sensitivity_siting/pollution_V2/1km_PM25_2050_income_highCDR_LA.png", p, width = 16, height = 12, units = "in", dpi = 600)
      ggsave("figures/BenMAP/sensitivity_siting/pollution_V2/1km_PM25_2050_income_highCDR_LA.svg", p, width = 16, height = 12, units = "in", dpi = 600)
      print(p)
      
      ##### 2.3.2. Low CDR ####
      # 2.1.1 Get mean value 
      southwest_nc_1km_stack_2050_income_lowCDR <- stack("data/WRF-CMAQ/sensitivity/2_southwest/Income/Dailymean.1km.LowCCS_Income.2050.PM25_AVG.nc", varname = "PM25_AVG")
      southwest_mean_raster_2050_income_lowCDR <- calc(southwest_nc_1km_stack_2050_income_lowCDR, fun = mean, na.rm = TRUE, na.rm.args = list(na.rm = TRUE))
      plot(southwest_mean_raster_2050_income_lowCDR, col = rev(magma(20)), 
           main = " Southwest 2050 Yearly Mean PM2.5 Concentrations (ug/m3) income Sensitivity low-CDR")
      ...
      # 1. Check the CRS
      st_crs(grid_data_south_west)
      crs(southwest_mean_raster_2050_income_lowCDR)
      #2. Print extent information
      print(extent(southwest_mean_raster_2050_income_lowCDR))
      print(st_bbox(grid_data_south_west))
      #3. Check resolution
      res(southwest_mean_raster_2050_income_lowCDR)
      bbox <- st_bbox(grid_data_south_west)
      resolution <- c((bbox$xmax - bbox$xmin) / num_cols,
                      (bbox$ymax - bbox$ymin) / num_rows) 
      print(resolution)
      # 4. Set the CRS for mean_raster_2019
      crs(southwest_mean_raster_2050_income_lowCDR) <- st_crs(grid_data_south_west)$proj4string
      # 5. Set the extent of mean_raster_aligned to match grid_data
      extent(southwest_mean_raster_2050_income_lowCDR) <- extent(grid_data_south_west)
      #6. Filter pixels 
      grid_data_south_west %>%
        filter(pixel_ID %in% LA_pixels) -> grid_data_south_west_LA
      ...
      # 2.1.3. Get values from raster and assign them to my grid 
      # 1. Extract values using st_extract
      system.time({southwest_values <- raster::extract(southwest_mean_raster_2050_income_lowCDR, grid_data_south_west_LA)}) #391 s = ~6.5 min
      grid_data_PM25_southwest_LA_2050_income_lowCDR <- grid_data_south_west_LA
      # 2. Assign the values to the PM25_AVG column
      grid_data_PM25_southwest_LA_2050_income_lowCDR$PM25_AVG <- southwest_values
      grid_data_PM25_southwest_LA_2050_income_lowCDR$PM25_AVG <- as.numeric(gsub("[^0-9.-]", "", grid_data_PM25_southwest_LA_2050_income_lowCDR$PM25_AVG))
      
      # 2.1.4. Prepare output table
      # 1. Convert to NAD83
      grid_data_PM25_southwest_LA_NAD_2050_income_lowCDR <- st_transform(grid_data_PM25_southwest_LA_2050_income_lowCDR, 
                                                                             crs = st_crs("+proj=longlat +datum=NAD83"))
      
      #Fix one pixel with NA (this may be because the intersection fraction is very small). We set pollution form neighboring pixel
      grid_data_PM25_southwest_LA_NAD_2050_income_lowCDR %>%
        filter(!is.na(PM25_AVG)) -> grid_data_PM25_southwest_LA_NAD_2050_income_lowCDR
      ...
      grid_data_PM25_southwest_LA_NAD_2050_income_lowCDR %>%
        ...
        mutate(Metric = "D24HourMean",
               `Seasonal Metric` = "QuarterlyMean",
               `Annual Metric` = "Mean") -> grid_data_PM25_southwest_LA_NAD_2050_income_lowCDR_final
      
      # 2. Save file
      write.csv(grid_data_PM25_southwest_LA_NAD_2050_income_lowCDR_final, row.names = FALSE, 
                file = paste("output_data/BenMAP_files/sensitivity_siting/LA/Date_", today_date,"PM25_2050_income_lowCDR_LA.csv"))
      
      st_write(grid_data_PM25_southwest_LA_NAD_2050_income_lowCDR, 
               "output_data/shapefiles/sensitivity_siting/LA/1_km_PM25_2050_income_lowCDR_LA.shp")
      grid_data_PM25_southwest_LA_NAD_2050_income_lowCDR <- st_read("output_data/shapefiles/sensitivity_siting/LA/1_km_PM25_2050_income_lowCDR_LA.shp")
      # 2.1.5. Prepare figure
      #Get national mean (population weighted)
      population_2019_data_pixel_age_final_LA %>%
        group_by(Row, Column) %>%
      ...
      grid_data_PM25_southwest_LA_NAD_2050_income_lowCDR_final %>%
        left_join(LA_pop_total, by = c("Row", "Column")) %>%
        filter(!is.na(Values))-> LA_pop_PM25_total
      ...
      total_population <- sum(LA_pop_PM25_total$population)
      # Calculate population-weighted average pollution
      LA_mean_BASE_2050_income_lowCDR <- sum_products / total_population
      mean(LA_pop_PM25_total$Values)
      
      #Set values > 9 to 9 for graphing purposes
      #Based on the EPA -> https://www.epa.gov/pm-pollution/national-ambient-air-quality-standards-naaqs-pm
      grid_data_PM25_southwest_LA_NAD_2050_income_lowCDR %>%
        mutate(PM25_AVG_graph = if_else(PM25_AVG >= 9, 9, PM25_AVG)) -> grid_data_PM25_southwest_LA_NAD_2050_income_lowCDR_graph
      
      # Perform intersection to retain only pixels within the city boundary
      grid_data_PM25_southwest_LA_NAD_2050_income_lowCDR_graph_crop <- st_intersection(grid_data_PM25_southwest_LA_NAD_2050_income_lowCDR_graph, cbsa_shapefile_LA)
      
      p <- ggplot() +
        geom_sf(data = grid_data_PM25_southwest_LA_NAD_2050_income_lowCDR_graph_crop, aes(fill = PM25_AVG_graph), color = NA, size = 0) +
        scale_fill_gradientn(colors = colors,
                             values = scales::rescale(c(0, 5, 10)),  # Set the midpoint to 5
                             limits = c(0, 10),  # Adjust limits to match your data range 
                             name = "PM2.5 (ug/m3)") +
        ggtitle("2050 low-CDR LA Mean 1km Yearly PM2.5 (ug/m3). Sensitivity income") +
        geom_text(aes(label = paste("Population weighted mean:", round(Philadelphia_mean_BASE_2050_income_lowCDR, 2), "ug/m3")),
                  x = Inf, y = -Inf, hjust = 1, vjust = -0.6, size = 5)+
        annotation_scale() +
        geom_sf(data = cbsa_shapefile_LA, color = "black", fill = NA)+
        theme_void() +
        geom_point(data = NULL, aes(x = -118.24271554610264,  y =  34.05368499865087), shape = 1, color = "#95C623", size = 6, stroke = 2) #LA city hall
      ggsave("figures/BenMAP/sensitivity_siting/pollution_V2/1km_PM25_2050_income_lowCDR_LA.png", p, width = 16, height = 12, units = "in", dpi = 600)
      ggsave("figures/BenMAP/sensitivity_siting/pollution_V2/1km_PM25_2050_income_lowCDR_LA.svg", p, width = 16, height = 12, units = "in", dpi = 600)
      print(p)
      
      
    #### 2.4 difference plots ####
      #### all in one ####
      #First get all plots into 1:
      # 1. Combine base + sensitivities into one sf object
      all_scenarios <- bind_rows(
        grid_data_PM25_southwest_LA_NAD_2050_high %>% mutate(Scenario = "Base High CDR"),
        grid_data_PM25_southwest_LA_NAD_2050_low %>% mutate(Scenario = "Base Low CDR"),
        grid_data_PM25_southwest_LA_NAD_2050_population_highCDR %>% mutate(Scenario = "Population High CDR"),
        grid_data_PM25_southwest_LA_NAD_2050_minority_highCDR %>% mutate(Scenario = "Minority High CDR"),
        grid_data_PM25_southwest_LA_NAD_2050_income_highCDR %>% mutate(Scenario = "Income High CDR"),
        grid_data_PM25_southwest_LA_NAD_2050_population_lowCDR %>% mutate(Scenario = "Population Low CDR"),
        grid_data_PM25_southwest_LA_NAD_2050_minority_lowCDR %>% mutate(Scenario = "Minority Low CDR"),
        grid_data_PM25_southwest_LA_NAD_2050_income_lowCDR %>% mutate(Scenario = "Income Low CDR")
      )
      
      # 2. Set Scenario factor to control order
      scenario_order <- c(
        "Base High CDR", "Population High CDR", "Minority High CDR", "Income High CDR",
        "Base Low CDR", "Population Low CDR", "Minority Low CDR", "Income Low CDR"
      )
      
      all_scenarios <- all_scenarios %>%
        mutate(Scenario = factor(Scenario, levels = scenario_order))
      
      # 3. Crop to LA boundary
      all_scenarios_crop <- st_intersection(all_scenarios, cbsa_shapefile_LA)
      
      # 4. Plot using your custom colors
      p_facets_all <- ggplot(all_scenarios_crop) +
        geom_sf(aes(fill = PM25_AVG), color = NA, size = 0) +
        geom_sf(data = cbsa_shapefile_LA, color = "black", fill = NA) +
        scale_fill_gradientn(colors = colors,
                             values = scales::rescale(c(0, 5, 10)),  # Set the midpoint to 5
                             limits = c(0, 10),  # Adjust limits to match your data range 
                             name = "PM2.5 (ug/m3)") +
        facet_wrap(~Scenario, ncol = 4) +  # 8 facets, 2 rows of 4
        theme_void() +
        ggtitle("2050 LA 1km PM2.5 Concentrations (Base + Sensitivity Scenarios)") +
        geom_point(aes(x = -118.2427, y = 34.0537), shape = 1, color = "#95C623", size = 6, stroke = 2) # LA City Hall
      ggsave("figures/BenMAP/sensitivity_siting/pollution_V2/1km_PM25_2050_LA_all_scenarios.png",
             p_facets_all, width = 20, height = 12, units = "in", dpi = 600)
      ggsave("figures/BenMAP/sensitivity_siting/pollution_V2/1km_PM25_2050_LA_all_scenarios.svg",
             p_facets_all, width = 20, height = 12, units = "in", dpi = 600)
      
      # Now do scatterplot
      scatter_df <- all_scenarios %>%
        st_drop_geometry() %>%
        mutate(Scenario = Scenario) %>%
        select(pixel_ID, row, col, Scenario, PM25_AVG)
      
      base_df <- scatter_df %>%
        filter(Scenario %in% c("Base High CDR", "Base Low CDR")) %>%
        mutate(
          CDR = if_else(grepl("High", Scenario), "High CDR", "Low CDR")
        ) %>%
        select(pixel_ID, row, col, CDR, PM25_base = PM25_AVG)
      
      sens_df <- scatter_df %>%
        filter(grepl("Population|Minority|Income", Scenario)) %>%
        mutate(
          Sensitivity = case_when(
            grepl("Population", Scenario) ~ "Population",
            grepl("Minority", Scenario)   ~ "Minority",
            grepl("Income", Scenario)     ~ "Income"
          ),
          CDR = if_else(grepl("High", Scenario), "High CDR", "Low CDR")
        ) %>%
        select(pixel_ID, row, col, Sensitivity, CDR, PM25_sens = PM25_AVG)
      
      scatter_plot_data <- base_df %>%
        left_join(
          sens_df,
          by = c("pixel_ID", "row", "col", "CDR")
        ) %>%
        filter(!is.na(PM25_sens))
      
      p_scatter <- ggplot(scatter_plot_data,
                          aes(x = PM25_base, y = PM25_sens)) +
        geom_point(alpha = 0.3, size = 0.6) +
        geom_abline(slope = 1, intercept = 0,
                    linetype = "dashed", color = "black") +
        facet_grid(CDR ~ Sensitivity) +
        theme_minimal() +
        labs(
          x = expression("Base PM"[2.5]~"(µg/m"^3*")"),
          y = expression("Sensitivity PM"[2.5]~"(µg/m"^3*")"),
          title = "Pixel-level PM2.5: Base vs Sensitivity Scenarios (LA, 2050)"
        )
      
      #Now plot difference vs base
      diff_plot_data <- scatter_plot_data %>%
        mutate(
          PM25_diff = PM25_sens - PM25_base
        )
      ...
      p_diff <- ggplot(
        ...
        labs(
          x = expression("Base PM"[2.5]~"(µg/m"^3*")"),
          y = expression(Delta*"PM"[2.5]~"(Sensitivity − Base, µg/m"^3*")"),
          color = "Sensitivity",
          title = "Pixel-level PM2.5 Changes Relative to Base Scenario (LA, 2050)"
        )
      
      
      #### difference ####
      # Your 6 sf objects (replace with actual names if different)
      sf_list <- list(
        minority_highCDR = grid_data_PM25_southwest_LA_NAD_2050_minority_highCDR,
        minority_lowCDR  = grid_data_PM25_southwest_LA_NAD_2050_minority_lowCDR,
        population_highCDR = grid_data_PM25_southwest_LA_NAD_2050_population_highCDR,
        population_lowCDR  = grid_data_PM25_southwest_LA_NAD_2050_population_lowCDR,
        income_highCDR = grid_data_PM25_southwest_LA_NAD_2050_income_highCDR,
        income_lowCDR  = grid_data_PM25_southwest_LA_NAD_2050_income_lowCDR
      )
      
      # Corresponding "base" datasets for difference calculation
      base_list <- list(
        minority_highCDR = grid_data_PM25_southwest_LA_NAD_2050_high,
        minority_lowCDR  = grid_data_PM25_southwest_LA_NAD_2050_low,
        population_highCDR = grid_data_PM25_southwest_LA_NAD_2050_high,
        population_lowCDR  = grid_data_PM25_southwest_LA_NAD_2050_low,
        income_highCDR = grid_data_PM25_southwest_LA_NAD_2050_high,
        income_lowCDR  = grid_data_PM25_southwest_LA_NAD_2050_low
      )
      
      # Scenario labels
      scenario_labels <- c("Minority High CDR", "Minority Low CDR",
                           "Population High CDR", "Population Low CDR",
                           "Income High CDR", "Income Low CDR")
      
      compute_difference <- function(sensitivity_sf, base_sf) {
        base_clean <- base_sf %>% filter(!is.na(PM25_AVG))
        sensitivity_clean <- sensitivity_sf %>% filter(!is.na(PM25_AVG))
        
        diff_sf <- base_clean %>%
          select(pixel_ID, row, col, PM25_base = PM25_AVG, geometry) %>%
          left_join(
            sensitivity_clean %>% st_drop_geometry() %>% select(pixel_ID, row, col, PM25_sens = PM25_AVG),
            by = c("pixel_ID", "row", "col")
          ) %>%
          mutate(PM25_diff = PM25_sens - PM25_base) %>%
          filter(!is.na(PM25_diff))
        
        return(diff_sf)
      }
      
      diff_list <- mapply(compute_difference, sf_list, base_list, SIMPLIFY = FALSE)
      names(diff_list) <- scenario_names
      
      #Combine all into 1
      diff_combined <- bind_rows(
        lapply(seq_along(diff_list), function(i) {
          diff_list[[i]] %>% mutate(Scenario = scenario_labels[i])
        })
      )
      
      #Crop LA city
      diff_combined_crop <- st_intersection(diff_combined, cbsa_shapefile_LA)
      
      #Colorscale values
      max_abs <- max(abs(diff_combined_crop$PM25_diff), na.rm = TRUE)
      
      #Define the scenario order
      scenario_order <- c(
        "Population High CDR", "Minority High CDR", "Income High CDR",
        "Population Low CDR", "Minority Low CDR", "Income Low CDR"
      )
      
      # Apply factor ordering
      diff_combined_crop <- diff_combined_crop %>%
        mutate(Scenario = factor(Scenario, levels = scenario_order))
      
      # Then use Scenario in faceting
      p_facets <- ggplot(diff_combined_crop) +
        geom_sf(aes(fill = PM25_diff), color = NA, size = 0) +
        geom_sf(data = cbsa_shapefile_LA, color = "black", fill = NA) +
        scale_fill_gradientn(colors = colors,
                             name = expression(Delta~PM[2.5]~(mu*g/m^3)))+
        # scale_fill_gradient2(low = "blue", mid = "white", high = "red",
        #                      midpoint = 0, name = expression(Delta~PM[2.5]~(mu*g/m^3))) +
        facet_wrap(~Scenario, ncol = 3) +  # This will respect the factor order
        theme_void() +
        ggtitle("2050 LA 1km PM2.5 Difference (Sensitivity - Base)")
      ggsave("figures/BenMAP/sensitivity_siting/pollution_V2/1km_PM25_2050_LA_DIFF_FACET_GRID.png",
             p_facets, width = 24, height = 16, units = "in", dpi = 600)
      ggsave("figures/BenMAP/sensitivity_siting/pollution_V2/1km_PM25_2050_LA_DIFF_FACET_GRID.svg",
             p_facets, width = 24, height = 16, units = "in", dpi = 600)
      
##### 11. PHILADELPHIA #####  
      #Read in original pollution files for diff plots
      grid_data_PM25_Philadelphia_NAD_2050_high <-
        st_read("output_data/shapefiles/1km_V2/Philadelphia/1_km_PM25_2050_high_phil.shp")
      
      grid_data_PM25_Philadelphia_NAD_2050_low <-
        st_read("output_data/shapefiles/1km_V2/Philadelphia/1_km_PM25_2050_low_phil.shp")
    
    #### 11.1. Minority ####
      ##### 11.1.1. High CDR ####
      # 2.1.1 Get mean value 
      phil_ny_nc_1km_stack_2050_minority_highCDR <- stack("data/WRF-CMAQ/sensitivity/9_phil_ny/Minority/Dailymean.1km.HighCCS_Minority.2050.PM25_AVG.nc", varname = "PM25_AVG")
      phil_ny_mean_raster_2050_minority_highCDR <- calc(phil_ny_nc_1km_stack_2050_minority_highCDR, fun = mean, na.rm = TRUE, na.rm.args = list(na.rm = TRUE))
      plot(phil_ny_mean_raster_2050_minority_highCDR, col = rev(magma(20)), 
           main = " Philadelphia & NY 2050 Yearly Mean PM2.5 Concentrations (ug/m3) Minority Sensitivity High-CDR")
      ...
      # 1. Check the CRS
      st_crs(grid_data_northeast)
      crs(phil_ny_mean_raster_2050_minority_highCDR)
      #2. Print extent information
      print(extent(phil_ny_mean_raster_2050_minority_highCDR))
      print(st_bbox(grid_data_northeast))
      #3. Check resolution
      res(phil_ny_mean_raster_2050_minority_highCDR)
      bbox <- st_bbox(grid_data_northeast)
      resolution <- c((bbox$xmax - bbox$xmin) / num_cols,
                      (bbox$ymax - bbox$ymin) / num_rows) 
      print(resolution)
      # 4. Set the CRS for mean_raster_2019
      crs(phil_ny_mean_raster_2050_minority_highCDR) <- st_crs(grid_data_northeast)$proj4string
      # 5. Set the extent of mean_raster_aligned to match grid_data
      extent(phil_ny_mean_raster_2050_minority_highCDR) <- extent(grid_data_northeast)
      #6. Filter pixels 
      grid_data_northeast %>%
        filter(pixel_ID %in% phil_pixels) -> grid_data_northeast_filtered_phil
      ...
      # 2.1.3. Get values from raster and assign them to my grid 
      # 1. Extract values using st_extract
      system.time({phil_ny_values <- raster::extract(phil_ny_mean_raster_2050_minority_highCDR, grid_data_northeast_filtered_phil)}) #391 s = ~6.5 min
      grid_data_PM25_phil_ny_2050_minority_highCDR <- grid_data_northeast_filtered_phil
      # 2. Assign the values to the PM25_AVG column
      grid_data_PM25_phil_ny_2050_minority_highCDR$PM25_AVG <- phil_ny_values
      grid_data_PM25_phil_ny_2050_minority_highCDR$PM25_AVG <- as.numeric(gsub("[^0-9.-]", "", grid_data_PM25_phil_ny_2050_minority_highCDR$PM25_AVG))
      
      # 2.1.4. Prepare output table
      # 1. Convert to NAD83
      grid_data_PM25_phil_ny_NAD_2050_minority_highCDR <- st_transform(grid_data_PM25_phil_ny_2050_minority_highCDR, 
                                                                            crs = st_crs("+proj=longlat +datum=NAD83"))
      
      #Fix one pixel with NA (this may be because the intersection fraction is very small). We set pollution form neighboring pixel
      grid_data_PM25_phil_ny_NAD_2050_minority_highCDR %>%
        filter(!is.na(PM25_AVG)) -> grid_data_PM25_phil_ny_NAD_2050_minority_highCDR
      ...
      grid_data_PM25_phil_ny_NAD_2050_minority_highCDR %>%
        ...
        mutate(Metric = "D24HourMean",
               `Seasonal Metric` = "QuarterlyMean",
               `Annual Metric` = "Mean") -> grid_data_PM25_phil_ny_NAD_2050_minority_highCDR_final
      
      # 2. Save file
      write.csv(grid_data_PM25_phil_ny_NAD_2050_minority_highCDR_final, row.names = FALSE, 
                file = paste("output_data/BenMAP_files/sensitivity_siting/Philadelphia/Date_", today_date,"PM25_2050_minority_highCDR_Philadelphia.csv"))
      
      st_write(grid_data_PM25_phil_ny_NAD_2050_minority_highCDR, 
               "output_data/shapefiles/sensitivity_siting/Philadelphia/1_km_PM25_2050_minority_highCDR_Philadelphia.shp")
      grid_data_PM25_phil_ny_NAD_2050_minority_highCDR <- st_read("output_data/shapefiles/sensitivity_siting/Philadelphia/1_km_PM25_2050_minority_highCDR_Philadelphia.shp")
      ...
      grid_data_PM25_phil_ny_NAD_2050_minority_highCDR_final %>%
        left_join(Philadelphia_pop_total, by = c("Row", "Column")) %>%
        filter(!is.na(Values))-> Philadelphia_pop_PM25_total
      ...
      # Calculate population-weighted average pollution
      Philadelphia_mean_BASE_2050_minority_highCDR <- sum_products / total_population
      mean(Philadelphia_pop_PM25_total$Values)
      
      #Set values > 9 to 9 for graphing purposes
      #Based on the EPA -> https://www.epa.gov/pm-pollution/national-ambient-air-quality-standards-naaqs-pm
      grid_data_PM25_phil_ny_NAD_2050_minority_highCDR %>%
        mutate(PM25_AVG_graph = if_else(PM25_AVG >= 9, 9, PM25_AVG)) -> grid_data_PM25_phil_ny_NAD_2050_minority_highCDR_graph
      
      # Perform intersection to retain only pixels within the city boundary
      grid_data_PM25_phil_ny_NAD_2050_minority_highCDR_graph_crop <- st_intersection(grid_data_PM25_phil_ny_NAD_2050_minority_highCDR_graph, cbsa_shapefile_phil)
      
      p <- ggplot() +
        geom_sf(data = grid_data_PM25_phil_ny_NAD_2050_minority_highCDR_graph_crop, aes(fill = PM25_AVG_graph), color = NA, size = 0) +
        scale_fill_gradientn(colors = colors,
                             values = scales::rescale(c(0, 5, 10)),  # Set the midpoint to 5
                             limits = c(0, 10),  # Adjust limits to match your data range 
                             name = "PM2.5 (ug/m3)") +
        ggtitle("2050 High-CDR Philadelphia Mean 1km Yearly PM2.5 (ug/m3). Sensitivity minority") +
        geom_text(aes(label = paste("Population weighted mean:", round(Philadelphia_mean_BASE_2050_minority_highCDR, 2), "ug/m3")),
                  x = Inf, y = -Inf, hjust = 1, vjust = -0.6, size = 5)+
        annotation_scale() +
        geom_sf(data = cbsa_shapefile_phil, color = "black", fill = NA)+
        theme_void() +
                geom_point(data = NULL, aes(x = -75.16326872597556,  y =  39.95279997140329), shape = 1, color = "#95C623", size = 6, stroke = 2) #Philadelphia city hall
      ggsave("figures/BenMAP/sensitivity_siting/pollution_V2/1km_PM25_2050_minority_highCDR_Philadelphia.png", p, width = 16, height = 12, units = "in", dpi = 600)
      ggsave("figures/BenMAP/sensitivity_siting/pollution_V2/1km_PM25_2050_minority_highCDR_Philadelphia.svg", p, width = 16, height = 12, units = "in", dpi = 600)
      print(p)
      
      ##### 11.1.2. Low CDR ####
      # 2.1.1 Get mean value 
      phil_ny_nc_1km_stack_2050_minority_lowCDR <- stack("data/WRF-CMAQ/sensitivity/9_phil_ny/Minority/Dailymean.1km.LowCCS_Minority.2050.PM25_AVG.nc", varname = "PM25_AVG")
      phil_ny_mean_raster_2050_minority_lowCDR <- calc(phil_ny_nc_1km_stack_2050_minority_lowCDR, fun = mean, na.rm = TRUE, na.rm.args = list(na.rm = TRUE))
      plot(phil_ny_mean_raster_2050_minority_lowCDR, col = rev(magma(20)), 
           main = " phil_ny 2050 Yearly Mean PM2.5 Concentrations (ug/m3) Minority Sensitivity low-CDR")
      ...
      # 1. Check the CRS
      st_crs(grid_data_northeast)
      crs(phil_ny_mean_raster_2050_minority_lowCDR)
      #2. Print extent information
      print(extent(phil_ny_mean_raster_2050_minority_lowCDR))
      print(st_bbox(grid_data_northeast))
      #3. Check resolution
      res(phil_ny_mean_raster_2050_minority_lowCDR)
      bbox <- st_bbox(grid_data_northeast)
      resolution <- c((bbox$xmax - bbox$xmin) / num_cols,
                      (bbox$ymax - bbox$ymin) / num_rows) 
      print(resolution)
      # 4. Set the CRS for mean_raster_2019
      crs(phil_ny_mean_raster_2050_minority_lowCDR) <- st_crs(grid_data_northeast)$proj4string
      # 5. Set the extent of mean_raster_aligned to match grid_data
      extent(phil_ny_mean_raster_2050_minority_lowCDR) <- extent(grid_data_northeast)
      #6. Filter pixels 
      grid_data_northeast %>%
        filter(pixel_ID %in% phil_pixels) -> grid_data_northeast_filtered_phil
      ...
      # 2.1.3. Get values from raster and assign them to my grid 
      # 1. Extract values using st_extract
      system.time({phil_ny_values <- raster::extract(phil_ny_mean_raster_2050_minority_lowCDR, grid_data_northeast_filtered_phil)}) #391 s = ~6.5 min
      grid_data_PM25_phil_ny_2050_minority_lowCDR <- grid_data_northeast_filtered_phil
      # 2. Assign the values to the PM25_AVG column
      grid_data_PM25_phil_ny_2050_minority_lowCDR$PM25_AVG <- phil_ny_values
      grid_data_PM25_phil_ny_2050_minority_lowCDR$PM25_AVG <- as.numeric(gsub("[^0-9.-]", "", grid_data_PM25_phil_ny_2050_minority_lowCDR$PM25_AVG))
      
      # 2.1.4. Prepare output table
      # 1. Convert to NAD83
      grid_data_PM25_phil_ny_NAD_2050_minority_lowCDR <- st_transform(grid_data_PM25_phil_ny_2050_minority_lowCDR, 
                                                                           crs = st_crs("+proj=longlat +datum=NAD83"))
      
      #Fix one pixel with NA (this may be because the intersection fraction is very small). We set pollution form neighboring pixel
      grid_data_PM25_phil_ny_NAD_2050_minority_lowCDR %>%
        filter(!is.na(PM25_AVG)) -> grid_data_PM25_phil_ny_NAD_2050_minority_lowCDR
      ...
      grid_data_PM25_phil_ny_NAD_2050_minority_lowCDR %>%
        ...
        mutate(Metric = "D24HourMean",
               `Seasonal Metric` = "QuarterlyMean",
               `Annual Metric` = "Mean") -> grid_data_PM25_phil_ny_NAD_2050_minority_lowCDR_final
      
      # 2. Save file
      write.csv(grid_data_PM25_phil_ny_NAD_2050_minority_lowCDR_final, row.names = FALSE, 
                file = paste("output_data/BenMAP_files/sensitivity_siting/Philadelphia/Date_", today_date,"PM25_2050_minority_lowCDR_Philadelphia.csv"))
      
      st_write(grid_data_PM25_phil_ny_NAD_2050_minority_lowCDR, 
               "output_data/shapefiles/sensitivity_siting/Philadelphia/1_km_PM25_2050_minority_lowCDR_Philadelphia.shp")
      grid_data_PM25_phil_ny_NAD_2050_minority_lowCDR <- st_read("output_data/shapefiles/sensitivity_siting/Philadelphia/1_km_PM25_2050_minority_lowCDR_Philadelphia.shp")
      ...
      grid_data_PM25_phil_ny_NAD_2050_minority_lowCDR_final %>%
        left_join(Philadelphia_pop_total, by = c("Row", "Column")) %>%
        filter(!is.na(Values))-> Philadelphia_pop_PM25_total
      ...
      total_population <- sum(Philadelphia_pop_PM25_total$population)
      # Calculate population-weighted average pollution
      Philadelphia_mean_BASE_2050_minority_lowCDR <- sum_products / total_population
      mean(Philadelphia_pop_PM25_total$Values)
      
      #Set values > 9 to 9 for graphing purposes
      #Based on the EPA -> https://www.epa.gov/pm-pollution/national-ambient-air-quality-standards-naaqs-pm
      grid_data_PM25_phil_ny_NAD_2050_minority_lowCDR %>%
        mutate(PM25_AVG_graph = if_else(PM25_AVG >= 9, 9, PM25_AVG)) -> grid_data_PM25_phil_ny_NAD_2050_minority_lowCDR_graph
      
      # Perform intersection to retain only pixels within the city boundary
      grid_data_PM25_phil_ny_NAD_2050_minority_lowCDR_graph_crop <- st_intersection(grid_data_PM25_phil_ny_NAD_2050_minority_lowCDR_graph, cbsa_shapefile_phil)
      
      p <- ggplot() +
        geom_sf(data = grid_data_PM25_phil_ny_NAD_2050_minority_lowCDR_graph_crop, aes(fill = PM25_AVG_graph), color = NA, size = 0) +
        scale_fill_gradientn(colors = colors,
                             values = scales::rescale(c(0, 5, 10)),  # Set the midpoint to 5
                             limits = c(0, 10),  # Adjust limits to match your data range 
                             name = "PM2.5 (ug/m3)") +
        ggtitle("2050 low-CDR Philadelphia Mean 1km Yearly PM2.5 (ug/m3). Sensitivity minority") +
        geom_text(aes(label = paste("Population weighted mean:", round(Philadelphia_mean_BASE_2050_minority_lowCDR, 2), "ug/m3")),
                  x = Inf, y = -Inf, hjust = 1, vjust = -0.6, size = 5)+
        annotation_scale() +
        geom_sf(data = cbsa_shapefile_phil, color = "black", fill = NA)+
        theme_void() +
                geom_point(data = NULL, aes(x = -75.16326872597556,  y =  39.95279997140329), shape = 1, color = "#95C623", size = 6, stroke = 2) #Philadelphia city hall
      ggsave("figures/BenMAP/sensitivity_siting/pollution_V2/1km_PM25_2050_minority_lowCDR_Philadelphia.png", p, width = 16, height = 12, units = "in", dpi = 600)
      ggsave("figures/BenMAP/sensitivity_siting/pollution_V2/1km_PM25_2050_minority_lowCDR_Philadelphia.svg", p, width = 16, height = 12, units = "in", dpi = 600)
      print(p)
      
    #### 11.2. Total people ####
      ##### 11.2.1. High CDR ####
      # 2.1.1 Get mean value 
      phil_ny_nc_1km_stack_2050_population_highCDR <- stack("data/WRF-CMAQ/sensitivity/9_phil_ny/Population/Dailymean.1km.HighCCS_Population.2050.PM25_AVG.nc", varname = "PM25_AVG")
      phil_ny_mean_raster_2050_population_highCDR <- calc(phil_ny_nc_1km_stack_2050_population_highCDR, fun = mean, na.rm = TRUE, na.rm.args = list(na.rm = TRUE))
      plot(phil_ny_mean_raster_2050_population_highCDR, col = rev(magma(20)), 
           main = " phil_ny 2050 Yearly Mean PM2.5 Concentrations (ug/m3) Population Sensitivity High-CDR")
      ...
      # 1. Check the CRS
      st_crs(grid_data_northeast)
      crs(phil_ny_mean_raster_2050_population_highCDR)
      #2. Print extent information
      print(extent(phil_ny_mean_raster_2050_population_highCDR))
      print(st_bbox(grid_data_northeast))
      #3. Check resolution
      res(phil_ny_mean_raster_2050_population_highCDR)
      bbox <- st_bbox(grid_data_northeast)
      resolution <- c((bbox$xmax - bbox$xmin) / num_cols,
                      (bbox$ymax - bbox$ymin) / num_rows) 
      print(resolution)
      # 4. Set the CRS for mean_raster_2019
      crs(phil_ny_mean_raster_2050_population_highCDR) <- st_crs(grid_data_northeast)$proj4string
      # 5. Set the extent of mean_raster_aligned to match grid_data
      extent(phil_ny_mean_raster_2050_population_highCDR) <- extent(grid_data_northeast)
      #6. Filter pixels 
      grid_data_northeast %>%
        filter(pixel_ID %in% phil_pixels) -> grid_data_northeast_filtered_phil
      ...
      # 2.1.3. Get values from raster and assign them to my grid 
      # 1. Extract values using st_extract
      system.time({phil_ny_values <- raster::extract(phil_ny_mean_raster_2050_population_highCDR, grid_data_northeast_filtered_phil)}) #391 s = ~6.5 min
      grid_data_PM25_phil_ny_2050_population_highCDR <- grid_data_northeast_filtered_phil
      # 2. Assign the values to the3 PM25_AVG column
      grid_data_PM25_phil_ny_2050_population_highCDR$PM25_AVG <- phil_ny_values
      grid_data_PM25_phil_ny_2050_population_highCDR$PM25_AVG <- as.numeric(gsub("[^0-9.-]", "", grid_data_PM25_phil_ny_2050_population_highCDR$PM25_AVG))
      
      # 2.1.4. Prepare output table
      # 1. Convert to NAD83
      grid_data_PM25_phil_ny_NAD_2050_population_highCDR <- st_transform(grid_data_PM25_phil_ny_2050_population_highCDR, 
                                                                              crs = st_crs("+proj=longlat +datum=NAD83"))
      
      #Fix one pixel with NA (this may be because the intersection fraction is very small). We set pollution form neighboring pixel
      grid_data_PM25_phil_ny_NAD_2050_population_highCDR %>%
        filter(!is.na(PM25_AVG)) -> grid_data_PM25_phil_ny_NAD_2050_population_highCDR
      ...
      grid_data_PM25_phil_ny_NAD_2050_population_highCDR %>%
        ...
        mutate(Metric = "D24HourMean",
               `Seasonal Metric` = "QuarterlyMean",
               `Annual Metric` = "Mean") -> grid_data_PM25_phil_ny_NAD_2050_population_highCDR_final
      
      # 2. Save file
      write.csv(grid_data_PM25_phil_ny_NAD_2050_population_highCDR_final, row.names = FALSE, 
                file = paste("output_data/BenMAP_files/sensitivity_siting/Philadelphia/Date_", today_date,"PM25_2050_population_highCDR_Philadelphia.csv"))
      
      st_write(grid_data_PM25_phil_ny_NAD_2050_population_highCDR, 
               "output_data/shapefiles/sensitivity_siting/Philadelphia/1_km_PM25_2050_population_highCDR_Philadelphia.shp")
      grid_data_PM25_phil_ny_NAD_2050_population_highCDR <- st_read( "output_data/shapefiles/sensitivity_siting/Philadelphia/1_km_PM25_2050_population_highCDR_Philadelphia.shp")
      ...
      grid_data_PM25_phil_ny_NAD_2050_population_highCDR_final %>%
        left_join(Philadelphia_pop_total, by = c("Row", "Column")) %>%
        filter(!is.na(Values))-> Philadelphia_pop_PM25_total
      ...
      total_population <- sum(Philadelphia_pop_PM25_total$population)
      # Calculate population-weighted average pollution
      Philadelphia_mean_BASE_2050_population_highCDR <- sum_products / total_population
      mean(Philadelphia_pop_PM25_total$Values)
      
      #Set values > 9 to 9 for graphing purposes
      #Based on the EPA -> https://www.epa.gov/pm-pollution/national-ambient-air-quality-standards-naaqs-pm
      grid_data_PM25_phil_ny_NAD_2050_population_highCDR %>%
        mutate(PM25_AVG_graph = if_else(PM25_AVG >= 9, 9, PM25_AVG)) -> grid_data_PM25_phil_ny_NAD_2050_population_highCDR_graph
      
      # Perform intersection to retain only pixels within the city boundary
      grid_data_PM25_phil_ny_NAD_2050_population_highCDR_graph_crop <- st_intersection(grid_data_PM25_phil_ny_NAD_2050_population_highCDR_graph, cbsa_shapefile_phil)
      
      p <- ggplot() +
        geom_sf(data = grid_data_PM25_phil_ny_NAD_2050_population_highCDR_graph_crop, aes(fill = PM25_AVG_graph), color = NA, size = 0) +
        scale_fill_gradientn(colors = colors,
                             values = scales::rescale(c(0, 5, 10)),  # Set the midpoint to 5
                             limits = c(0, 10),  # Adjust limits to match your data range 
                             name = "PM2.5 (ug/m3)") +
        ggtitle("2050 High-CDR LA Mean 1km Yearly PM2.5 (ug/m3). Sensitivity population") +
        geom_text(aes(label = paste("Population weighted mean:", round(Philadelphia_mean_BASE_2050_population_highCDR, 2), "ug/m3")),
                  x = Inf, y = -Inf, hjust = 1, vjust = -0.6, size = 5)+
        annotation_scale() +
        geom_sf(data = cbsa_shapefile_phil, color = "black", fill = NA)+
        theme_void() +
                geom_point(data = NULL, aes(x = -75.16326872597556,  y =  39.95279997140329), shape = 1, color = "#95C623", size = 6, stroke = 2) #Philadelphia city hall
      ggsave("figures/BenMAP/sensitivity_siting/pollution_V2/1km_PM25_2050_population_highCDR_Philadelphia.png", p, width = 16, height = 12, units = "in", dpi = 600)
      ggsave("figures/BenMAP/sensitivity_siting/pollution_V2/1km_PM25_2050_population_highCDR_Philadelphia.svg", p, width = 16, height = 12, units = "in", dpi = 600)
      print(p)
      
      ##### 11.2.2. Low CDR ####
      # 2.1.1 Get mean value 
      phil_ny_nc_1km_stack_2050_population_lowCDR <- stack("data/WRF-CMAQ/sensitivity/9_phil_ny/Population/Dailymean.1km.LowCCS_Population.2050.PM25_AVG.nc", varname = "PM25_AVG")
      phil_ny_mean_raster_2050_population_lowCDR <- calc(phil_ny_nc_1km_stack_2050_population_lowCDR, fun = mean, na.rm = TRUE, na.rm.args = list(na.rm = TRUE))
      plot(phil_ny_mean_raster_2050_population_lowCDR, col = rev(magma(20)), 
           main = " phil_ny 2050 Yearly Mean PM2.5 Concentrations (ug/m3) population Sensitivity low-CDR")
      ...
      # 1. Check the CRS
      st_crs(grid_data_northeast)
      crs(phil_ny_mean_raster_2050_population_lowCDR)
      #2. Print extent information
      print(extent(phil_ny_mean_raster_2050_population_lowCDR))
      print(st_bbox(grid_data_northeast))
      #3. Check resolution
      res(phil_ny_mean_raster_2050_population_lowCDR)
      bbox <- st_bbox(grid_data_northeast)
      resolution <- c((bbox$xmax - bbox$xmin) / num_cols,
                      (bbox$ymax - bbox$ymin) / num_rows) 
      print(resolution)
      # 4. Set the CRS for mean_raster_2019
      crs(phil_ny_mean_raster_2050_population_lowCDR) <- st_crs(grid_data_northeast)$proj4string
      # 5. Set the extent of mean_raster_aligned to match grid_data
      extent(phil_ny_mean_raster_2050_population_lowCDR) <- extent(grid_data_northeast)
      #6. Filter pixels 
      grid_data_northeast %>%
        filter(pixel_ID %in% phil_pixels) -> grid_data_northeast_filtered_phil
      ...
      # 2.1.3. Get values from raster and assign them to my grid 
      # 1. Extract values using st_extract
      system.time({phil_ny_values <- raster::extract(phil_ny_mean_raster_2050_population_lowCDR, grid_data_northeast_filtered_phil)}) #391 s = ~6.5 min
      grid_data_PM25_phil_ny_2050_population_lowCDR <- grid_data_northeast_filtered_phil
      # 2. Assign the values to the PM25_AVG column
      grid_data_PM25_phil_ny_2050_population_lowCDR$PM25_AVG <- phil_ny_values
      grid_data_PM25_phil_ny_2050_population_lowCDR$PM25_AVG <- as.numeric(gsub("[^0-9.-]", "", grid_data_PM25_phil_ny_2050_population_lowCDR$PM25_AVG))
      
      # 2.1.4. Prepare output table
      # 1. Convert to NAD83
      grid_data_PM25_phil_ny_NAD_2050_population_lowCDR <- st_transform(grid_data_PM25_phil_ny_2050_population_lowCDR, 
                                                                             crs = st_crs("+proj=longlat +datum=NAD83"))
      
      #Fix one pixel with NA (this may be because the intersection fraction is very small). We set pollution form neighboring pixel
      grid_data_PM25_phil_ny_NAD_2050_population_lowCDR %>%
        filter(!is.na(PM25_AVG)) -> grid_data_PM25_phil_ny_NAD_2050_population_lowCDR
      ...
      grid_data_PM25_phil_ny_NAD_2050_population_lowCDR %>%
        ...
        mutate(Metric = "D24HourMean",
               `Seasonal Metric` = "QuarterlyMean",
               `Annual Metric` = "Mean") -> grid_data_PM25_phil_ny_NAD_2050_population_lowCDR_final
      
      # 2. Save file
      write.csv(grid_data_PM25_phil_ny_NAD_2050_population_lowCDR_final, row.names = FALSE, 
                file = paste("output_data/BenMAP_files/sensitivity_siting/Philadelphia/Date_", today_date,"PM25_2050_population_lowCDR_Philadelphia.csv"))
      
      st_write(grid_data_PM25_phil_ny_NAD_2050_population_lowCDR, 
               "output_data/shapefiles/sensitivity_siting/Philadelphia/1_km_PM25_2050_population_lowCDR_Philadelphia.shp")
      grid_data_PM25_phil_ny_NAD_2050_population_lowCDR <- st_read( "output_data/shapefiles/sensitivity_siting/Philadelphia/1_km_PM25_2050_population_lowCDR_Philadelphia.shp")
      ...
      grid_data_PM25_phil_ny_NAD_2050_population_lowCDR_final %>%
        left_join(Philadelphia_pop_total, by = c("Row", "Column")) %>%
        filter(!is.na(Values))-> Philadelphia_pop_PM25_total
      ...
      total_population <- sum(Philadelphia_pop_PM25_total$population)
      # Calculate population-weighted average pollution
      Philadelphia_mean_BASE_2050_population_lowCDR <- sum_products / total_population
      mean(Philadelphia_pop_PM25_total$Values)
      
      #Set values > 9 to 9 for graphing purposes
      #Based on the EPA -> https://www.epa.gov/pm-pollution/national-ambient-air-quality-standards-naaqs-pm
      grid_data_PM25_phil_ny_NAD_2050_population_lowCDR %>%
        mutate(PM25_AVG_graph = if_else(PM25_AVG >= 9, 9, PM25_AVG)) -> grid_data_PM25_phil_ny_NAD_2050_population_lowCDR_graph
      
      # Perform intersection to retain only pixels within the city boundary
      grid_data_PM25_phil_ny_NAD_2050_population_lowCDR_graph_crop <- st_intersection(grid_data_PM25_phil_ny_NAD_2050_population_lowCDR_graph, cbsa_shapefile_phil)
      
      p <- ggplot() +
        geom_sf(data = grid_data_PM25_phil_ny_NAD_2050_population_lowCDR_graph_crop, aes(fill = PM25_AVG_graph), color = NA, size = 0) +
        scale_fill_gradientn(colors = colors,
                             values = scales::rescale(c(0, 5, 10)),  # Set the midpoint to 5
                             limits = c(0, 10),  # Adjust limits to match your data range 
                             name = "PM2.5 (ug/m3)") +
        ggtitle("2050 low-CDR LA Mean 1km Yearly PM2.5 (ug/m3). Sensitivity population") +
        geom_text(aes(label = paste("Population weighted mean:", round(Philadelphia_mean_BASE_2050_population_lowCDR, 2), "ug/m3")),
                  x = Inf, y = -Inf, hjust = 1, vjust = -0.6, size = 5)+
        annotation_scale() +
        geom_sf(data = cbsa_shapefile_phil, color = "black", fill = NA)+
        theme_void() +
                geom_point(data = NULL, aes(x = -75.16326872597556,  y =  39.95279997140329), shape = 1, color = "#95C623", size = 6, stroke = 2) #Philadelphia city hall
      ggsave("figures/BenMAP/sensitivity_siting/pollution_V2/1km_PM25_2050_population_lowCDR_Philadelphia.png", p, width = 16, height = 12, units = "in", dpi = 600)
      ggsave("figures/BenMAP/sensitivity_siting/pollution_V2/1km_PM25_2050_population_lowCDR_Philadelphia.svg", p, width = 16, height = 12, units = "in", dpi = 600)
      print(p)
    
    #### 11.3. Income ####
      ##### 11.3.1. High CDR ####
      # 2.1.1 Get mean value 
      phil_ny_nc_1km_stack_2050_income_highCDR <- stack("data/WRF-CMAQ/sensitivity/9_phil_ny/Income/Dailymean.1km.HighCCS_Income.2050.PM25_AVG.nc", varname = "PM25_AVG")
      phil_ny_mean_raster_2050_income_highCDR <- calc(phil_ny_nc_1km_stack_2050_income_highCDR, fun = mean, na.rm = TRUE, na.rm.args = list(na.rm = TRUE))
      plot(phil_ny_mean_raster_2050_income_highCDR, col = rev(magma(20)), 
           main = " phil_ny 2050 Yearly Mean PM2.5 Concentrations (ug/m3) income Sensitivity High-CDR")
      ...
      # 1. Check the CRS
      st_crs(grid_data_northeast)
      crs(phil_ny_mean_raster_2050_income_highCDR)
      #2. Print extent information
      print(extent(phil_ny_mean_raster_2050_income_highCDR))
      print(st_bbox(grid_data_northeast))
      #3. Check resolution
      res(phil_ny_mean_raster_2050_income_highCDR)
      bbox <- st_bbox(grid_data_northeast)
      resolution <- c((bbox$xmax - bbox$xmin) / num_cols,
                      (bbox$ymax - bbox$ymin) / num_rows) 
      print(resolution)
      # 4. Set the CRS for mean_raster_2019
      crs(phil_ny_mean_raster_2050_income_highCDR) <- st_crs(grid_data_northeast)$proj4string
      # 5. Set the extent of mean_raster_aligned to match grid_data
      extent(phil_ny_mean_raster_2050_income_highCDR) <- extent(grid_data_northeast)
      #6. Filter pixels 
      grid_data_northeast %>%
        filter(pixel_ID %in% phil_pixels) -> grid_data_northeast_filtered_phil
      ...
      # 2.1.3. Get values from raster and assign them to my grid 
      # 1. Extract values using st_extract
      system.time({phil_ny_values <- raster::extract(phil_ny_mean_raster_2050_income_highCDR, grid_data_northeast_filtered_phil)}) #391 s = ~6.5 min
      grid_data_PM25_phil_ny_2050_income_highCDR <- grid_data_northeast_filtered_phil
      # 2. Assign the values to the PM25_AVG column
      grid_data_PM25_phil_ny_2050_income_highCDR$PM25_AVG <- phil_ny_values
      grid_data_PM25_phil_ny_2050_income_highCDR$PM25_AVG <- as.numeric(gsub("[^0-9.-]", "", grid_data_PM25_phil_ny_2050_income_highCDR$PM25_AVG))
      
      # 2.1.4. Prepare output table
      # 1. Convert to NAD83
      grid_data_PM25_phil_ny_NAD_2050_income_highCDR <- st_transform(grid_data_PM25_phil_ny_2050_income_highCDR, 
                                                                          crs = st_crs("+proj=longlat +datum=NAD83"))
      
      #Fix one pixel with NA (this may be because the intersection fraction is very small). We set pollution form neighboring pixel
      grid_data_PM25_phil_ny_NAD_2050_income_highCDR %>%
        filter(!is.na(PM25_AVG)) -> grid_data_PM25_phil_ny_NAD_2050_income_highCDR
      ...
      grid_data_PM25_phil_ny_NAD_2050_income_highCDR %>%
        ...
        mutate(Metric = "D24HourMean",
               `Seasonal Metric` = "QuarterlyMean",
               `Annual Metric` = "Mean") -> grid_data_PM25_phil_ny_NAD_2050_income_highCDR_final
      
      # 2. Save file
      write.csv(grid_data_PM25_phil_ny_NAD_2050_income_highCDR_final, row.names = FALSE, 
                file = paste("output_data/BenMAP_files/sensitivity_siting/Philadelphia/Date_", today_date,"PM25_2050_income_highCDR_Philadelphia.csv"))
      
      st_write(grid_data_PM25_phil_ny_NAD_2050_income_highCDR, 
               "output_data/shapefiles/sensitivity_siting/Philadelphia/1_km_PM25_2050_income_highCDR_Philadelphia.shp")
      grid_data_PM25_phil_ny_NAD_2050_income_highCDR <- st_read("output_data/shapefiles/sensitivity_siting/Philadelphia/1_km_PM25_2050_income_highCDR_Philadelphia.shp")
      ...
      grid_data_PM25_phil_ny_NAD_2050_income_highCDR_final %>%
        left_join(Philadelphia_pop_total, by = c("Row", "Column")) %>%
        filter(!is.na(Values))-> Philadelphia_pop_PM25_total
      ...
      total_population <- sum(Philadelphia_pop_PM25_total$population)
      # Calculate population-weighted average pollution
      Philadelphia_mean_BASE_2050_income_highCDR <- sum_products / total_population
      mean(Philadelphia_pop_PM25_total$Values)
      
      #Set values > 9 to 9 for graphing purposes
      #Based on the EPA -> https://www.epa.gov/pm-pollution/national-ambient-air-quality-standards-naaqs-pm
      grid_data_PM25_phil_ny_NAD_2050_income_highCDR %>%
        mutate(PM25_AVG_graph = if_else(PM25_AVG >= 9, 9, PM25_AVG)) -> grid_data_PM25_phil_ny_NAD_2050_income_highCDR_graph
      
      # Perform intersection to retain only pixels within the city boundary
      grid_data_PM25_phil_ny_NAD_2050_income_highCDR_graph_crop <- st_intersection(grid_data_PM25_phil_ny_NAD_2050_income_highCDR_graph, cbsa_shapefile_phil)
      
      p <- ggplot() +
        geom_sf(data = grid_data_PM25_phil_ny_NAD_2050_income_highCDR_graph_crop, aes(fill = PM25_AVG_graph), color = NA, size = 0) +
        scale_fill_gradientn(colors = colors,
                             values = scales::rescale(c(0, 5, 10)),  # Set the midpoint to 5
                             limits = c(0, 10),  # Adjust limits to match your data range 
                             name = "PM2.5 (ug/m3)") +
        ggtitle("2050 High-CDR Philadelphia Mean 1km Yearly PM2.5 (ug/m3). Sensitivity income") +
        geom_text(aes(label = paste("Population weighted mean:", round(Philadelphia_mean_BASE_2050_income_highCDR, 2), "ug/m3")),
                  x = Inf, y = -Inf, hjust = 1, vjust = -0.6, size = 5)+
        annotation_scale() +
        geom_sf(data = cbsa_shapefile_phil, color = "black", fill = NA)+
        theme_void() +
                geom_point(data = NULL, aes(x = -75.16326872597556,  y =  39.95279997140329), shape = 1, color = "#95C623", size = 6, stroke = 2) #Philadelphia city hall
      ggsave("figures/BenMAP/sensitivity_siting/pollution_V2/1km_PM25_2050_income_highCDR_Philadelphia.png", p, width = 16, height = 12, units = "in", dpi = 600)
      ggsave("figures/BenMAP/sensitivity_siting/pollution_V2/1km_PM25_2050_income_highCDR_Philadelphia.svg", p, width = 16, height = 12, units = "in", dpi = 600)
      print(p)
      
      ##### 11.3.2. Low CDR ####
      # 2.1.1 Get mean value 
      phil_ny_nc_1km_stack_2050_income_lowCDR <- stack("data/WRF-CMAQ/sensitivity/9_phil_ny/Income/Dailymean.1km.LowCCS_Income.2050.PM25_AVG.nc", varname = "PM25_AVG")
      phil_ny_mean_raster_2050_income_lowCDR <- calc(phil_ny_nc_1km_stack_2050_income_lowCDR, fun = mean, na.rm = TRUE, na.rm.args = list(na.rm = TRUE))
      plot(phil_ny_mean_raster_2050_income_lowCDR, col = rev(magma(20)), 
           main = " phil_ny 2050 Yearly Mean PM2.5 Concentrations (ug/m3) income Sensitivity low-CDR")
      ...
      # 1. Check the CRS
      st_crs(grid_data_northeast)
      crs(phil_ny_mean_raster_2050_income_lowCDR)
      #2. Print extent information
      print(extent(phil_ny_mean_raster_2050_income_lowCDR))
      print(st_bbox(grid_data_northeast))
      #3. Check resolution
      res(phil_ny_mean_raster_2050_income_lowCDR)
      bbox <- st_bbox(grid_data_northeast)
      resolution <- c((bbox$xmax - bbox$xmin) / num_cols,
                      (bbox$ymax - bbox$ymin) / num_rows) 
      print(resolution)
      # 4. Set the CRS for mean_raster_2019
      crs(phil_ny_mean_raster_2050_income_lowCDR) <- st_crs(grid_data_northeast)$proj4string
      # 5. Set the extent of mean_raster_aligned to match grid_data
      extent(phil_ny_mean_raster_2050_income_lowCDR) <- extent(grid_data_northeast)
      #6. Filter pixels 
      grid_data_northeast %>%
        filter(pixel_ID %in% phil_pixels) -> grid_data_northeast_filtered_phil
      ...
      # 2.1.3. Get values from raster and assign them to my grid 
      # 1. Extract values using st_extract
      system.time({phil_ny_values <- raster::extract(phil_ny_mean_raster_2050_income_lowCDR, grid_data_northeast_filtered_phil)}) #391 s = ~6.5 min
      grid_data_PM25_phil_ny_2050_income_lowCDR <- grid_data_northeast_filtered_phil
      # 2. Assign the values to the PM25_AVG column
      grid_data_PM25_phil_ny_2050_income_lowCDR$PM25_AVG <- phil_ny_values
      grid_data_PM25_phil_ny_2050_income_lowCDR$PM25_AVG <- as.numeric(gsub("[^0-9.-]", "", grid_data_PM25_phil_ny_2050_income_lowCDR$PM25_AVG))
      
      # 2.1.4. Prepare output table
      # 1. Convert to NAD83
      grid_data_PM25_phil_ny_NAD_2050_income_lowCDR <- st_transform(grid_data_PM25_phil_ny_2050_income_lowCDR, 
                                                                         crs = st_crs("+proj=longlat +datum=NAD83"))
      
      #Fix one pixel with NA (this may be because the intersection fraction is very small). We set pollution form neighboring pixel
      grid_data_PM25_phil_ny_NAD_2050_income_lowCDR %>%
        filter(!is.na(PM25_AVG)) -> grid_data_PM25_phil_ny_NAD_2050_income_lowCDR
      ...
      grid_data_PM25_phil_ny_NAD_2050_income_lowCDR %>%
        ...
        mutate(Metric = "D24HourMean",
               `Seasonal Metric` = "QuarterlyMean",
               `Annual Metric` = "Mean") -> grid_data_PM25_phil_ny_NAD_2050_income_lowCDR_final
      
      # 2. Save file
      write.csv(grid_data_PM25_phil_ny_NAD_2050_income_lowCDR_final, row.names = FALSE, 
                file = paste("output_data/BenMAP_files/sensitivity_siting/Philadelphia/Date_", today_date,"PM25_2050_income_lowCDR_Philadelphia.csv"))
      
      st_write(grid_data_PM25_phil_ny_NAD_2050_income_lowCDR, 
               "output_data/shapefiles/sensitivity_siting/Philadelphia/1_km_PM25_2050_income_lowCDR_Philadelphia.shp")
      grid_data_PM25_phil_ny_NAD_2050_income_lowCDR <- st_read("output_data/shapefiles/sensitivity_siting/Philadelphia/1_km_PM25_2050_income_lowCDR_Philadelphia.shp")
      ...
      grid_data_PM25_phil_ny_NAD_2050_income_lowCDR_final %>%
        left_join(Philadelphia_pop_total, by = c("Row", "Column")) %>%
        filter(!is.na(Values))-> Philadelphia_pop_PM25_total
      ...
      total_population <- sum(Philadelphia_pop_PM25_total$population)
      # Calculate population-weighted average pollution
      Philadelphia_mean_BASE_2050_income_lowCDR <- sum_products / total_population
      mean(Philadelphia_pop_PM25_total$Values)
      
      #Set values > 9 to 9 for graphing purposes
      #Based on the EPA -> https://www.epa.gov/pm-pollution/national-ambient-air-quality-standards-naaqs-pm
      grid_data_PM25_phil_ny_NAD_2050_income_lowCDR %>%
        mutate(PM25_AVG_graph = if_else(PM25_AVG >= 9, 9, PM25_AVG)) -> grid_data_PM25_phil_ny_NAD_2050_income_lowCDR_graph
      
      # Perform intersection to retain only pixels within the city boundary
      grid_data_PM25_phil_ny_NAD_2050_income_lowCDR_graph_crop <- st_intersection(grid_data_PM25_phil_ny_NAD_2050_income_lowCDR_graph, cbsa_shapefile_phil)
      
      p <- ggplot() +
        geom_sf(data = grid_data_PM25_phil_ny_NAD_2050_income_lowCDR_graph_crop, aes(fill = PM25_AVG_graph), color = NA, size = 0) +
        scale_fill_gradientn(colors = colors,
                             values = scales::rescale(c(0, 5, 10)),  # Set the midpoint to 5
                             limits = c(0, 10),  # Adjust limits to match your data range 
                             name = "PM2.5 (ug/m3)") +
        ggtitle("2050 low-CDR Philadelphia Mean 1km Yearly PM2.5 (ug/m3). Sensitivity income") +
        geom_text(aes(label = paste("Population weighted mean:", round(Philadelphia_mean_BASE_2050_income_lowCDR, 2), "ug/m3")),
                  x = Inf, y = -Inf, hjust = 1, vjust = -0.6, size = 5)+
        annotation_scale() +
        geom_sf(data = cbsa_shapefile_phil, color = "black", fill = NA)+
        theme_void() +
                geom_point(data = NULL, aes(x = -75.16326872597556,  y =  39.95279997140329), shape = 1, color = "#95C623", size = 6, stroke = 2) #Philadelphia city hall
      ggsave("figures/BenMAP/sensitivity_siting/pollution_V2/1km_PM25_2050_income_lowCDR_Philadelphia.png", p, width = 16, height = 12, units = "in", dpi = 600)
      ggsave("figures/BenMAP/sensitivity_siting/pollution_V2/1km_PM25_2050_income_lowCDR_Philadelphia.svg", p, width = 16, height = 12, units = "in", dpi = 600)
      print(p)
    ...
    #First get all plots into 1:
    # 1. Combine base + sensitivities into one sf object
      all_scenarios_phil <- bind_rows(
        grid_data_PM25_Philadelphia_NAD_2050_high %>% mutate(Scenario = "Base High CDR"),
        grid_data_PM25_Philadelphia_NAD_2050_low %>% mutate(Scenario = "Base Low CDR"),
        grid_data_PM25_phil_ny_NAD_2050_population_highCDR %>% mutate(Scenario = "Population High CDR"),
        grid_data_PM25_phil_ny_NAD_2050_minority_highCDR %>% mutate(Scenario = "Minority High CDR"),
        grid_data_PM25_phil_ny_NAD_2050_income_highCDR %>% mutate(Scenario = "Income High CDR"),
        grid_data_PM25_phil_ny_NAD_2050_population_lowCDR %>% mutate(Scenario = "Population Low CDR"),
        grid_data_PM25_phil_ny_NAD_2050_minority_lowCDR %>% mutate(Scenario = "Minority Low CDR"),
        grid_data_PM25_phil_ny_NAD_2050_income_lowCDR %>% mutate(Scenario = "Income Low CDR")
      )
      ...
      all_scenarios_phil <- all_scenarios_phil %>%
        mutate(Scenario = factor(Scenario, levels = scenario_order))
    
      #3. Crop to Philadelphia boundary
      all_scenarios_crop_phil <- st_intersection(all_scenarios_phil, cbsa_shapefile_phil)
    
      # 4. Plot using your custom colors
      p_facets_all_phil <- ggplot(all_scenarios_crop_phil) +
        geom_sf(aes(fill = PM25_AVG), color = NA, size = 0) +
        geom_sf(data = cbsa_shapefile_phil, color = "black", fill = NA) +
        scale_fill_gradientn(
          colors = colors,
        ...
        facet_wrap(~Scenario, ncol = 4) +  # 8 facets, 2 rows of 4
        theme_void() +
        ggtitle("2050 Philadelphia 1km PM2.5 Concentrations (Base + Sensitivity Scenarios)") +
        geom_point(aes(x = -75.1652, y = 39.9526), shape = 1, color = "#95C623", size = 6, stroke = 2) # Philadelphia City Hall
      ggsave("figures/BenMAP/sensitivity_siting/pollution_V2/1km_PM25_2050_Philadelphia_all_scenarios.png",
             p_facets_all_phil, width = 20, height = 12, units = "in", dpi = 600)
      ggsave("figures/BenMAP/sensitivity_siting/pollution_V2/1km_PM25_2050_Philadelphia_all_scenarios.svg",
             p_facets_all_phil, width = 20, height = 12, units = "in", dpi = 600)
    
      #### difference ####
    # List of sensitivity sf objects
      sf_list_phil <- list(
        minority_highCDR = grid_data_PM25_phil_ny_NAD_2050_minority_highCDR,
        minority_lowCDR  = grid_data_PM25_phil_ny_NAD_2050_minority_lowCDR,
        population_highCDR = grid_data_PM25_phil_ny_NAD_2050_population_highCDR,
        population_lowCDR  = grid_data_PM25_phil_ny_NAD_2050_population_lowCDR,
        income_highCDR = grid_data_PM25_phil_ny_NAD_2050_income_highCDR,
        income_lowCDR  = grid_data_PM25_phil_ny_NAD_2050_income_lowCDR
      )
      
      # Corresponding "base" datasets
      base_list_phil <- list(
        minority_highCDR = grid_data_PM25_Philadelphia_NAD_2050_high,
        minority_lowCDR  = grid_data_PM25_Philadelphia_NAD_2050_low,
        population_highCDR = grid_data_PM25_Philadelphia_NAD_2050_high,
        population_lowCDR  = grid_data_PM25_Philadelphia_NAD_2050_low,
        income_highCDR = grid_data_PM25_Philadelphia_NAD_2050_high,
        income_lowCDR  = grid_data_PM25_Philadelphia_NAD_2050_low
      )
    ...
    # Compute differences
    diff_list_phil <- mapply(compute_difference, sf_list_phil, base_list_phil, SIMPLIFY = FALSE)
    names(diff_list_phil) <- scenario_labels
    
    # Combine all differences into one sf
    diff_combined_phil <- bind_rows(
      lapply(seq_along(diff_list_phil), function(i) {
        diff_list_phil[[i]] %>% mutate(Scenario = scenario_labels[i])
      })
    )
    
    # Crop to Philadelphia
    diff_combined_crop_phil <- st_intersection(diff_combined_phil, cbsa_shapefile_phil)
    
    # Apply factor order
    scenario_order_diff <- c(
      "Population High CDR", "Minority High CDR", "Income High CDR",
      "Population Low CDR", "Minority Low CDR", "Income Low CDR"
    )
    
    diff_combined_crop_phil <- diff_combined_crop_phil %>%
      mutate(Scenario = factor(Scenario, levels = scenario_order_diff))
    
    # Plot differences
    p_facets_diff_phil <- ggplot(diff_combined_crop_phil) +
      geom_sf(aes(fill = PM25_diff), color = NA, size = 0) +
      geom_sf(data = cbsa_shapefile_phil, color = "black", fill = NA) +
      scale_fill_gradientn(colors = colors,
                           name = expression(Delta~PM[2.5]~(mu*g/m^3))) +
      ...
      theme_void() +
      ggtitle("2050 Philadelphia 1km PM2.5 Difference (Sensitivity - Base)")
    ggsave("figures/BenMAP/sensitivity_siting/pollution_V2/1km_PM25_2050_Philadelphia_DIFF_FACET_GRID.png",
           p_facets_diff_phil, width = 24, height = 16, units = "in", dpi = 600)
    ggsave("figures/BenMAP/sensitivity_siting/pollution_V2/1km_PM25_2050_Philadelphia_DIFF_FACET_GRID.svg",
           p_facets_diff_phil, width = 24, height = 16, units = "in", dpi = 600)
    
#### 14. DETROIT  ####
    #Read in original pollution files for diff plots
    grid_data_PM25_detroit_NAD_2050_high <-
      st_read("output_data/shapefiles/1km_V2/Detroit/1_km_PM25_2050_high_detroit.shp")
    
    grid_data_PM25_detroit_NAD_2050_low <-
      st_read("output_data/shapefiles/1km_V2/Detroit/1_km_PM25_2050_low_detroit.shp")
    #### 14.1. Minority ####
      #### 14.1.1. High CDR ####
    # 14.1.1 Get mean value 
    # Read the NetCDF file using stack
    detroit_nc_1km_stack_2050_minority_highCDR <- stack("data/WRF-CMAQ/sensitivity/11_detroit/Minority/Dailymean.1km.HighCCS_Minority.2050.PM25_AVG.nc", varname = "PM25_AVG")
    # Calculate the mean across all bands (365 days) for each cell
    detroit_mean_raster_2050_minority_highCDR <- calc(detroit_nc_1km_stack_2050_minority_highCDR, fun = mean, na.rm = TRUE, na.rm.args = list(na.rm = TRUE))
    # Plot the mean raster
    plot(detroit_mean_raster_2050_minority_highCDR, col = rev(magma(20)), main = "detroit 2050 high-CDR Yearly Mean PM2.5 Conc. (ug/m3). Sensitivity minority")
    ...
    # 1. Check the CRS
    st_crs(grid_data_detroit)
    crs(detroit_mean_raster_2050_minority_highCDR)
    #2. Print extent information
    print(extent(detroit_mean_raster_2050_minority_highCDR))
    print(st_bbox(grid_data_detroit))
    #3. Check resolution
    res(detroit_mean_raster_2050_minority_highCDR)
    # Get the bounding box of the grid_data
    bbox <- st_bbox(grid_data_detroit)
    # Calculate the resolution
    resolution <- c((bbox$xmax - bbox$xmin) / num_cols,
    ...
    # 2. Set the CRS for mean_raster_2050_minority_highCDR
    crs(detroit_mean_raster_2050_minority_highCDR) <- st_crs(grid_data_detroit)$proj4string
    
    # 3. Set the extent of mean_raster_aligned to match grid_data
    extent(detroit_mean_raster_2050_minority_highCDR) <- extent(grid_data_detroit)
    ...
    # 14.1.3. Get values from raster and assign them to my grid 
    # 1. Extract values using st_extract
    system.time({detroit_values <- raster::extract(detroit_mean_raster_2050_minority_highCDR, grid_data_detroit_filtered)}) #7311s
    grid_data_PM25_detroit_2050_minority_highCDR <- grid_data_detroit_filtered
    
    # 2. Assign the values to the PM25_AVG column
    grid_data_PM25_detroit_2050_minority_highCDR$PM25_AVG <- detroit_values
    grid_data_PM25_detroit_2050_minority_highCDR$PM25_AVG <- as.numeric(gsub("[^0-9.-]", "", grid_data_PM25_detroit_2050_minority_highCDR$PM25_AVG))
    
    # 14.1.4. Prepare output table
    # 1. Convert to NAD83
    grid_data_PM25_detroit_2050_minority_highCDR_NAD <- st_transform(grid_data_PM25_detroit_2050_minority_highCDR, crs = st_crs("+proj=longlat +datum=NAD83"))
    
    #Get right columns 
    grid_data_PM25_detroit_2050_minority_highCDR_NAD %>%
      st_drop_geometry() %>%
      dplyr::select(-pixel_ID) %>%
      rename(Column = col,
             Row = row,
      ...
      mutate(Metric = "D24HourMean",
             `Seasonal Metric` = "QuarterlyMean",
             `Annual Metric` = "Mean") -> grid_data_PM25_detroit_2050_minority_highCDR_NAD_filtered_final
    
    # 2. Save file
    #Csv
    write.csv(grid_data_PM25_detroit_2050_minority_highCDR_NAD_filtered_final, row.names = FALSE, file = paste("output_data/BenMAP_files/sensitivity_siting/Detroit/Date_", today_date,"PM25_2050_minority_highCDR_Detroit.csv"))
    
    #Shapefile
    st_write(grid_data_PM25_detroit_2050_minority_highCDR_NAD, "output_data/shapefiles/sensitivity_siting/Detroit/1_km_PM25_2050_minority_highCDR_detroit.shp")
    grid_data_PM25_detroit_2050_minority_highCDR_NAD <- st_read("output_data/shapefiles/sensitivity_siting/Detroit/1_km_PM25_2050_minority_highCDR_detroit.shp")
    ...
    grid_data_PM25_detroit_2050_minority_highCDR_NAD_filtered_final %>%
      left_join(detroit_pop_total, by = c("Row", "Column")) %>%
      filter(!is.na(Values))-> detroit_pop_PM25_total
    ...
    #Based on the EPA -> https://www.epa.gov/pm-pollution/national-ambient-air-quality-standards-naaqs-pm
    grid_data_PM25_detroit_2050_minority_highCDR_NAD %>%
      mutate(PM25_AVG_graph = if_else(PM25_AVG >= 9, 9, PM25_AVG)) -> grid_data_PM25_detroit_2050_minority_highCDR_NAD_filtered_graph
    
    #Perform intersection to retain only pixels within the city boundary
    grid_data_PM25_detroit_2050_minority_highCDR_NAD_filtered_graph_crop <- st_intersection(grid_data_PM25_detroit_2050_minority_highCDR_NAD_filtered_graph, cbsa_shapefile_detroit)
    
    p <- ggplot() +
      geom_sf(data = grid_data_PM25_detroit_2050_minority_highCDR_NAD_filtered_graph_crop, aes(fill = PM25_AVG_graph), color = NA, size = 0) +
      scale_fill_gradientn(colors = colors,         
                           values = scales::rescale(c(0, 5, 10)),  # Set the midpoint to 5
                           limits = c(0, 10),  # Adjust limits to match your data range
                           name = "PM2.5 (ug/m3)"       ) +
      ggtitle("2050 High-CDR Detroit Mean 1km Yearly PM2.5 (ug/m3) Minority sensitivity") +
      geom_text(aes(label = paste("Population weighted mean:", round(detroit_mean_BASE_2019, 2), "ug/m3")),
                x = Inf, y = -Inf, hjust = 1, vjust = -0.6, size = 5) +
      annotation_scale() + 
      geom_sf(data = cbsa_shapefile_detroit, color = "black", fill = NA)+
      theme_void() +
      geom_point(data = NULL, aes(x = -83.04362102880836,  y =  42.32974049721709), shape = 1, color = "#95C623", size = 6, stroke = 2) #Detroit city hall
    ggsave("figures/BenMAP/sensitivity_siting/pollution_V2/1km_PM25_2050_minority_highCDR_detroit.png", p, width = 16, height = 12, units = "in", dpi = 600)
    ggsave("figures/BenMAP/sensitivity_siting/pollution_V2/1km_PM25_2050_minority_highCDR_detroit.svg", p, width = 16, height = 12, units = "in", dpi = 600)
    print(p)   
    
      #### 14.1.2. Low CDR ####
    # 14.1.1 Get mean value 
    # Read the NetCDF file using stack
    detroit_nc_1km_stack_2050_minority_lowCDR <- stack("data/WRF-CMAQ/sensitivity/11_detroit/Minority/Dailymean.1km.LowCCS_Minority.2050.PM25_AVG.nc", varname = "PM25_AVG")
    # Calculate the mean across all bands (365 days) for each cell
    detroit_mean_raster_2050_minority_lowCDR <- calc(detroit_nc_1km_stack_2050_minority_lowCDR, fun = mean, na.rm = TRUE, na.rm.args = list(na.rm = TRUE))
    # Plot the mean raster
    plot(detroit_mean_raster_2050_minority_lowCDR, col = rev(magma(20)), main = "detroit 2050 low-CDR Yearly Mean PM2.5 Conc. (ug/m3). Sensitivity minority")
    ...
    # 1. Check the CRS
    st_crs(grid_data_detroit)
    crs(detroit_mean_raster_2050_minority_lowCDR)
    #2. Print extent information
    print(extent(detroit_mean_raster_2050_minority_lowCDR))
    print(st_bbox(grid_data_detroit))
    #3. Check resolution
    res(detroit_mean_raster_2050_minority_lowCDR)
    # Get the bounding box of the grid_data
    bbox <- st_bbox(grid_data_detroit)
    # Calculate the resolution
    resolution <- c((bbox$xmax - bbox$xmin) / num_cols,
    ...
    # 2. Set the CRS for mean_raster_2050_minority_lowCDR
    crs(detroit_mean_raster_2050_minority_lowCDR) <- st_crs(grid_data_detroit)$proj4string
    
    # 3. Set the extent of mean_raster_aligned to match grid_data
    extent(detroit_mean_raster_2050_minority_lowCDR) <- extent(grid_data_detroit)
    ...
    # 14.1.3. Get values from raster and assign them to my grid 
    # 1. Extract values using st_extract
    system.time({detroit_values <- raster::extract(detroit_mean_raster_2050_minority_lowCDR, grid_data_detroit_filtered)}) #311s
    grid_data_PM25_detroit_2050_minority_lowCDR <- grid_data_detroit_filtered
    
    # 2. Assign the values to the PM25_AVG column
    grid_data_PM25_detroit_2050_minority_lowCDR$PM25_AVG <- detroit_values
    grid_data_PM25_detroit_2050_minority_lowCDR$PM25_AVG <- as.numeric(gsub("[^0-9.-]", "", grid_data_PM25_detroit_2050_minority_lowCDR$PM25_AVG))
    
    # 14.1.4. Prepare output table
    # 1. Convert to NAD83
    grid_data_PM25_detroit_2050_minority_lowCDR_NAD <- st_transform(grid_data_PM25_detroit_2050_minority_lowCDR, crs = st_crs("+proj=longlat +datum=NAD83"))
    
    #Get right columns 
    grid_data_PM25_detroit_2050_minority_lowCDR_NAD %>%
      st_drop_geometry() %>%
      dplyr::select(-pixel_ID) %>%
      rename(Column = col,
             Row = row,
      ...
      mutate(Metric = "D24HourMean",
             `Seasonal Metric` = "QuarterlyMean",
             `Annual Metric` = "Mean") -> grid_data_PM25_detroit_2050_minority_lowCDR_NAD_filtered_final
    
    # 2. Save file
    #Csv
    write.csv(grid_data_PM25_detroit_2050_minority_lowCDR_NAD_filtered_final, row.names = FALSE, file = paste("output_data/BenMAP_files/sensitivity_siting/Detroit/Date_", today_date,"PM25_2050_minority_lowCDR_Detroit.csv"))
    
    #Shapefile
    st_write(grid_data_PM25_detroit_2050_minority_lowCDR_NAD, "output_data/shapefiles/sensitivity_siting/Detroit/1_km_PM25_2050_minority_lowCDR_detroit.shp")
    grid_data_PM25_detroit_2050_minority_lowCDR_NAD <- st_read("output_data/shapefiles/sensitivity_siting/Detroit/1_km_PM25_2050_minority_lowCDR_detroit.shp")
    ...
    grid_data_PM25_detroit_2050_minority_lowCDR_NAD_filtered_final %>%
      left_join(detroit_pop_total, by = c("Row", "Column")) %>%
      filter(!is.na(Values))-> detroit_pop_PM25_total
    ...
    #Based on the EPA -> https://www.epa.gov/pm-pollution/national-ambient-air-quality-standards-naaqs-pm
    grid_data_PM25_detroit_2050_minority_lowCDR_NAD %>%
      mutate(PM25_AVG_graph = if_else(PM25_AVG >= 9, 9, PM25_AVG)) -> grid_data_PM25_detroit_2050_minority_lowCDR_NAD_filtered_graph
    
    #Perform intersection to retain only pixels within the city boundary
    grid_data_PM25_detroit_2050_minority_lowCDR_NAD_filtered_graph_crop <- st_intersection(grid_data_PM25_detroit_2050_minority_lowCDR_NAD_filtered_graph, cbsa_shapefile_detroit)
    
    p <- ggplot() +
      geom_sf(data = grid_data_PM25_detroit_2050_minority_lowCDR_NAD_filtered_graph_crop, aes(fill = PM25_AVG_graph), color = NA, size = 0) +
      scale_fill_gradientn(colors = colors,         
                           values = scales::rescale(c(0, 5, 10)),  # Set the midpoint to 5
                           limits = c(0, 10),  # Adjust limits to match your data range
                           name = "PM2.5 (ug/m3)"       ) +
      ggtitle("2050 low-CDR Detroit Mean 1km Yearly PM2.5 (ug/m3) Minority sensitivity") +
      geom_text(aes(label = paste("Population weighted mean:", round(detroit_mean_BASE_2019, 2), "ug/m3")),
                x = Inf, y = -Inf, hjust = 1, vjust = -0.6, size = 5) +
      annotation_scale() + 
      geom_sf(data = cbsa_shapefile_detroit, color = "black", fill = NA)+
      theme_void() +
      geom_point(data = NULL, aes(x = -83.04362102880836,  y =  42.32974049721709), shape = 1, color = "#95C623", size = 6, stroke = 2) #Detroit city hall
    ggsave("figures/BenMAP/sensitivity_siting/pollution_V2/1km_PM25_2050_minority_lowCDR_detroit.png", p, width = 16, height = 12, units = "in", dpi = 600)
    ggsave("figures/BenMAP/sensitivity_siting/pollution_V2/1km_PM25_2050_minority_lowCDR_detroit.svg", p, width = 16, height = 12, units = "in", dpi = 600)
    print(p)   
    ...
    # 14.1.1 Get mean value 
    # Read the NetCDF file using stack
    detroit_nc_1km_stack_2050_population_highCDR <- stack("data/WRF-CMAQ/sensitivity/11_detroit/Population/Dailymean.1km.HighCCS_Population.2050.PM25_AVG.nc", varname = "PM25_AVG")
    # Calculate the mean across all bands (365 days) for each cell
    detroit_mean_raster_2050_population_highCDR <- calc(detroit_nc_1km_stack_2050_population_highCDR, fun = mean, na.rm = TRUE, na.rm.args = list(na.rm = TRUE))
    # Plot the mean raster
    plot(detroit_mean_raster_2050_population_highCDR, col = rev(magma(20)), main = "detroit 2050 high-CDR Yearly Mean PM2.5 Conc. (ug/m3). Sensitivity population")
    ...
    # 1. Check the CRS
    st_crs(grid_data_detroit)
    crs(detroit_mean_raster_2050_population_highCDR)
    #2. Print extent information
    print(extent(detroit_mean_raster_2050_population_highCDR))
    print(st_bbox(grid_data_detroit))
    #3. Check resolution
    res(detroit_mean_raster_2050_population_highCDR)
    # Get the bounding box of the grid_data
    bbox <- st_bbox(grid_data_detroit)
    # Calculate the resolution
    resolution <- c((bbox$xmax - bbox$xmin) / num_cols,
    ...
    # 2. Set the CRS for mean_raster_2050_population_highCDR
    crs(detroit_mean_raster_2050_population_highCDR) <- st_crs(grid_data_detroit)$proj4string
    
    # 3. Set the extent of mean_raster_aligned to match grid_data
    extent(detroit_mean_raster_2050_population_highCDR) <- extent(grid_data_detroit)
    ...
    # 14.1.3. Get values from raster and assign them to my grid 
    # 1. Extract values using st_extract
    system.time({detroit_values <- raster::extract(detroit_mean_raster_2050_population_highCDR, grid_data_detroit_filtered)}) #7311s
    grid_data_PM25_detroit_2050_population_highCDR <- grid_data_detroit_filtered
    
    # 2. Assign the values to the PM25_AVG column
    grid_data_PM25_detroit_2050_population_highCDR$PM25_AVG <- detroit_values
    grid_data_PM25_detroit_2050_population_highCDR$PM25_AVG <- as.numeric(gsub("[^0-9.-]", "", grid_data_PM25_detroit_2050_population_highCDR$PM25_AVG))
    
    # 14.1.4. Prepare output table
    # 1. Convert to NAD83
    grid_data_PM25_detroit_2050_population_highCDR_NAD <- st_transform(grid_data_PM25_detroit_2050_population_highCDR, crs = st_crs("+proj=longlat +datum=NAD83"))
    
    #Get right columns 
    grid_data_PM25_detroit_2050_population_highCDR_NAD %>%
      st_drop_geometry() %>%
      dplyr::select(-pixel_ID) %>%
      rename(Column = col,
             Row = row,
      ...
      mutate(Metric = "D24HourMean",
             `Seasonal Metric` = "QuarterlyMean",
             `Annual Metric` = "Mean") -> grid_data_PM25_detroit_2050_population_highCDR_NAD_filtered_final
    
    # 2. Save file
    #Csv
    write.csv(grid_data_PM25_detroit_2050_population_highCDR_NAD_filtered_final, row.names = FALSE, file = paste("output_data/BenMAP_files/sensitivity_siting/Detroit/Date_", today_date,"PM25_2050_population_highCDR_Detroit.csv"))
    
    #Shapefile
    st_write(grid_data_PM25_detroit_2050_population_highCDR_NAD, "output_data/shapefiles/sensitivity_siting/Detroit/1_km_PM25_2050_population_highCDR_detroit.shp")
    grid_data_PM25_detroit_2050_population_highCDR_NAD <- st_read("output_data/shapefiles/sensitivity_siting/Detroit/1_km_PM25_2050_population_highCDR_detroit.shp")
    ...
    grid_data_PM25_detroit_2050_population_highCDR_NAD_filtered_final %>%
      left_join(detroit_pop_total, by = c("Row", "Column")) %>%
      filter(!is.na(Values))-> detroit_pop_PM25_total
    ...
    #Based on the EPA -> https://www.epa.gov/pm-pollution/national-ambient-air-quality-standards-naaqs-pm
    grid_data_PM25_detroit_2050_population_highCDR_NAD %>%
      mutate(PM25_AVG_graph = if_else(PM25_AVG >= 9, 9, PM25_AVG)) -> grid_data_PM25_detroit_2050_population_highCDR_NAD_filtered_graph
    
    #Perform intersection to retain only pixels within the city boundary
    grid_data_PM25_detroit_2050_population_highCDR_NAD_filtered_graph_crop <- st_intersection(grid_data_PM25_detroit_2050_population_highCDR_NAD_filtered_graph, cbsa_shapefile_detroit)
    
    p <- ggplot() +
      geom_sf(data = grid_data_PM25_detroit_2050_population_highCDR_NAD_filtered_graph_crop, aes(fill = PM25_AVG_graph), color = NA, size = 0) +
      scale_fill_gradientn(colors = colors,         
                           values = scales::rescale(c(0, 5, 10)),  # Set the midpoint to 5
                           limits = c(0, 10),  # Adjust limits to match your data range
                           name = "PM2.5 (ug/m3)"       ) +
      ggtitle("2050 High-CDR Detroit Mean 1km Yearly PM2.5 (ug/m3) population sensitivity") +
      geom_text(aes(label = paste("Population weighted mean:", round(detroit_mean_BASE_2019, 2), "ug/m3")),
                x = Inf, y = -Inf, hjust = 1, vjust = -0.6, size = 5) +
      annotation_scale() + 
      geom_sf(data = cbsa_shapefile_detroit, color = "black", fill = NA)+
      theme_void() +
      geom_point(data = NULL, aes(x = -83.04362102880836,  y =  42.32974049721709), shape = 1, color = "#95C623", size = 6, stroke = 2) #Detroit city hall
    ggsave("figures/BenMAP/sensitivity_siting/pollution_V2/1km_PM25_2050_population_highCDR_detroit.png", p, width = 16, height = 12, units = "in", dpi = 600)
    ggsave("figures/BenMAP/sensitivity_siting/pollution_V2/1km_PM25_2050_population_highCDR_detroit.svg", p, width = 16, height = 12, units = "in", dpi = 600)
    print(p)   
    
      #### 14.2.2. Low CDR ####
    # 14.1.1 Get mean value 
    # Read the NetCDF file using stack
    detroit_nc_1km_stack_2050_population_lowCDR <- stack("data/WRF-CMAQ/sensitivity/11_detroit/Population/Dailymean.1km.LowCCS_Population.2050.PM25_AVG.nc", varname = "PM25_AVG")
    # Calculate the mean across all bands (365 days) for each cell
    detroit_mean_raster_2050_population_lowCDR <- calc(detroit_nc_1km_stack_2050_population_lowCDR, fun = mean, na.rm = TRUE, na.rm.args = list(na.rm = TRUE))
    # Plot the mean raster
    plot(detroit_mean_raster_2050_population_lowCDR, col = rev(magma(20)), main = "detroit 2050 low-CDR Yearly Mean PM2.5 Conc. (ug/m3). Sensitivity population")
    ...
    # 1. Check the CRS
    st_crs(grid_data_detroit)
    crs(detroit_mean_raster_2050_population_lowCDR)
    #2. Print extent information
    print(extent(detroit_mean_raster_2050_population_lowCDR))
    print(st_bbox(grid_data_detroit))
    #3. Check resolution
    res(detroit_mean_raster_2050_population_lowCDR)
    # Get the bounding box of the grid_data
    bbox <- st_bbox(grid_data_detroit)
    # Calculate the resolution
    resolution <- c((bbox$xmax - bbox$xmin) / num_cols,
    ...
    # 2. Set the CRS for mean_raster_2050_population_lowCDR
    crs(detroit_mean_raster_2050_population_lowCDR) <- st_crs(grid_data_detroit)$proj4string
    
    # 3. Set the extent of mean_raster_aligned to match grid_data
    extent(detroit_mean_raster_2050_population_lowCDR) <- extent(grid_data_detroit)
    ...
    # 14.1.3. Get values from raster and assign them to my grid 
    # 1. Extract values using st_extract
    system.time({detroit_values <- raster::extract(detroit_mean_raster_2050_population_lowCDR, grid_data_detroit_filtered)}) #311s
    grid_data_PM25_detroit_2050_population_lowCDR <- grid_data_detroit_filtered
    
    # 2. Assign the values to the PM25_AVG column
    grid_data_PM25_detroit_2050_population_lowCDR$PM25_AVG <- detroit_values
    grid_data_PM25_detroit_2050_population_lowCDR$PM25_AVG <- as.numeric(gsub("[^0-9.-]", "", grid_data_PM25_detroit_2050_population_lowCDR$PM25_AVG))
    
    # 14.1.4. Prepare output table
    # 1. Convert to NAD83
    grid_data_PM25_detroit_2050_population_lowCDR_NAD <- st_transform(grid_data_PM25_detroit_2050_population_lowCDR, crs = st_crs("+proj=longlat +datum=NAD83"))
    
    #Get right columns 
    grid_data_PM25_detroit_2050_population_lowCDR_NAD %>%
      st_drop_geometry() %>%
      dplyr::select(-pixel_ID) %>%
      rename(Column = col,
             Row = row,
      ...
      mutate(Metric = "D24HourMean",
             `Seasonal Metric` = "QuarterlyMean",
             `Annual Metric` = "Mean") -> grid_data_PM25_detroit_2050_population_lowCDR_NAD_filtered_final
    
    # 2. Save file
    #Csv
    write.csv(grid_data_PM25_detroit_2050_population_lowCDR_NAD_filtered_final, row.names = FALSE, file = paste("output_data/BenMAP_files/sensitivity_siting/Detroit/Date_", today_date,"PM25_2050_population_lowCDR_Detroit.csv"))
    
    #Shapefile
    st_write(grid_data_PM25_detroit_2050_population_lowCDR_NAD, "output_data/shapefiles/sensitivity_siting/Detroit/1_km_PM25_2050_population_lowCDR_detroit.shp")
    grid_data_PM25_detroit_2050_population_lowCDR_NAD <- st_read("output_data/shapefiles/sensitivity_siting/Detroit/1_km_PM25_2050_population_lowCDR_detroit.shp")
    ...
    grid_data_PM25_detroit_2050_population_lowCDR_NAD_filtered_final %>%
      left_join(detroit_pop_total, by = c("Row", "Column")) %>%
      filter(!is.na(Values))-> detroit_pop_PM25_total
    ...
    #Based on the EPA -> https://www.epa.gov/pm-pollution/national-ambient-air-quality-standards-naaqs-pm
    grid_data_PM25_detroit_2050_population_lowCDR_NAD %>%
      mutate(PM25_AVG_graph = if_else(PM25_AVG >= 9, 9, PM25_AVG)) -> grid_data_PM25_detroit_2050_population_lowCDR_NAD_filtered_graph
    
    #Perform intersection to retain only pixels within the city boundary
    grid_data_PM25_detroit_2050_population_lowCDR_NAD_filtered_graph_crop <- st_intersection(grid_data_PM25_detroit_2050_population_lowCDR_NAD_filtered_graph, cbsa_shapefile_detroit)
    
    p <- ggplot() +
      geom_sf(data = grid_data_PM25_detroit_2050_population_lowCDR_NAD_filtered_graph_crop, aes(fill = PM25_AVG_graph), color = NA, size = 0) +
      scale_fill_gradientn(colors = colors,         
                           values = scales::rescale(c(0, 5, 10)),  # Set the midpoint to 5
                           limits = c(0, 10),  # Adjust limits to match your data range
                           name = "PM2.5 (ug/m3)"       ) +
      ggtitle("2050 low-CDR Detroit Mean 1km Yearly PM2.5 (ug/m3) population sensitivity") +
      geom_text(aes(label = paste("Population weighted mean:", round(detroit_mean_BASE_2019, 2), "ug/m3")),
                x = Inf, y = -Inf, hjust = 1, vjust = -0.6, size = 5) +
      annotation_scale() + 
      geom_sf(data = cbsa_shapefile_detroit, color = "black", fill = NA)+
      theme_void() +
      geom_point(data = NULL, aes(x = -83.04362102880836,  y =  42.32974049721709), shape = 1, color = "#95C623", size = 6, stroke = 2) #Detroit city hall
    ggsave("figures/BenMAP/sensitivity_siting/pollution_V2/1km_PM25_2050_population_lowCDR_detroit.png", p, width = 16, height = 12, units = "in", dpi = 600)
    ggsave("figures/BenMAP/sensitivity_siting/pollution_V2/1km_PM25_2050_population_lowCDR_detroit.svg", p, width = 16, height = 12, units = "in", dpi = 600)
    print(p)   
    #### 14.3. Income ####
      #### 14.3.1. High CDR ####
    # 14.1.1 Get mean value 
    # Read the NetCDF file using stack
    detroit_nc_1km_stack_2050_income_highCDR <- stack("data/WRF-CMAQ/sensitivity/11_detroit/Income/Dailymean.1km.HighCCS_Income.2050.PM25_AVG.nc", varname = "PM25_AVG")
    # Calculate the mean across all bands (365 days) for each cell
    detroit_mean_raster_2050_income_highCDR <- calc(detroit_nc_1km_stack_2050_income_highCDR, fun = mean, na.rm = TRUE, na.rm.args = list(na.rm = TRUE))
    # Plot the mean raster
    plot(detroit_mean_raster_2050_income_highCDR, col = rev(magma(20)), main = "detroit 2050 high-CDR Yearly Mean PM2.5 Conc. (ug/m3). Sensitivity income")
    ...
    # 1. Check the CRS
    st_crs(grid_data_detroit)
    crs(detroit_mean_raster_2050_income_highCDR)
    #2. Print extent information
    print(extent(detroit_mean_raster_2050_income_highCDR))
    print(st_bbox(grid_data_detroit))
    #3. Check resolution
    res(detroit_mean_raster_2050_income_highCDR)
    # Get the bounding box of the grid_data
    bbox <- st_bbox(grid_data_detroit)
    # Calculate the resolution
    resolution <- c((bbox$xmax - bbox$xmin) / num_cols,
    ...
    # 2. Set the CRS for mean_raster_2050_income_highCDR
    crs(detroit_mean_raster_2050_income_highCDR) <- st_crs(grid_data_detroit)$proj4string
    
    # 3. Set the extent of mean_raster_aligned to match grid_data
    extent(detroit_mean_raster_2050_income_highCDR) <- extent(grid_data_detroit)
    ...
    # 14.1.3. Get values from raster and assign them to my grid 
    # 1. Extract values using st_extract
    system.time({detroit_values <- raster::extract(detroit_mean_raster_2050_income_highCDR, grid_data_detroit_filtered)}) #7311s
    grid_data_PM25_detroit_2050_income_highCDR <- grid_data_detroit_filtered
    
    # 2. Assign the values to the PM25_AVG column
    grid_data_PM25_detroit_2050_income_highCDR$PM25_AVG <- detroit_values
    grid_data_PM25_detroit_2050_income_highCDR$PM25_AVG <- as.numeric(gsub("[^0-9.-]", "", grid_data_PM25_detroit_2050_income_highCDR$PM25_AVG))
    
    # 14.1.4. Prepare output table
    # 1. Convert to NAD83
    grid_data_PM25_detroit_2050_income_highCDR_NAD <- st_transform(grid_data_PM25_detroit_2050_income_highCDR, crs = st_crs("+proj=longlat +datum=NAD83"))
    
    #Get right columns 
    grid_data_PM25_detroit_2050_income_highCDR_NAD %>%
      st_drop_geometry() %>%
      dplyr::select(-pixel_ID) %>%
      rename(Column = col,
             Row = row,
      ...
      mutate(Metric = "D24HourMean",
             `Seasonal Metric` = "QuarterlyMean",
             `Annual Metric` = "Mean") -> grid_data_PM25_detroit_2050_income_highCDR_NAD_filtered_final
    
    # 2. Save file
    #Csv
    write.csv(grid_data_PM25_detroit_2050_income_highCDR_NAD_filtered_final, row.names = FALSE, file = paste("output_data/BenMAP_files/sensitivity_siting/Detroit/Date_", today_date,"PM25_2050_income_highCDR_Detroit.csv"))
    
    #Shapefile
    st_write(grid_data_PM25_detroit_2050_income_highCDR_NAD, "output_data/shapefiles/sensitivity_siting/Detroit/1_km_PM25_2050_income_highCDR_detroit.shp")
    grid_data_PM25_detroit_2050_income_highCDR_NAD <- st_read("output_data/shapefiles/sensitivity_siting/Detroit/1_km_PM25_2050_income_highCDR_detroit.shp")
    ...
    grid_data_PM25_detroit_2050_income_highCDR_NAD_filtered_final %>%
      left_join(detroit_pop_total, by = c("Row", "Column")) %>%
      filter(!is.na(Values))-> detroit_pop_PM25_total
    ...
    #Based on the EPA -> https://www.epa.gov/pm-pollution/national-ambient-air-quality-standards-naaqs-pm
    grid_data_PM25_detroit_2050_income_highCDR_NAD %>%
      mutate(PM25_AVG_graph = if_else(PM25_AVG >= 9, 9, PM25_AVG)) -> grid_data_PM25_detroit_2050_income_highCDR_NAD_filtered_graph
    
    #Perform intersection to retain only pixels within the city boundary
    grid_data_PM25_detroit_2050_income_highCDR_NAD_filtered_graph_crop <- st_intersection(grid_data_PM25_detroit_2050_income_highCDR_NAD_filtered_graph, cbsa_shapefile_detroit)
    
    p <- ggplot() +
      geom_sf(data = grid_data_PM25_detroit_2050_income_highCDR_NAD_filtered_graph_crop, aes(fill = PM25_AVG_graph), color = NA, size = 0) +
      scale_fill_gradientn(colors = colors,         
                           values = scales::rescale(c(0, 5, 10)),  # Set the midpoint to 5
                           limits = c(0, 10),  # Adjust limits to match your data range
                           name = "PM2.5 (ug/m3)"       ) +
      ggtitle("2050 High-CDR Detroit Mean 1km Yearly PM2.5 (ug/m3) income sensitivity") +
      geom_text(aes(label = paste("Population weighted mean:", round(detroit_mean_BASE_2019, 2), "ug/m3")),
                x = Inf, y = -Inf, hjust = 1, vjust = -0.6, size = 5) +
      annotation_scale() + 
      geom_sf(data = cbsa_shapefile_detroit, color = "black", fill = NA)+
      theme_void() +
      geom_point(data = NULL, aes(x = -83.04362102880836,  y =  42.32974049721709), shape = 1, color = "#95C623", size = 6, stroke = 2) #Detroit city hall
    ggsave("figures/BenMAP/sensitivity_siting/pollution_V2/1km_PM25_2050_income_highCDR_detroit.png", p, width = 16, height = 12, units = "in", dpi = 600)
    ggsave("figures/BenMAP/sensitivity_siting/pollution_V2/1km_PM25_2050_income_highCDR_detroit.svg", p, width = 16, height = 12, units = "in", dpi = 600)
    print(p)   
    
      #### 14.3.2. Low CDR ####
    # 14.1.1 Get mean value 
    # Read the NetCDF file using stack
    detroit_nc_1km_stack_2050_income_lowCDR <- stack("data/WRF-CMAQ/sensitivity/11_detroit/Income/Dailymean.1km.LowCCS_Income.2050.PM25_AVG.nc", varname = "PM25_AVG")
    # Calculate the mean across all bands (365 days) for each cell
    detroit_mean_raster_2050_income_lowCDR <- calc(detroit_nc_1km_stack_2050_income_lowCDR, fun = mean, na.rm = TRUE, na.rm.args = list(na.rm = TRUE))
    # Plot the mean raster
    plot(detroit_mean_raster_2050_income_lowCDR, col = rev(magma(20)), main = "detroit 2050 low-CDR Yearly Mean PM2.5 Conc. (ug/m3). Sensitivity income")
    ...
    # 1. Check the CRS
    st_crs(grid_data_detroit)
    crs(detroit_mean_raster_2050_income_lowCDR)
    #2. Print extent information
    print(extent(detroit_mean_raster_2050_income_lowCDR))
    print(st_bbox(grid_data_detroit))
    #3. Check resolution
    res(detroit_mean_raster_2050_income_lowCDR)
    # Get the bounding box of the grid_data
    bbox <- st_bbox(grid_data_detroit)
    # Calculate the resolution
    resolution <- c((bbox$xmax - bbox$xmin) / num_cols,
    ...
    # 2. Set the CRS for mean_raster_2050_income_lowCDR
    crs(detroit_mean_raster_2050_income_lowCDR) <- st_crs(grid_data_detroit)$proj4string
    
    # 3. Set the extent of mean_raster_aligned to match grid_data
    extent(detroit_mean_raster_2050_income_lowCDR) <- extent(grid_data_detroit)
    ...
    # 14.1.3. Get values from raster and assign them to my grid 
    # 1. Extract values using st_extract
    system.time({detroit_values <- raster::extract(detroit_mean_raster_2050_income_lowCDR, grid_data_detroit_filtered)}) #311s
    grid_data_PM25_detroit_2050_income_lowCDR <- grid_data_detroit_filtered
    
    # 2. Assign the values to the PM25_AVG column
    grid_data_PM25_detroit_2050_income_lowCDR$PM25_AVG <- detroit_values
    grid_data_PM25_detroit_2050_income_lowCDR$PM25_AVG <- as.numeric(gsub("[^0-9.-]", "", grid_data_PM25_detroit_2050_income_lowCDR$PM25_AVG))
    
    # 14.1.4. Prepare output table
    # 1. Convert to NAD83
    grid_data_PM25_detroit_2050_income_lowCDR_NAD <- st_transform(grid_data_PM25_detroit_2050_income_lowCDR, crs = st_crs("+proj=longlat +datum=NAD83"))
    
    #Get right columns 
    grid_data_PM25_detroit_2050_income_lowCDR_NAD %>%
      st_drop_geometry() %>%
      dplyr::select(-pixel_ID) %>%
      rename(Column = col,
             Row = row,
      ...
      mutate(Metric = "D24HourMean",
             `Seasonal Metric` = "QuarterlyMean",
             `Annual Metric` = "Mean") -> grid_data_PM25_detroit_2050_income_lowCDR_NAD_filtered_final
    
    # 2. Save file
    #Csv
    write.csv(grid_data_PM25_detroit_2050_income_lowCDR_NAD_filtered_final, row.names = FALSE, file = paste("output_data/BenMAP_files/sensitivity_siting/Detroit/Date_", today_date,"PM25_2050_income_lowCDR_Detroit.csv"))
    
    #Shapefile
    st_write(grid_data_PM25_detroit_2050_income_lowCDR_NAD, "output_data/shapefiles/sensitivity_siting/Detroit/1_km_PM25_2050_income_lowCDR_detroit.shp")
    grid_data_PM25_detroit_2050_income_lowCDR_NAD <- st_read("output_data/shapefiles/sensitivity_siting/Detroit/1_km_PM25_2050_income_lowCDR_detroit.shp")
    ...
    grid_data_PM25_detroit_2050_income_lowCDR_NAD_filtered_final %>%
      left_join(detroit_pop_total, by = c("Row", "Column")) %>%
      filter(!is.na(Values))-> detroit_pop_PM25_total
    ...
    #Based on the EPA -> https://www.epa.gov/pm-pollution/national-ambient-air-quality-standards-naaqs-pm
    grid_data_PM25_detroit_2050_income_lowCDR_NAD %>%
      mutate(PM25_AVG_graph = if_else(PM25_AVG >= 9, 9, PM25_AVG)) -> grid_data_PM25_detroit_2050_income_lowCDR_NAD_filtered_graph
    
    #Perform intersection to retain only pixels within the city boundary
    grid_data_PM25_detroit_2050_income_lowCDR_NAD_filtered_graph_crop <- st_intersection(grid_data_PM25_detroit_2050_income_lowCDR_NAD_filtered_graph, cbsa_shapefile_detroit)
    
    p <- ggplot() +
      geom_sf(data = grid_data_PM25_detroit_2050_income_lowCDR_NAD_filtered_graph_crop, aes(fill = PM25_AVG_graph), color = NA, size = 0) +
      scale_fill_gradientn(colors = colors,         
                           values = scales::rescale(c(0, 5, 10)),  # Set the midpoint to 5
                           limits = c(0, 10),  # Adjust limits to match your data range
                           name = "PM2.5 (ug/m3)"       ) +
      ggtitle("2050 low-CDR Detroit Mean 1km Yearly PM2.5 (ug/m3) income sensitivity") +
      geom_text(aes(label = paste("Population weighted mean:", round(detroit_mean_BASE_2019, 2), "ug/m3")),
                x = Inf, y = -Inf, hjust = 1, vjust = -0.6, size = 5) +
      annotation_scale() + 
      geom_sf(data = cbsa_shapefile_detroit, color = "black", fill = NA)+
      theme_void() +
      geom_point(data = NULL, aes(x = -83.04362102880836,  y =  42.32974049721709), shape = 1, color = "#95C623", size = 6, stroke = 2) #Detroit city hall
    ggsave("figures/BenMAP/sensitivity_siting/pollution_V2/1km_PM25_2050_income_lowCDR_detroit.png", p, width = 16, height = 12, units = "in", dpi = 600)
    ggsave("figures/BenMAP/sensitivity_siting/pollution_V2/1km_PM25_2050_income_lowCDR_detroit.svg", p, width = 16, height = 12, units = "in", dpi = 600)
    print(p)   
    
    #### 14. Plots ####
      #### all in one ####
    all_scenarios_detroit <- bind_rows(
      grid_data_PM25_detroit_NAD_2050_high %>% mutate(Scenario = "Base High CDR"),
      grid_data_PM25_detroit_NAD_2050_low  %>% mutate(Scenario = "Base Low CDR"),
      grid_data_PM25_detroit_2050_population_highCDR_NAD %>% mutate(Scenario = "Population High CDR"),
      grid_data_PM25_detroit_2050_minority_highCDR_NAD   %>% mutate(Scenario = "Minority High CDR"),
      grid_data_PM25_detroit_2050_income_highCDR_NAD     %>% mutate(Scenario = "Income High CDR"),
      grid_data_PM25_detroit_2050_population_lowCDR_NAD  %>% mutate(Scenario = "Population Low CDR"),
      grid_data_PM25_detroit_2050_minority_lowCDR_NAD    %>% mutate(Scenario = "Minority Low CDR"),
      grid_data_PM25_detroit_2050_income_lowCDR_NAD      %>% mutate(Scenario = "Income Low CDR")
    )
    
    # 2. Control facet order
    scenario_order <- c(
      "Base High CDR", "Population High CDR", "Minority High CDR", "Income High CDR",
      "Base Low CDR",  "Population Low CDR",  "Minority Low CDR",  "Income Low CDR"
    )
    
    all_scenarios_detroit <- all_scenarios_detroit %>%
      mutate(Scenario = factor(Scenario, levels = scenario_order))
      
    all_scenarios_crop_detroit <-
      st_intersection(all_scenarios_detroit, cbsa_shapefile_detroit)
    
    p_facets_all_detroit <- ggplot(all_scenarios_crop_detroit) +
      geom_sf(aes(fill = PM25_AVG), color = NA, size = 0) +
      geom_sf(data = cbsa_shapefile_detroit, color = "black", fill = NA) +
      scale_fill_gradientn(
        colors = colors,
      ...
      facet_wrap(~Scenario, ncol = 4) +   # 2 rows × 4 columns
      theme_void() +
      ggtitle("2050 Detroit 1km PM2.5 Concentrations (Base + Sensitivity Scenarios)") +
      geom_point(
        aes(x = -83.0458, y = 42.3314),    # Detroit City Hall
        shape = 1, color = "#95C623", size = 6, stroke = 2
      )
    ggsave(
      "figures/BenMAP/sensitivity_siting/pollution_V2/1km_PM25_2050_Detroit_all_scenarios.png",
      p_facets_all_detroit,
      width = 20, height = 12, units = "in", dpi = 600
    )
    
    ggsave(
      "figures/BenMAP/sensitivity_siting/pollution_V2/1km_PM25_2050_Detroit_all_scenarios.svg",
      p_facets_all_detroit,
      width = 20, height = 12, units = "in", dpi = 600
    )
      ...
      sf_list_detroit <- list(
        minority_highCDR     = grid_data_PM25_detroit_2050_minority_highCDR_NAD,
        minority_lowCDR      = grid_data_PM25_detroit_2050_minority_lowCDR_NAD,
        population_highCDR   = grid_data_PM25_detroit_2050_population_highCDR_NAD,
        population_lowCDR    = grid_data_PM25_detroit_2050_population_lowCDR_NAD,
        income_highCDR       = grid_data_PM25_detroit_2050_income_highCDR_NAD,
        income_lowCDR        = grid_data_PM25_detroit_2050_income_lowCDR_NAD
      )
      
      # Corresponding base runs
      base_list_detroit <- list(
        minority_highCDR     = grid_data_PM25_detroit_NAD_2050_high,
        minority_lowCDR      = grid_data_PM25_detroit_NAD_2050_low,
        population_highCDR   = grid_data_PM25_detroit_NAD_2050_high,
        population_lowCDR    = grid_data_PM25_detroit_NAD_2050_low,
        income_highCDR       = grid_data_PM25_detroit_NAD_2050_high,
        income_lowCDR        = grid_data_PM25_detroit_NAD_2050_low
      )
      
      # Scenario labels (facet text)
      scenario_labels_detroit <- c(
        "Minority High CDR", "Minority Low CDR",
        "Population High CDR", "Population Low CDR",
        "Income High CDR", "Income Low CDR"
      )
      ...
      compute_difference <- function(sensitivity_sf, base_sf) {
        ...
        base_clean <- base_sf %>% filter(!is.na(PM25_AVG))
        sens_clean <- sensitivity_sf %>% filter(!is.na(PM25_AVG))
        ...
        diff_sf <- base_clean %>%
          ...
          left_join(
            sens_clean %>%
              st_drop_geometry() %>%
              select(pixel_ID, row, col, PM25_sens = PM25_AVG),
            by = c("pixel_ID", "row", "col")
          ) %>%
          mutate(PM25_diff = PM25_sens - PM25_base) %>%
          filter(!is.na(PM25_diff))
      ...
      diff_combined_detroit <- bind_rows(
        lapply(seq_along(diff_list_detroit), function(i) {
          diff_list_detroit[[i]] %>%
            mutate(Scenario = scenario_labels_detroit[i])
        })
      )
      
      diff_combined_crop_detroit <-
        st_intersection(diff_combined_detroit, cbsa_shapefile_detroit)
      ...
      diff_combined_crop_detroit <- diff_combined_crop_detroit %>%
        mutate(Scenario = factor(Scenario, levels = scenario_order_diff))
      
      p_facets_diff_detroit <- ggplot(diff_combined_crop_detroit) +
        geom_sf(aes(fill = PM25_diff), color = NA, size = 0) +
        geom_sf(data = cbsa_shapefile_detroit, color = "black", fill = NA) +
        scale_fill_gradientn(
          colors = colors,
        ...
        facet_wrap(~Scenario, ncol = 3) +
        theme_void() +
        ggtitle("2050 Detroit 1km PM2.5 Difference (Sensitivity − Base)")
      ggsave(
        "figures/BenMAP/sensitivity_siting/pollution_V2/1km_PM25_2050_Detroit_DIFF_FACET_GRID.png",
        p_facets_diff_detroit,
        width = 24, height = 16, units = "in", dpi = 600
      )
      
      ggsave(
        "figures/BenMAP/sensitivity_siting/pollution_V2/1km_PM25_2050_Detroit_DIFF_FACET_GRID.svg",
        p_facets_diff_detroit,
        width = 24, height = 16, units = "in", dpi = 600
      )
      
#------------------------------------------------- SITING SENSITIVITY: 2. MORTALITY BENMAP RESULTS (1-KM) ---------------------------------------------------------------------------
##### 2. LOS ANGELES #####
    #### 2.1 Mortality figures ####
    #Load original BenMAP results
    benmap_base_HIGH <- st_read("BenMAP_results/1km_V2/LA/health_impacts_2050_1km_highvszero/Health Impacts-Pope 2019_20240520.shp")
    benmap_base_LOW <- st_read("BenMAP_results/1km_V2/LA/health_impacts_2050_1km_lowvszero/Health Impacts-Pope 2019_20240520.shp")
    
    #Load sensitivity BenMAP results (6 total)
    benmap_highVSzero_minority <- st_read("BenMAP_results/1km_V2/LA/siting_sensitivity/highVSzero_mortality_siting_sens_minority/Health Impacts-Pope 2019_20251231.shp")
    benmap_lowVSzero_minority <- st_read("BenMAP_results/1km_V2/LA/siting_sensitivity/lowVSzero_mortality_siting_sens_minority/Health Impacts-Pope 2019_20251231.shp")
    benmap_highVSzero_population <- st_read("BenMAP_results/1km_V2/LA/siting_sensitivity/highVSzero_mortality_siting_sens_population/Health Impacts-Pope 2019_20251231.shp")
    benmap_lowVSzero_population <- st_read("BenMAP_results/1km_V2/LA/siting_sensitivity/lowVSzero_mortality_siting_sens_population/Health Impacts-Pope 2019_20251231.shp")
    benmap_highVSzero_income<- st_read("BenMAP_results/1km_V2/LA/siting_sensitivity/highVSzero_mortality_siting_sens_income/Health Impacts-Pope 2019_20251231.shp")
    benmap_lowVSzero_income<- st_read("BenMAP_results/1km_V2/LA/siting_sensitivity/lowVSzero_mortality_siting_sens_income/Health Impacts-Pope 2019_20251231.shp")
    
    # Function to process BenMAP shapefiles consistently
    process_benmap <- function(benmap_sf, scenario_name) {
      # Set NA population/Point.Estim to 0
      benmap_sf <- benmap_sf %>%
        mutate(
          Point.Estim = if_else(is.na(Point.Estim), 0, Point.Estim),
          Population = if_else(is.na(Population), 0, Population),
          deaths_per_1000 = if_else(Population == 0, 0, (Point.Estim / Population) * 1000),
          deaths_per_1000_graph = if_else(deaths_per_1000 >= 1, 1, deaths_per_1000)
        )
      benmap_sf <- st_transform(benmap_sf, crs = st_crs(cbsa_shapefile_LA)) # Transform CRS to match LA boundary
      benmap_sf <- st_intersection(benmap_sf, cbsa_shapefile_LA)  # Clip to LA boundary
      benmap_sf$Scenario <- scenario_name  # Add scenario sensitivity label
      return(benmap_sf)
    }
    
    #Process base
    benmap_base_high_proc <- process_benmap(benmap_base_HIGH, "Base High CDR")
    benmap_base_low_proc <- process_benmap(benmap_base_LOW, "Base Low CDR")
    benmap_highVSzero_minority_proc <- process_benmap(benmap_highVSzero_minority, "Minority High CDR")
    benmap_lowVSzero_minority_proc<- process_benmap(benmap_lowVSzero_minority, "Minority Low CDR")
    benmap_highVSzero_population_proc <-  process_benmap(benmap_highVSzero_population, "Population High CDR")
    benmap_lowVSzero_population_proc <- process_benmap(benmap_lowVSzero_population, "Population Low CDR")
    benmap_highVSzero_income_proc <- process_benmap(benmap_highVSzero_income, "Income High CDR")
    benmap_lowVSzero_income_proc <- process_benmap(benmap_lowVSzero_income, "Income Low CDR")
    
    # Combine BASE + SSPs
    benmap_all_LA <- bind_rows(list(benmap_base_high_proc, benmap_base_low_proc,
                                    benmap_highVSzero_minority_proc, benmap_lowVSzero_minority_proc, 
                                    benmap_highVSzero_population_proc, benmap_lowVSzero_population_proc, 
                                    benmap_highVSzero_income_proc, benmap_lowVSzero_income_proc))
    
    scenario_order <- c("Base High CDR", "Population High CDR", "Minority High CDR", "Income High CDR",
                        "Base Low CDR", "Population Low CDR", "Minority Low CDR", "Income Low CDR")
    
    benmap_all_LA %>%
      mutate(Scenario = factor(Scenario, levels = scenario_order)) -> benmap_all_LA
    
    #  Create faceted plot
    p_facets <- ggplot() +
      geom_sf(data = benmap_all_LA, 
              aes(fill = Point.Estim), color = NA, size = 0) +
      geom_sf(data = cbsa_shapefile_LA, color = "black", fill = NA) +
      scale_fill_viridis_c(option = "rocket", direction = -1) +#limits = c(0, 1)
      #facet_grid(CDR~Sensitivity) +
      ...
      annotation_scale() +
      theme_void() +
      labs(title = "2050 LA Mortality due to PM2.5  (siting sensitivity)",
           fill = "Deaths per pixel") 
    ggsave("figures/BenMAP/sensitivity_siting/mortality/LA/1km_mortality_2050_Benmap_LA_total_SENS.png", p_facets, width = 16, height = 12, units = "in", dpi = 600)
    ggsave("figures/BenMAP/sensitivity_siting/mortality/LA/1km_mortality_2050_Benmap_LA_total_SENS.svg", p_facets, width = 16, height = 12, units = "in", dpi = 600)
    
    #Difference
    benmap_highVSzero_minority_proc %>% 
      bind_rows(benmap_highVSzero_population_proc, benmap_highVSzero_income_proc) %>%
      select(ROW, COL, Point.Estim, Scenario) %>%
      rename(mortality_sensitivity = Point.Estim) %>%
      #st_drop_geometry() %>%
      left_join(benmap_base_high_proc %>%
                  select(ROW, COL, Point.Estim, Scenario) %>%
                  rename(mortality_base = Point.Estim) %>%
                  st_drop_geometry(), by = c("ROW", "COL")) %>%
      #Get difference of sensitivity minus base
      #If positive, it means mortality increased in sensitivity
      #If negative, it means mortality decreased in sensitivity
      mutate(difference = mortality_sensitivity - mortality_base)-> benmap_LA_highCDR_difference
    
    benmap_lowVSzero_minority_proc %>% 
      bind_rows(benmap_lowVSzero_population_proc, benmap_lowVSzero_income_proc) %>%
      select(ROW, COL, Point.Estim, Scenario) %>%
      rename(mortality_sensitivity = Point.Estim) %>%
            left_join(benmap_base_low_proc %>%
                  select(ROW, COL, Point.Estim, Scenario) %>%
                    rename(mortality_base = Point.Estim) %>%
                  st_drop_geometry(), by = c("ROW", "COL")) %>%
      #Get difference of sensitivity minus base
        #If positive, it means mortality increased in sensitivity
        #If negative, it means mortality decreased in sensitivity
      mutate(difference = mortality_sensitivity - mortality_base)-> benmap_LA_lowCDR_difference
    
    benmap_all_LA_difference <- bind_rows(benmap_LA_highCDR_difference, benmap_LA_lowCDR_difference)
    
    #plot
    p_facets <- ggplot() +
      geom_sf(data = benmap_all_LA_difference %>% mutate(Scenario.x = factor(Scenario.x, levels = scenario_order)),
              aes(fill = difference), color = NA, size = 0) +
      geom_sf(data = cbsa_shapefile_LA, color = "black", fill = NA) +
      #scale_fill_viridis_c(option = "rocket", direction = -1) +#limits = c(0, 1)
      scale_fill_gradient2(
        ...
        high = "red",
        midpoint = 0,
        name = "Deaths per pixel"
      ) +
        facet_wrap(~Scenario.x, ncol = 3) +
      annotation_scale() +
      theme_void() +
      labs(title = "2050 LA Mortality due to PM2.5 (siting sensitivity difference vs base)", 
           subtitle = "If positive, mortality increased in sensitivity.
           If negative, mortality decreased in sensitivity",
           fill = "Deaths per pixel") 
    ggsave("figures/BenMAP/sensitivity_siting/mortality/LA/1km_mortality_2050_Benmap_LA_total_SENS_diff.png", p_facets, width = 16, height = 12, units = "in", dpi = 600)
    ggsave("figures/BenMAP/sensitivity_siting/mortality/LA/1km_mortality_2050_Benmap_LA_total_SENS_diff.svg", p_facets, width = 16, height = 12, units = "in", dpi = 600)
    

    #### 2.2 Mortality totals ####
    # Get 2019 population
    partitions_LA <- read.csv("output_data/Partitions/partitions_LA.csv")[, -1] 
    
    partitions_LA_complete <- partitions_LA %>%
      dplyr::select(-state_ID, -block_area) %>%
      left_join(census_block_variables, by = "census_block_ID")
    
    population_LA_2019 <- partitions_LA_complete %>%
      mutate(Population = population_in * area_ratio_block) %>%
      group_by(pixel_ID, row, col) %>%
      summarise(pop_2019 = sum(Population)) %>%
      ungroup()
    
    # Summarize total mortality per scenario
    benmap_all_summary <- benmap_all_LA %>%
      st_drop_geometry() %>%
      group_by(Scenario) %>%
      summarise(total_deaths = sum(Point.Estim, na.rm = TRUE), .groups = "drop")  %>%
      mutate(deaths_per_million = (total_deaths / sum(population_LA_2019$pop_2019)) * 1e6
      )

##### 11. PHILADELPHIA ####
    #### 11.1 Mortality figures ####
    #Load original BenMAP results
    benmap_base_HIGH_phil <- st_read("BenMAP_results/1km_V2/Philadelphia/health_impacts_2050_1km_highvszero/Health Impacts-Pope 2019_20240520.shp")
    benmap_base_LOW_phil <- st_read("BenMAP_results/1km_V2/Philadelphia/health_impacts_2050_1km_lowvszero/Health Impacts-Pope 2019_20240520.shp")
    
    #Load sensitivity BenMAP results (6 total)
    benmap_highVSzero_minority_phil <- st_read("BenMAP_results/1km_V2/Philadelphia/siting_sensitivity/highVSzero_mortality_siting_sens_minority/Health Impacts-Pope 2019_20251231.shp")
    benmap_lowVSzero_minority_phil <- st_read("BenMAP_results/1km_V2/Philadelphia/siting_sensitivity/lowVSzero_mortality_siting_sens_minority/Health Impacts-Pope 2019_20251231.shp")
    benmap_highVSzero_population_phil <- st_read("BenMAP_results/1km_V2/Philadelphia/siting_sensitivity/highVSzero_mortality_siting_sens_population/Health Impacts-Pope 2019_20251231.shp")
    benmap_lowVSzero_population_phil <- st_read("BenMAP_results/1km_V2/Philadelphia/siting_sensitivity/lowVSzero_mortality_siting_sens_population/Health Impacts-Pope 2019_20251231.shp")
    benmap_highVSzero_income_phil <- st_read("BenMAP_results/1km_V2/Philadelphia/siting_sensitivity/highVSzero_mortality_siting_sens_income/Health Impacts-Pope 2019_20251231.shp")
    benmap_lowVSzero_income_phil <- st_read("BenMAP_results/1km_V2/Philadelphia/siting_sensitivity/lowVSzero_mortality_siting_sens_income/Health Impacts-Pope 2019_20251231.shp")
    ...
    process_benmap <- function(benmap_sf, scenario_name) {
      ...
      benmap_sf <- st_transform(benmap_sf, crs = st_crs(cbsa_shapefile_phil)) # Transform CRS to match phil boundary
      benmap_sf <- st_intersection(benmap_sf, cbsa_shapefile_phil)  # Clip to phil boundary
      benmap_sf$Scenario <- scenario_name  # Add scenario sensitivity label
      return(benmap_sf)
    }
    
    #Process base
    benmap_base_high_proc_phil <- process_benmap(benmap_base_HIGH_phil, "Base High CDR")
    benmap_base_low_proc_phil <- process_benmap(benmap_base_LOW_phil, "Base Low CDR")
    benmap_highVSzero_minority_proc_phil <- process_benmap(benmap_highVSzero_minority_phil, "Minority High CDR")
    benmap_lowVSzero_minority_proc_phil<- process_benmap(benmap_lowVSzero_minority_phil, "Minority Low CDR")
    benmap_highVSzero_population_proc_phil <-  process_benmap(benmap_highVSzero_population_phil, "Population High CDR")
    benmap_lowVSzero_population_proc_phil <- process_benmap(benmap_lowVSzero_population_phil, "Population Low CDR")
    benmap_highVSzero_income_proc_phil <- process_benmap(benmap_highVSzero_income_phil, "Income High CDR")
    benmap_lowVSzero_income_proc_phil <- process_benmap(benmap_lowVSzero_income_phil, "Income Low CDR")
    
    # Combine BASE + SSPs
    benmap_all_phil <- bind_rows(list(benmap_base_high_proc_phil, benmap_base_low_proc_phil,
                                    benmap_highVSzero_minority_proc_phil, benmap_lowVSzero_minority_proc_phil, 
                                    benmap_highVSzero_population_proc_phil, benmap_lowVSzero_population_proc_phil, 
                                    benmap_highVSzero_income_proc_phil, benmap_lowVSzero_income_proc_phil))
    
    scenario_order <- c("Base High CDR", "Population High CDR", "Minority High CDR", "Income High CDR",
                        "Base Low CDR", "Population Low CDR", "Minority Low CDR", "Income Low CDR")
    
    benmap_all_phil %>%
      mutate(Scenario = factor(Scenario, levels = scenario_order)) -> benmap_all_phil
    
    #  Create faceted plot
    p_facets <- ggplot() +
      geom_sf(data = benmap_all_phil, 
              aes(fill = Point.Estim), color = NA, size = 0) +
      geom_sf(data = cbsa_shapefile_phil, color = "black", fill = NA) +
      scale_fill_viridis_c(option = "rocket", direction = -1) +#limits = c(0, 1)
      #facet_grid(CDR~Sensitivity) +
      ...
      annotation_scale() +
      theme_void() +
      labs(title = "2050 Philadelphia Mortality due to PM2.5  (siting sensitivity)",
           fill = "Deaths per pixel") 
    ggsave("figures/BenMAP/sensitivity_siting/mortality/Philadelphia/1km_mortality_2050_Benmap_phil_total_SENS.png", p_facets, width = 16, height = 12, units = "in", dpi = 600)
    ggsave("figures/BenMAP/sensitivity_siting/mortality/Philadelphia/1km_mortality_2050_Benmap_phil_total_SENS.svg", p_facets, width = 16, height = 12, units = "in", dpi = 600)
    
    #Difference
    benmap_highVSzero_minority_proc_phil %>% 
      bind_rows(benmap_highVSzero_population_proc_phil, benmap_highVSzero_income_proc_phil) %>%
      select(ROW, COL, Point.Estim, Scenario) %>%
      rename(mortality_sensitivity = Point.Estim) %>%
      #st_drop_geometry() %>%
      left_join(benmap_base_high_proc_phil %>%
                  select(ROW, COL, Point.Estim, Scenario) %>%
                  rename(mortality_base = Point.Estim) %>%
                  st_drop_geometry(), by = c("ROW", "COL")) %>%
      #Get difference of sensitivity minus base
      #If positive, it means mortality increased in sensitivity
      #If negative, it means mortality decreased in sensitivity
      mutate(difference = mortality_sensitivity - mortality_base)-> benmap_phil_highCDR_difference
    
    benmap_lowVSzero_minority_proc_phil %>% 
      bind_rows(benmap_lowVSzero_population_proc_phil, benmap_lowVSzero_income_proc_phil) %>%
      select(ROW, COL, Point.Estim, Scenario) %>%
      rename(mortality_sensitivity = Point.Estim) %>%
      left_join(benmap_base_low_proc_phil %>%
                  select(ROW, COL, Point.Estim, Scenario) %>%
                  rename(mortality_base = Point.Estim) %>%
                  st_drop_geometry(), by = c("ROW", "COL")) %>%
      #Get difference of sensitivity minus base
      #If positive, it means mortality increased in sensitivity
      #If negative, it means mortality decreased in sensitivity
      mutate(difference = mortality_sensitivity - mortality_base)-> benmap_phil_lowCDR_difference
    
    benmap_all_phil_difference <- bind_rows(benmap_phil_highCDR_difference, benmap_phil_lowCDR_difference)
    
    #plot
    p_facets <- ggplot() +
      geom_sf(data = benmap_all_phil_difference %>% mutate(Scenario.x = factor(Scenario.x, levels = scenario_order)),
              aes(fill = difference), color = NA, size = 0) +
      geom_sf(data = cbsa_shapefile_phil, color = "black", fill = NA) +
      #scale_fill_viridis_c(option = "rocket", direction = -1) +#limits = c(0, 1)
      scale_fill_gradient2(
      ...
      annotation_scale() +
      theme_void() +
      labs(title = "2050 Philadelphia Mortality due to PM2.5 (siting sensitivity difference vs base)", 
           subtitle = "If positive, mortality increased in sensitivity.
           If negative, mortality decreased in sensitivity",
           fill = "Deaths per pixel") 
    ggsave("figures/BenMAP/sensitivity_siting/mortality/Philadelphia/1km_mortality_2050_Benmap_phil_total_SENS_diff.png", p_facets, width = 16, height = 12, units = "in", dpi = 600)
    ggsave("figures/BenMAP/sensitivity_siting/mortality/Philadelphia/1km_mortality_2050_Benmap_phil_total_SENS_diff.svg", p_facets, width = 16, height = 12, units = "in", dpi = 600)
    
    
    #### 11.2 Mortality totals ####
    # Get 2019 population
    partitions_philadelphia <- read.csv("output_data/Partitions/partitions_phil.csv")[, -1] 
    
    partitions_phil_complete <- partitions_philadelphia %>%
      dplyr::select(-state_ID, -block_area) %>%
      left_join(census_block_variables, by = "census_block_ID")
    
    population_phil_2019 <- partitions_phil_complete %>%
      mutate(Population = population_in * area_ratio_block) %>%
      group_by(pixel_ID, row, col) %>%
      summarise(pop_2019 = sum(Population)) %>%
      ungroup()
    ...
    # Summarize total mortality per scenario
    benmap_all_summary_phil <- benmap_all_phil %>%
      st_drop_geometry() %>%
      group_by(Scenario) %>%
      summarise(total_deaths = sum(Point.Estim, na.rm = TRUE), .groups = "drop")  %>%
      mutate(deaths_per_million = (total_deaths / sum(population_phil_2019$pop_2019)) * 1e6
      )
    
##### 14. DETROIT #####
    #### 14.1 Mortality figures ####
    #Load original BenMAP results
    benmap_base_HIGH_detroit <- st_read("BenMAP_results/1km_V2/Detroit/health_impacts_2050_1km_highvszero/Health Impacts-Pope 2019_20240517.shp")
    benmap_base_LOW_detroit <- st_read("BenMAP_results/1km_V2/Detroit/health_impacts_2050_1km_lowvszero/Health Impacts-Pope 2019_20240517.shp")
    
    #Load sensitivity BenMAP results (6 total)
    benmap_highVSzero_minority_detroit <- st_read("BenMAP_results/1km_V2/Detroit/siting_sensitivity/highVSzero_mortality_siting_sens_minority/Health Impacts-Pope 2019_20251231.shp")
    benmap_lowVSzero_minority_detroit <- st_read("BenMAP_results/1km_V2/Detroit/siting_sensitivity/lowVSzero_mortality_siting_sens_minority/Health Impacts-Pope 2019_20251231.shp")
    benmap_highVSzero_population_detroit <- st_read("BenMAP_results/1km_V2/Detroit/siting_sensitivity/highVSzero_mortality_siting_sens_population/Health Impacts-Pope 2019_20251231.shp")
    benmap_lowVSzero_population_detroit <- st_read("BenMAP_results/1km_V2/Detroit/siting_sensitivity/lowVSzero_mortality_siting_sens_population/Health Impacts-Pope 2019_20251231.shp")
    benmap_highVSzero_income_detroit <- st_read("BenMAP_results/1km_V2/Detroit/siting_sensitivity/highVSzero_mortality_siting_sens_income/Health Impacts-Pope 2019_20251231.shp")
    benmap_lowVSzero_income_detroit <- st_read("BenMAP_results/1km_V2/Detroit/siting_sensitivity/lowVSzero_mortality_siting_sens_income/Health Impacts-Pope 2019_20251231.shp")
    ...
    process_benmap <- function(benmap_sf, scenario_name) {
      ...
      benmap_sf <- st_transform(benmap_sf, crs = st_crs(cbsa_shapefile_detroit)) # Transform CRS to match detroit boundary
      benmap_sf <- st_intersection(benmap_sf, cbsa_shapefile_detroit)  # Clip to detroit boundary
      benmap_sf$Scenario <- scenario_name  # Add scenario sensitivity label
      return(benmap_sf)
    }
    
    #Process base
    benmap_base_high_proc_detroit <- process_benmap(benmap_base_HIGH_detroit, "Base High CDR")
    benmap_base_low_proc_detroit <- process_benmap(benmap_base_LOW_detroit, "Base Low CDR")
    benmap_highVSzero_minority_proc_detroit <- process_benmap(benmap_highVSzero_minority_detroit, "Minority High CDR")
    benmap_lowVSzero_minority_proc_detroit<- process_benmap(benmap_lowVSzero_minority_detroit, "Minority Low CDR")
    benmap_highVSzero_population_proc_detroit <-  process_benmap(benmap_highVSzero_population_detroit, "Population High CDR")
    benmap_lowVSzero_population_proc_detroit <- process_benmap(benmap_lowVSzero_population_detroit, "Population Low CDR")
    benmap_highVSzero_income_proc_detroit <- process_benmap(benmap_highVSzero_income_detroit, "Income High CDR")
    benmap_lowVSzero_income_proc_detroit <- process_benmap(benmap_lowVSzero_income_detroit, "Income Low CDR")
    
    # Combine BASE + SSPs
    benmap_all_detroit <- bind_rows(list(benmap_base_high_proc_detroit, benmap_base_low_proc_detroit,
                                      benmap_highVSzero_minority_proc_detroit, benmap_lowVSzero_minority_proc_detroit, 
                                      benmap_highVSzero_population_proc_detroit, benmap_lowVSzero_population_proc_detroit, 
                                      benmap_highVSzero_income_proc_detroit, benmap_lowVSzero_income_proc_detroit))
    
    scenario_order <- c("Base High CDR", "Population High CDR", "Minority High CDR", "Income High CDR",
                        "Base Low CDR", "Population Low CDR", "Minority Low CDR", "Income Low CDR")
    
    benmap_all_detroit %>%
      mutate(Scenario = factor(Scenario, levels = scenario_order)) -> benmap_all_detroit
    
    #  Create faceted plot
    p_facets <- ggplot() +
      geom_sf(data = benmap_all_detroit, 
              aes(fill = Point.Estim), color = NA, size = 0) +
      geom_sf(data = cbsa_shapefile_detroit, color = "black", fill = NA) +
      scale_fill_viridis_c(option = "rocket", direction = -1) +#limits = c(0, 1)
      #facet_grid(CDR~Sensitivity) +
      ...
      annotation_scale() +
      theme_void() +
      labs(title = "2050 Detroit Mortality due to PM2.5  (siting sensitivity)",
           fill = "Deaths per pixel") 
    ggsave("figures/BenMAP/sensitivity_siting/mortality/Detroit/1km_mortality_2050_Benmap_detroit_total_SENS.png", p_facets, width = 16, height = 12, units = "in", dpi = 600)
    ggsave("figures/BenMAP/sensitivity_siting/mortality/Detroit/1km_mortality_2050_Benmap_detroit_total_SENS.svg", p_facets, width = 16, height = 12, units = "in", dpi = 600)
    
    #Difference
    benmap_highVSzero_minority_proc_detroit %>% 
      bind_rows(benmap_highVSzero_population_proc_detroit, benmap_highVSzero_income_proc_detroit) %>%
      select(ROW, COL, Point.Estim, Scenario) %>%
      rename(mortality_sensitivity = Point.Estim) %>%
      #st_drop_geometry() %>%
      left_join(benmap_base_high_proc_detroit %>%
                  select(ROW, COL, Point.Estim, Scenario) %>%
                  rename(mortality_base = Point.Estim) %>%
                  st_drop_geometry(), by = c("ROW", "COL")) %>%
      #Get difference of sensitivity minus base
      #If positive, it means mortality increased in sensitivity
      #If negative, it means mortality decreased in sensitivity
      mutate(difference = mortality_sensitivity - mortality_base)-> benmap_detroit_highCDR_difference
    
    benmap_lowVSzero_minority_proc_detroit %>% 
      bind_rows(benmap_lowVSzero_population_proc_detroit, benmap_lowVSzero_income_proc_detroit) %>%
      select(ROW, COL, Point.Estim, Scenario) %>%
      rename(mortality_sensitivity = Point.Estim) %>%
      left_join(benmap_base_low_proc_detroit %>%
                  select(ROW, COL, Point.Estim, Scenario) %>%
                  rename(mortality_base = Point.Estim) %>%
                  st_drop_geometry(), by = c("ROW", "COL")) %>%
      #Get difference of sensitivity minus base
      #If positive, it means mortality increased in sensitivity
      #If negative, it means mortality decreased in sensitivity
      mutate(difference = mortality_sensitivity - mortality_base)-> benmap_detroit_lowCDR_difference
    
    benmap_all_detroit_difference <- bind_rows(benmap_detroit_highCDR_difference, benmap_detroit_lowCDR_difference)
    
    #plot
    p_facets <- ggplot() +
      geom_sf(data = benmap_all_detroit_difference %>% mutate(Scenario.x = factor(Scenario.x, levels = scenario_order)),
              aes(fill = difference), color = NA, size = 0) +
      geom_sf(data = cbsa_shapefile_detroit, color = "black", fill = NA) +
      #scale_fill_viridis_c(option = "rocket", direction = -1) +#limits = c(0, 1)
      scale_fill_gradient2(
      ...
      annotation_scale() +
      theme_void() +
      labs(title = "2050 Detroit Mortality due to PM2.5 (siting sensitivity difference vs base)", 
           subtitle = "If positive, mortality increased in sensitivity.
           If negative, mortality decreased in sensitivity",
           fill = "Deaths per pixel") 
    ggsave("figures/BenMAP/sensitivity_siting/mortality/Detroit/1km_mortality_2050_Benmap_detroit_total_SENS_diff.png", p_facets, width = 16, height = 12, units = "in", dpi = 600)
    ggsave("figures/BenMAP/sensitivity_siting/mortality/Detroit/1km_mortality_2050_Benmap_detroit_total_SENS_diff.svg", p_facets, width = 16, height = 12, units = "in", dpi = 600)
    
    
    #### 14.2 Mortality totals ####
    # Get 2019 population
    partitions_Detroit <- read.csv("output_data/Partitions/partitions_detroit.csv")[, -1] 
    
    partitions_detroit_complete <- partitions_Detroit %>%
      dplyr::select(-state_ID, -block_area) %>%
      left_join(census_block_variables, by = "census_block_ID")
    
    population_detroit_2019 <- partitions_detroit_complete %>%
      mutate(Population = population_in * area_ratio_block) %>%
      group_by(pixel_ID, row, col) %>%
      summarise(pop_2019 = sum(Population)) %>%
      ungroup()
    ...
    # Summarize total mortality per scenario
    benmap_all_summary_detroit <- benmap_all_detroit %>%
      st_drop_geometry() %>%
      group_by(Scenario) %>%
      summarise(total_deaths = sum(Point.Estim, na.rm = TRUE), .groups = "drop")  %>%
      mutate(deaths_per_million = (total_deaths / sum(population_detroit_2019$pop_2019)) * 1e6
      )
    
    
#------------------------------------------------- SITING SENSITIVITY: 3. DISTIRBUTION ASSESSMENT ---------------------------------------------------------------------------    
#### FUNCTIONS ####
    ...
    weighted_average <- function(data, impact_var, population_var) {
      ...
      round(sum_products / total_population, 4)
    }
    ...
    calculate_weighted_average_bins_income <- function(city_name, city_data) {
      ...
      percentiles <- quantile(city_data$weighted_median_income,
                              probs = c(1/3, 2/3), na.rm = TRUE)
      
      bins <- list(
        list(name = "0to33percentile", min = -Inf, max = percentiles[1]),
        list(name = "33to66percentile", min = percentiles[1], max = percentiles[2]),
        list(name = "66to100percentile", min = percentiles[2], max = Inf)
      )
      
      results <- data.frame()
      
      for (sc in unique(city_data$Scenario)) {
        
        df_sc <- filter(city_data, Scenario == sc)
        
        for (b in bins) {
          
          df_bin <- df_sc %>%
            filter(weighted_median_income >= b$min &
                     weighted_median_income <  b$max)
          ...
          results <- bind_rows(
            ...
            data.frame(
              ...
              Scenario = sc,
              bin     = b$name,
              PM25    = weighted_average(df_bin, "PM25", "Population"),
              deaths  = weighted_average(df_bin, "deaths", "Population")
            )
          )
        }
      }
    ...
    #4. ciity analysis
    city_analysis <- function(city_name, city_data) {
      
      groups <- c("Population", "White", "Black", "Hispanic", "Other")
        ...
        for (g in groups) {
          ...
          results <- bind_rows(
            ...
            data.frame(
              ...
              Scenario = sc,
              group    = g,
              PM25     = weighted_average(df_sc, "PM25", g),
              deaths  = weighted_average(df_sc, "deaths", g)
            )
          )
        }
      }
...
#### Census  data ####
    ...
    input_ethnicity_data <- read.csv("data/BenMAP/ACS/ACSDT5Y2019.B03002_2024-04-13T182630/ACSDT5Y2019.B03002-Data.csv")  #From census data table B03002 on race at the census block level. Note we rename column to match census block metadata
    
    input_income_data <-read.csv("data/BenMAP/ACS/ACSDT5Y2019.B19013_2024-02-28T213319/ACSDT5Y2019.B19013-Data.csv")  #From census data table B19013 on race at the census block level. Note we rename column to match census block metadata
    
    input_income_data_tract <-read.csv("data/BenMAP/ACS/ACSDT5Y2019.B19013_2024-04-17T151300_tract/ACSDT5Y2019.B19013-Data.csv")  #From census data table B19013 on race at the census TRACT level. Note we rename column to match census block metadata
    
    input_income_data_county <-read.csv("data/BenMAP/ACS/ACSDT5Y2019.B19013_2024-04-17T154614_county/ACSDT5Y2019.B19013-Data.csv")  #From census data table B19013 on race at the census TRACT level. Note we rename column to match census block metadata
...
##### 2. LOS ANGELES #####    
    #### 2.1. Load all relevant files ####
    #Partitions
    partitions_LA <- read.csv("output_data/Partitions/partitions_LA.csv") %>% dplyr::select(-X)
    
    #This one has pollution
    all_scenarios  %>%
      st_drop_geometry() %>% 
      dplyr::select(row, col, PM25_AVG, Scenario) %>%
      rename(PM25 = PM25_AVG)-> pollution_all_LA_distribution
    
    #This one has mortality
    benmap_all_LA %>%
      st_drop_geometry() %>% 
      dplyr::select(ROW, COL, Point.Estim, Scenario) %>%
      rename(row = ROW,
        col = COL,
        deaths = Point.Estim)-> benmap_all_LA_distribution
    
    #### 2.2. Create partitions file ####
    #Race
    partitions_LA %>%
    ...
    #Join all
    analysis_LA <- partitions_LA_race %>%
      left_join(partitions_LA_income,
                by = c("pixel_ID", "row", "col")) %>%
      left_join(pollution_all_LA_distribution,
                by = c("row", "col")) %>%
      left_join(benmap_all_LA_distribution,
                by = c("row", "col", "Scenario")) %>%
      #Filter out NAs in PM25 (which happen at borders)
      filter(!is.na(PM25)) %>%
      mutate(deaths = if_else(Population == 0, 0, deaths))
    
    #### 2.3. Apply functions ####
    LA_city_weighted <- city_analysis("LA", analysis_LA)
  
    LA_income_results <- calculate_weighted_income_scenarios("LA", analysis_LA)
    LA_race_results <- calculate_weighted_average_bins_white("LA", analysis_LA)
    
    #### 2.4 Figures ####
    LA_city_weighted %>%
      filter(group == "Population") %>%
      mutate(CDR = ifelse(grepl("High CDR", Scenario), "High CDR", "Low CDR"),
             Sensitivity = case_when(grepl("^Base", Scenario) ~ "Base",
                                grepl("Population", Scenario) ~ "Population",
                                grepl("Minority", Scenario) ~ "Minority",
                                grepl("Income", Scenario) ~ "Income"),
              Sensitivity = factor(Sensitivity, levels = c("Base", "Population", "Minority", "Income"))) %>%
      tidyr::pivot_longer(
        cols = c(PM25, deaths),
        names_to = "Metric",
        values_to = "Value" ) %>%
      mutate( Value = as.numeric(Value),  
              Metric = recode(Metric, PM25 = "PM2.5 exposure",deaths = "Mortality"))-> city_avg_long
    
    bin_colors <- c(
      "low" = "#522a79",     # low income
      "mid" = "#b6387a",     # middle income
      "high" = "#f58863"     # high income
    )
    
    bin_colors_race <- c(
      "0to30white" = "#522a79",     # low income
      "31to60white" = "#b6387a",     # middle income
      "61to100white" = "#f58863"     # high income
    )
    
      #### Income####
      plot_data <- LA_income_results %>%
        mutate(
          CDR = ifelse(grepl("High CDR", Scenario), "High CDR", "Low CDR"),
          Sensitivity = case_when(
          ...
          Sensitivity = factor(
            Sensitivity,
            levels = c("Base", "Population", "Minority", "Income"))) %>%
        tidyr::pivot_longer(
          cols = c(PM25, deaths),
          names_to = "Metric",
          values_to = "Value" ) %>%
        mutate( Value = as.numeric(Value),  
                Metric = recode(Metric, PM25 = "PM2.5 exposure",deaths = "Mortality"))
      # Make Sensitivity factor with labels for both tables
      plot_data <- plot_data %>%
        mutate(Sensitivity = factor(Sensitivity, levels = c("Base", "Population", "Minority", "Income")))
      
      city_avg_long <- city_avg_long %>%
        mutate(Sensitivity = factor(Sensitivity, levels = c("Base", "Population", "Minority", "Income")))
      
      # Plot
      p <- ggplot(plot_data, aes(x = Sensitivity, y = Value, fill = bin)) +
        geom_point(shape = 21, color = "black", size = 6, stroke = 0.5) +  # black outline
        geom_point(data = city_avg_long, aes(x = Sensitivity, y = Value), shape = 21, fill = "black", color = "black", size = 6, stroke = 0.5) +  # city avg
        facet_grid(Metric ~ CDR, scales = "free_y") +
        scale_fill_manual(values = bin_colors, name = "Income bin") +
        labs(title = "Population-weighted PM2.5 exposure and mortality",
             subtitle = "Base case and sensitivity scenarios (Los Angeles, 2050)",
             y = "Value", x = NULL) +
        theme_bw() +
        theme(panel.border = element_rect(color = "black", fill = NA, size = 0.5),
              strip.background = element_rect(fill = "white", color = "black"),
              axis.text.x = element_text(angle = 30, hjust = 1))
      ggsave("figures/BenMAP/sensitivity_siting/distribution/LA/income_based_distribution.png", p, width = 16, height = 12, units = "in", dpi = 600)
      ggsave("figures/BenMAP/sensitivity_siting/distribution/LA/income_based_distribution.svg", p, width = 16, height = 12, units = "in", dpi = 600)
    
      #### Race####
      plot_data_race <- LA_race_results %>%
        mutate(
          CDR = ifelse(grepl("High CDR", Scenario), "High CDR", "Low CDR"),
          Sensitivity = case_when(
            grepl("^Base", Scenario) ~ "Base",
        ...
        tidyr::pivot_longer(cols = c(PM25,deaths), names_to = "Metric", values_to = "Value") %>%
        mutate(Value = as.numeric(Value),
               Metric = recode(Metric, PM25 = "PM2.5 exposure", deaths = "Mortality"),
               Sensitivity = factor(Sensitivity, levels = c("Base","Population","Minority","Income")))
               
       p_race <- ggplot(plot_data_race, aes(x = Sensitivity, y = Value, fill = bin)) +
         geom_point(shape = 21, color = "black", size = 6, stroke = 0.5) +  # race bins
         geom_point(data = city_avg_long, aes(x = Sensitivity, y = Value), shape = 21, fill = "black", color = "black", size = 6, stroke = 0.5) +  # city-wide avg
         facet_grid(Metric ~ CDR, scales = "free_y") +
         scale_fill_manual(values = bin_colors_race, name = "Percent white") +
         labs(title = "Population-weighted PM2.5 exposure and mortality by race",
              subtitle = "Base case and sensitivity scenarios (Los Angeles, 2050)",
              y = "Value", x = NULL) +
         theme_bw() +
         theme(panel.border = element_rect(color = "black", fill = NA, size = 0.5),
               strip.background = element_rect(fill = "white", color = "black"),
               axis.text.x = element_text(angle = 30, hjust = 1))
       ggsave("figures/BenMAP/sensitivity_siting/distribution/LA/race_based_distribution.png", p_race, width = 16, height = 12, units = "in", dpi = 600)
       ggsave("figures/BenMAP/sensitivity_siting/distribution/LA/race_based_distribution.svg", p_race, width = 16, height = 12, units = "in", dpi = 600)
         
##### 11. PHILADELPHIA ####
    #### 11.1. Load all relevant files ####
    #Partitions
    partitions_phil <- read.csv("output_data/Partitions/partitions_phil.csv") %>% dplyr::select(-X)
    
    #This one has pollution
    all_scenarios_phil  %>%
      st_drop_geometry() %>% 
      dplyr::select(row, col, PM25_AVG, Scenario) %>%
      rename(PM25 = PM25_AVG)-> pollution_all_phil_distribution
    ...
    benmap_all_phil %>%
      ...
      rename(row = ROW,
             col = COL,
             deaths = Point.Estim)-> benmap_all_phil_distribution
    
    #### 11.2. Create partitions file ####
    #Race
    partitions_phil %>%
      left_join(ethnicity_data, by = "census_block_ID") %>%
      mutate(Population = total_population * area_ratio_block,
      ...
      mutate(percent_black = if_else(Population == 0, 0, (Black * 100)/Population),
             percent_white = if_else(Population == 0, 0, (White * 100)/Population),
             percent_hispanic = if_else(Population == 0, 0, (Hispanic * 100)/Population),
             percent_other = if_else(Population == 0, 0, (Other * 100)/Population))-> partitions_phil_race
    
    #Create table for SM
    partitions_phil_race %>%
      summarise(population = sum(Population), White = sum(White), Black = sum(Black),
                Hispanic = sum(Hispanic), Other = sum(Other)) -> total_race_phil
    ...
    partitions_phil %>%
      ...
      ungroup() %>%
      dplyr::select(pixel_ID, row, col, weighted_median_income) %>%
      unique() -> partitions_phil_income
    
    #Join all
    analysis_phil <- partitions_phil_race %>%
      left_join(partitions_phil_income,
                by = c("pixel_ID", "row", "col")) %>%
      left_join(pollution_all_phil_distribution,
                by = c("row", "col")) %>%
      left_join(benmap_all_phil_distribution,
                by = c("row", "col", "Scenario")) %>%
      #Filter out NAs in PM25 (which happen at borders)
      filter(!is.na(PM25)) %>%
      mutate(deaths = if_else(Population == 0, 0, deaths))
    
    #### 11.3. Apply functions ####
    Philadelphia_city_weighted <- city_analysis("Philadelphia", analysis_phil)
    
    Philadelphia_income_results <- calculate_weighted_income_scenarios("Philadelphia", analysis_phil)
    Philadelphia_race_results <- calculate_weighted_average_bins_white("Philadelphia", analysis_phil)
    
    #### 11.4 Figures ####
    Philadelphia_city_weighted %>%
      filter(group == "Population") %>%
      mutate(CDR = ifelse(grepl("High CDR", Scenario), "High CDR", "Low CDR"),
             Sensitivity = case_when(grepl("^Base", Scenario) ~ "Base",
                                     grepl("Population", Scenario) ~ "Population",
      ...
      #### Income####
      plot_data <- Philadelphia_income_results %>%
        mutate(
          CDR = ifelse(grepl("High CDR", Scenario), "High CDR", "Low CDR"),
          Sensitivity = case_when(
            grepl("^Base", Scenario) ~ "Base",
      ...
      p <- ggplot(plot_data, aes(x = Sensitivity, y = Value, fill = bin)) +
        ...
        scale_fill_manual(values = bin_colors, name = "Income bin") +
        labs(title = "Population-weighted PM2.5 exposure and mortality",
             subtitle = "Base case and sensitivity scenarios (Philadelphia, 2050)",
             y = "Value", x = NULL) +
        theme_bw() +
        theme(panel.border = element_rect(color = "black", fill = NA, size = 0.5),
              strip.background = element_rect(fill = "white", color = "black"),
              axis.text.x = element_text(angle = 30, hjust = 1))
      ggsave("figures/BenMAP/sensitivity_siting/distribution/Philadelphia/income_based_distribution.png", p, width = 16, height = 12, units = "in", dpi = 600)
      ggsave("figures/BenMAP/sensitivity_siting/distribution/Philadelphia/income_based_distribution.svg", p, width = 16, height = 12, units = "in", dpi = 600)
      
      ####Race####
      plot_data_race <- Philadelphia_race_results %>%
        mutate(
          CDR = ifelse(grepl("High CDR", Scenario), "High CDR", "Low CDR"),
          Sensitivity = case_when(
            grepl("^Base", Scenario) ~ "Base",
      ...
      p_race <- ggplot(plot_data_race, aes(x = Sensitivity, y = Value, fill = bin)) +
        ...
        theme(panel.border = element_rect(color = "black", fill = NA, size = 0.5),
              strip.background = element_rect(fill = "white", color = "black"),
              axis.text.x = element_text(angle = 30, hjust = 1))
      ggsave("figures/BenMAP/sensitivity_siting/distribution/Philadelphia/race_based_distribution.png", p_race, width = 16, height = 12, units = "in", dpi = 600)
      ggsave("figures/BenMAP/sensitivity_siting/distribution/Philadelphia/race_based_distribution.svg", p_race, width = 16, height = 12, units = "in", dpi = 600)  
      
##### 14. DETROIT ####
    ### 14.1. Load all relevant files ####
    #Partitions
    partitions_detroit <- read.csv("output_data/Partitions/partitions_detroit.csv") %>% dplyr::select(-X)
    
    #This one has pollution
    all_scenarios_detroit  %>%
      st_drop_geometry() %>% 
      dplyr::select(row, col, PM25_AVG, Scenario) %>%
      rename(PM25 = PM25_AVG)-> pollution_all_detroit_distribution
    ...
    benmap_all_detroit %>%
      ...
      rename(row = ROW,
             col = COL,
             deaths = Point.Estim)-> benmap_all_detroit_distribution
    ...
    #Join all
    analysis_detroit <- partitions_detroit_race %>%
      left_join(partitions_detroit_income,
                by = c("pixel_ID", "row", "col")) %>%
      left_join(pollution_all_detroit_distribution,
                by = c("row", "col")) %>%
      left_join(benmap_all_detroit_distribution,
                by = c("row", "col", "Scenario")) %>%
      #Filter out NAs in PM25 (which happen at borders)
      filter(!is.na(PM25)) %>%
      mutate(deaths = if_else(Population == 0, 0, deaths))
    ...
    #### 11.3 Apply functions ####
    Detroit_city_weighted <- city_analysis("Detroit", analysis_detroit)
    
    Detroit_income_results <- calculate_weighted_income_scenarios("Detroit", analysis_detroit)
    Detroit_race_results <- calculate_weighted_average_bins_white("Detroit", analysis_detroit)
    
    #### 11.4 Figures ####
    Detroit_city_weighted %>%
      filter(group == "Population") %>%
      mutate(CDR = ifelse(grepl("High CDR", Scenario), "High CDR", "Low CDR"),
             Sensitivity = case_when(grepl("^Base", Scenario) ~ "Base",
                                     grepl("Population", Scenario) ~ "Population",
    ...
    #### Income####
    plot_data <- Detroit_income_results %>%
      mutate(
        CDR = ifelse(grepl("High CDR", Scenario), "High CDR", "Low CDR"),
        Sensitivity = case_when(
          grepl("^Base", Scenario) ~ "Base",
    ...
    p <- ggplot(plot_data, aes(x = Sensitivity, y = Value, fill = bin)) +
      ...
      scale_fill_manual(values = bin_colors, name = "Income bin") +
      labs(title = "Population-weighted PM2.5 exposure and mortality",
           subtitle = "Base case and sensitivity scenarios (Detroit, 2050)",
           y = "Value", x = NULL) +
      theme_bw() +
      theme(panel.border = element_rect(color = "black", fill = NA, size = 0.5),
            strip.background = element_rect(fill = "white", color = "black"),
            axis.text.x = element_text(angle = 30, hjust = 1))
    ggsave("figures/BenMAP/sensitivity_siting/distribution/Detroit/income_based_distribution.png", p, width = 16, height = 12, units = "in", dpi = 600)
    ggsave("figures/BenMAP/sensitivity_siting/distribution/Detroit/income_based_distribution.svg", p, width = 16, height = 12, units = "in", dpi = 600)
    
    ####Race####
    plot_data_race <- Detroit_race_results %>%
      mutate(
        CDR = ifelse(grepl("High CDR", Scenario), "High CDR", "Low CDR"),
        Sensitivity = case_when(
          grepl("^Base", Scenario) ~ "Base",
    ...
    p_race <- ggplot(plot_data_race, aes(x = Sensitivity, y = Value, fill = bin)) +
      ...
      theme(panel.border = element_rect(color = "black", fill = NA, size = 0.5),
            strip.background = element_rect(fill = "white", color = "black"),
            axis.text.x = element_text(angle = 30, hjust = 1))
    ggsave("figures/BenMAP/sensitivity_siting/distribution/Detroit/race_based_distribution.png", p_race, width = 16, height = 12, units = "in", dpi = 600)
    ggsave("figures/BenMAP/sensitivity_siting/distribution/Detroit/race_based_distribution.svg", p_race, width = 16, height = 12, units = "in", dpi = 600)  
    
    
#------------------------------------------------- POPULATION SENSITIVITY: 1. PREPARING POPULATION FILES ---------------------------------------------------------------------------
      #Here we want to test sensitivity to our results for 3 cities: Los Angeles, Philadelphia, Detroit (same as sensitivity for air pollution) 
      #We use future population projections from https://www-nature-com.ezproxy.library.wisc.edu/articles/s41597-022-01675-x
      #authors analyze different SSPs and see where population would live in a 1km grid
      ...
      pop_2050_SSP2 <- rast("data/population/SSP2_2050.tif") 
      pop_2050_SSP1 <- rast("data/population/SSP1_2050.tif") 
      pop_2050_SSP3 <- rast("data/population/SSP3_2050.tif") 
      pop_2050_SSP4 <- rast("data/population/SSP4_2050.tif") 
      pop_2050_SSP5 <- rast("data/population/SSP5_2050.tif") 
      
      #Files and paths
      ssp_list <- c("SSP1", "SSP2", "SSP3", "SSP4", "SSP5")
      pop_folder <- "data/population/"
      out_folder <- "figures/BenMAP/sensitivity/"
      
#### 2. LOS ANGELES ####
  #### 2.1. Reframe data ####
    # 1. Load LA shapefile and grid
      cbsa_shapefile_LA <- cbsa_shapefile %>%
        filter(AFFGEOID == "310M500US31080")
      
      grid_data_south_west_NAD83_LA <- st_read("output_data/shapefiles/1km/LA/1km_shapefile_2019_03252024_LA.shp") 
      
      # Ensure CRS alignment
      grid_data_south_west_NAD83_LA <- st_transform(grid_data_south_west_NAD83_LA, crs = crs(pop_2050_SSP2))
      cbsa_shapefile_LA <- st_transform(cbsa_shapefile_LA, crs = crs(pop_2050_SSP2))
      
    # 2. Get 2019 population (same for all SSPs)
      partitions_LA <- read.csv("output_data/Partitions/partitions_LA.csv")[, -1] 
    ...
    # 3. Initialize results table
      population_totals_LA <- data.frame(
        SSP = character(),
        Year = character(),
    ...
    # 4. Loop over SSPs
      for(ssp in ssp_list){
        ...
        # Load 2050 raster
        pop_2050 <- rast(paste0(pop_folder, ssp, "_2050.tif"))
        
        # Crop and mask for LA
        pop_LA <- crop(pop_2050, cbsa_shapefile_LA)
        pop_LA <- mask(pop_LA, cbsa_shapefile_LA)
        
        # Extract raster values to the LA grid
        pop_values <- terra::extract(pop_LA, vect(grid_data_south_west_NAD83_LA), fun = sum, na.rm = TRUE)
        
        # Merge 2019 and 2050
        population_LA <- grid_data_south_west_NAD83_LA %>%
          left_join(population_LA_2019, by = c("pixel_ID", "row", "col")) %>%
          mutate(
            pop_2019 = replace_na(pop_2019, 0),
            pop_2050 = replace_na(pop_values[,2], 0)
          )
        
        # Compute totals and append to table
        population_totals_LA <- rbind(
          population_totals_LA,
          data.frame(
            SSP = ssp,
            Year = "2019",
            Total_Population = sum(population_LA$pop_2019, na.rm = TRUE)
          ),
          data.frame(
            SSP = ssp,
            Year = "2050",
            Total_Population = sum(population_LA$pop_2050, na.rm = TRUE)
          )
        )
        
        # Assign the dataframe to the environment dynamically
        assign(paste0("population_", ssp, "_2050_LA"), population_LA, envir = .GlobalEnv)
        
        # Prepare long dataframe for faceted plot
        grid_pop_long <- population_LA %>%
          st_drop_geometry() %>%
          dplyr::select(pixel_ID, row, col, pop_2019, pop_2050) %>%
          pivot_longer(cols = c(pop_2019, pop_2050),
                       names_to = "Year",
                       values_to = "Population") %>%
          mutate(Year = ifelse(Year == "pop_2019", "2019", "2050")) 
          
        population_LA %>%
          left_join(grid_pop_long, by = c("pixel_ID","row","col")) -> grid_pop_long_sf
         
        # Faceted population plot
        p_facets_sf <- ggplot() +
          geom_sf(data = grid_pop_long_sf, aes(fill = Population), color = NA) +
          geom_sf(data = cbsa_shapefile_LA, fill = NA, color = "black", size = 0.5) +
          scale_fill_viridis_c(option = "magma") +
          facet_wrap(~Year) +
          labs(title = paste("Los Angeles Population per Grid Cell -", ssp),
               fill = "Population") +
          theme_minimal() +
       ...
       #ggsave(paste0(out_folder,"LA/LA_pop_2019_2050_", ssp,".png"), p_facets_sf, width = 10, height = 8, dpi = 300)
        #ggsave(paste0(out_folder,"LA/LA_pop_2019_2050_", ssp,".svg"), p_facets_sf, width = 10, height = 8, dpi = 300)
        
      }
      
      #### All in one figure
      population_SSP1_2050_LA %>% 
        dplyr::select(-pop_2019, pop_2050_SSP1 = pop_2050) %>%
        left_join(population_SSP2_2050_LA %>% 
                    st_drop_geometry()%>%
                    dplyr::select(-pop_2019, pop_2050_SSP2 = pop_2050)) %>%
        left_join(population_SSP3_2050_LA %>% 
                    st_drop_geometry()%>%
                    dplyr::select(-pop_2019, pop_2050_SSP3 = pop_2050)) %>%
        left_join(population_SSP4_2050_LA %>% 
                    st_drop_geometry()%>%
                    dplyr::select(-pop_2019, pop_2050_SSP4 = pop_2050)) %>%
        left_join(population_SSP5_2050_LA %>% 
                    st_drop_geometry()%>%
                    dplyr::select(-pop_2019, pop_2050_SSP5 = pop_2050)) %>%
        left_join(population_LA_2019)-> population_LA_2019_2050_allSSPs
      
      pop_LA_long <- population_LA_2019_2050_allSSPs %>%
        pivot_longer(
          cols = starts_with("pop_2050_") | pop_2019,
          names_to = "Variable",
          values_to = "Population"
        ) %>%
        mutate(
          Year = case_when(
            Variable == "pop_2019" ~ "2019",
            grepl("pop_2050_", Variable) ~ sub("pop_2050_", "2050 ", Variable)
          )
        )
      
      p_all <- ggplot(pop_LA_long) +
        geom_sf(aes(fill = Population), color = NA) +
        geom_sf(data = cbsa_shapefile_LA, fill = NA, color = "black", size = 0.3) +
        scale_fill_viridis_c(option = "magma",  direction = -1) +
        facet_wrap(~Year, ncol = 3) +
        labs(
          title = "Los Angeles Population per 1-km Grid Cell: 2019 vs 2050 SSPs",
          fill = "Population"
        ) +
        annotation_scale() +
        theme_minimal() +
      ...
      #ggsave(paste0(out_folder,"LA/population/LA_pop_allSSPs_2050_2019.png"), p_all, width = 10, height = 8, dpi = 300)
      #ggsave(paste0(out_folder,"LA/population/LA_pop_allSSPs_2050_2019.svg"), p_all, width = 10, height = 8, dpi = 300)
      p_all
      
      #### 2.2. Bring age groups ####
      #Here we want to calculate how much population changed from 2019 to 2050, and then apply that change to each age group from 2019 to get its value in 2050. 
      #Note this is not assuming an aging population, just future population aand same split
      # 2019 age-by-pixel table
      population_2019_data_pixel_age_final_LA <- read.csv(
        "output_data/BenMAP_files/1km/LA/Date_ 2024-03-31 1_km_population_2019_LA.csv"
      )
      
      # List of SSP columns in your combined table
      ssp_cols <- c("pop_2050_SSP1", "pop_2050_SSP2", "pop_2050_SSP3",
                    "pop_2050_SSP4", "pop_2050_SSP5")
      
      # Loop over each SSP and generate output
      for (ssp in ssp_cols) {
        
        cat("Processing", ssp, "...\n")
        
        output_name <- sub("pop_2050_", "", ssp)  # e.g., SSP1
        
        df_age <- population_LA_2019_2050_allSSPs %>%
          st_as_sf() %>%
          
          # Avoid divide-by-zero
          mutate(pop_2019_adj = if_else(pop_2019 == 0, 0.001, pop_2019),
                 change = !!sym(ssp) / pop_2019_adj) %>%
          
          # Join pixel-level 2019 age structure
          left_join(
            population_2019_data_pixel_age_final_LA,
            by = c("row" = "Row", "col" = "Column")
          ) %>%
          
          # Apply population change to each age group
          mutate(population_2050 = Population * change) %>%
          
          # Cleanup + final columns for BenMAP
          rename(
            Population_old = Population,
            Row = row, 
            Column = col,
            Population = population_2050
          ) %>%
          dplyr::select(Row, Column, AgeRange, Population, Year, 
                        Race, Ethnicity, Gender) %>%
          st_drop_geometry()
        
        # Save CSV
        outpath <- paste0(
          "output_data/BenMAP_files/1km/LA/population_2050",
          output_name,
          "_data_pixel_age_final_LA.csv"
        )
        
        #write.csv(df_age, outpath, row.names = FALSE)
...
#### 11. PHILADELPHIA ####
    #### 11.1. Reframe data ####
      # 1. Load Philadelphia shapefile and grid
      cbsa_shapefile_phil <- cbsa_shapefile %>%
        filter(AFFGEOID == "310M500US37980")
      
      grid_data_south_west_NAD83_phil <- st_read("output_data/shapefiles/1km/Philadelphia/1km_shapefile_2019_03252024_phil.shp") 
      
      # Ensure CRS alignment
      grid_data_south_west_NAD83_phil <- st_transform(grid_data_south_west_NAD83_phil, crs = crs(pop_2050_SSP2))
      cbsa_shapefile_phil <- st_transform(cbsa_shapefile_phil, crs = crs(pop_2050_SSP2))
      
      # 2. Get 2019 population (same for all SSPs)
      partitions_phil <- read.csv("output_data/Partitions/partitions_phil.csv")[, -1] 
      
      partitions_phil_complete <- partitions_phil %>%
        dplyr::select(-state_ID, -block_area) %>%
        left_join(census_block_variables, by = "census_block_ID")
      ...
      population_phil_2019 <- partitions_phil_complete %>%
        mutate(Population = population_in * area_ratio_block) %>%
        group_by(pixel_ID, row, col) %>%
        dplyr::summarise(pop_2019 = sum(Population)) %>%
        ungroup()
      
      # 3. Initialize results table
      population_totals_phil <- data.frame(
        SSP = character(),
        Year = character(),
        Total_Population = numeric()
      )
      ...
      for(ssp in ssp_list){
        ...
        # Crop and mask for phil
        pop_phil <- crop(pop_2050, cbsa_shapefile_phil)
        pop_phil <- mask(pop_phil, cbsa_shapefile_phil)
        
        # Extract raster values to the phil grid
        pop_values <- terra::extract(pop_phil, vect(grid_data_south_west_NAD83_phil), fun = sum, na.rm = TRUE)
        
        # Merge 2019 and 2050
        population_phil <- grid_data_south_west_NAD83_phil %>%
          left_join(population_phil_2019, by = c("pixel_ID", "row", "col")) %>%
          mutate(
            pop_2019 = replace_na(pop_2019, 0),
            pop_2050 = replace_na(pop_values[,2], 0)
          )
        ...
        # Compute totals and append to table
        population_totals_phil <- rbind(
          population_totals_phil,
          data.frame(
            SSP = ssp,
            Year = "2019",
            Total_Population = sum(population_phil$pop_2019, na.rm = TRUE)
          ),
          data.frame(
            SSP = ssp,
            Year = "2050",
            Total_Population = sum(population_phil$pop_2050, na.rm = TRUE)
          )
        )
        
        # Assign the dataframe to the environment dynamically
        assign(paste0("population_", ssp, "_2050_phil"), population_phil, envir = .GlobalEnv)
        
        # Prepare long dataframe for faceted plot
        grid_pop_long <- population_phil %>%
          st_drop_geometry() %>%
          dplyr::select(pixel_ID, row, col, pop_2019, pop_2050) %>%
          pivot_longer(cols = c(pop_2019, pop_2050),
                       names_to = "Year",
        ...
        p_facets_sf <- ggplot() +
          geom_sf(data = grid_pop_long_sf, aes(fill = Population), color = NA) +
          geom_sf(data = cbsa_shapefile_phil, fill = NA, color = "black", size = 0.5) +
          scale_fill_viridis_c(option = "magma") +
          facet_wrap(~Year) +
          labs(title = paste("Philadelphia Population per Grid Cell -", ssp),
               fill = "Population") +
          theme_minimal() +
          theme(axis.text = element_blank(),
                axis.title = element_blank(),
        ...
        ggsave(paste0(out_folder,"Philadelphia/Philadelphia_pop_2019_2050_", ssp,".png"), p_facets_sf, width = 10, height = 8, dpi = 300)
        ggsave(paste0(out_folder,"Philadelphia/Philadelphia_pop_2019_2050_", ssp,".svg"), p_facets_sf, width = 10, height = 8, dpi = 300)
        
        # Difference plot
        population_phil$pop_diff <- population_phil$pop_2050 - population_phil$pop_2019
        
        p_diff <- ggplot() +
          geom_sf(data = population_phil, aes(fill = pop_diff), color = NA) +
          geom_sf(data = cbsa_shapefile_phil, fill = NA, color = "black", size = 0.5) +
          scale_fill_gradient2(
            low = "red",
            mid = "white",
          ...
          labs(title = paste("Philadelphia Population Difference (2050 - 2019) -", ssp)) +
          theme_minimal() +
          theme(axis.text = element_blank(),
                axis.title = element_blank(),
                panel.grid = element_blank())
        
        #ggsave(paste0(out_folder,"Philadelphia/population/Philadelphia_pop_diff_2050_2019_", ssp,".png"), p_diff, width = 10, height = 8, dpi = 300)
        #ggsave(paste0(out_folder,"Philadelphia/population/Philadelphia_pop_diff_2050_2019_", ssp,".svg"), p_diff, width = 10, height = 8, dpi = 300)
        
      }
      
      #### All in one figure
      population_SSP1_2050_phil %>% 
        dplyr::select(-pop_2019, pop_2050_SSP1 = pop_2050) %>%
        left_join(population_SSP2_2050_phil %>% 
                    st_drop_geometry()%>%
                    dplyr::select(-pop_2019, pop_2050_SSP2 = pop_2050)) %>%
        left_join(population_SSP3_2050_phil %>% 
                    st_drop_geometry()%>%
                    dplyr::select(-pop_2019, pop_2050_SSP3 = pop_2050)) %>%
        left_join(population_SSP4_2050_phil %>% 
                    st_drop_geometry()%>%
                    dplyr::select(-pop_2019, pop_2050_SSP4 = pop_2050)) %>%
        left_join(population_SSP5_2050_phil %>% 
                    st_drop_geometry()%>%
                    dplyr::select(-pop_2019, pop_2050_SSP5 = pop_2050)) %>%
        left_join(population_phil_2019)-> population_phil_2019_2050_allSSPs
      
      pop_phil_long <- population_phil_2019_2050_allSSPs %>%
        pivot_longer(
          cols = starts_with("pop_2050_") | pop_2019,
          names_to = "Variable",
          values_to = "Population"
      ...
      p_all <- ggplot(pop_phil_long) +
        geom_sf(aes(fill = Population), color = NA) +
        geom_sf(data = cbsa_shapefile_phil, fill = NA, color = "black", size = 0.3) +
        scale_fill_viridis_c(option = "magma",  direction = -1) +
        facet_wrap(~Year, ncol = 3) +
        labs(
          title = "Philadelphia Population per 1-km Grid Cell: 2019 vs 2050 SSPs",
          fill = "Population"
        ) +
        annotation_scale() +
        theme_minimal() +
      ...
      #ggsave(paste0(out_folder,"Philadelphia/population/Philadelphia_pop_allSSPs_2050_2019.png"), p_all, width = 10, height = 8, dpi = 300)
      #ggsave(paste0(out_folder,"Philadelphia/population/Philadelphia_pop_allSSPs_2050_2019.svg"), p_all, width = 10, height = 8, dpi = 300)
      p_all
    
    #### 11.2. Bring age groups ####
    #Here we want to calculate how much population changed from 2019 to 2050, and then apply that change to each age group from 2019 to get its value in 2050. 
    #Note this is not assuming an aging population, just future population
    # 2019 age-by-pixel base file
    population_2019_data_pixel_age_final_phil <- read.csv(
      "output_data/BenMAP_files/1km/Philadelphia/Date_ 2024-03-31 1_km_population_2019_philadelphia.csv"
    )
    
    # List of SSP columns in your combined Philadelphia table
    ssp_cols_phil <- c("pop_2050_SSP1", "pop_2050_SSP2", "pop_2050_SSP3",
                       "pop_2050_SSP4", "pop_2050_SSP5")
    
    # Loop over SSPs
    for (ssp in ssp_cols_phil) {
      
      cat("Processing", ssp, "...\n")
      
      output_name <- sub("pop_2050_", "", ssp)  # SSP1 → SSP1
      
      df_age <- population_phil_2019_2050_allSSPs %>%
        st_as_sf() %>%
        
        # Avoid divide-by-zero in 2019 population
        mutate(pop_2019_adj = if_else(pop_2019 == 0, 0.001, pop_2019),
               change = !!sym(ssp) / pop_2019_adj) %>%
        ...
        # Scale each age group to 2050
        mutate(population_2050 = Population * change) %>%
        
        # Output format for BenMAP
        rename(
          Population_old = Population,
          Row = row,
          Column = col,
      ...
      # Output path
      outpath <- paste0(
        "output_data/BenMAP_files/1km/Philadelphia/population_2050",
        output_name,
        "_data_pixel_age_final_phil.csv"
      )
...
#### 14. DETROIT ####
    #### 14.1. Reframe data ####
    # 1. Load Detroit shapefile and grid
    cbsa_shapefile_detroit <- cbsa_shapefile %>%
      filter(AFFGEOID == "310M500US19820")
    
    grid_data_south_west_NAD83_detroit <- st_read("output_data/shapefiles/1km/Detroit/1km_shapefile_2019_03252024_detroit.shp") 
    
    # Ensure CRS alignment
    grid_data_south_west_NAD83_detroit <- st_transform(grid_data_south_west_NAD83_detroit, crs = crs(pop_2050_SSP2))
    cbsa_shapefile_detroit <- st_transform(cbsa_shapefile_detroit, crs = crs(pop_2050_SSP2))
    ...
    # 3. Initialize results table
    population_totals_detroit <- data.frame(
      SSP = character(),
      Year = character(),
      Total_Population = numeric()
    )
    ...
    for(ssp in ssp_list){
      ...
      # Crop and mask for detroit
      pop_detroit <- crop(pop_2050, cbsa_shapefile_detroit)
      pop_detroit <- mask(pop_detroit, cbsa_shapefile_detroit)
      
      # Extract raster values to the detroit grid
      pop_values <- terra::extract(pop_detroit, vect(grid_data_south_west_NAD83_detroit), fun = sum, na.rm = TRUE)
      
      # Merge 2019 and 2050
      population_detroit <- grid_data_south_west_NAD83_detroit %>%
        left_join(population_detroit_2019, by = c("pixel_ID", "row", "col")) %>%
        mutate(
          pop_2019 = replace_na(pop_2019, 0),
          pop_2050 = replace_na(pop_values[,2], 0)
        )
      ...
      # Compute totals and append to table
      population_totals_detroit <- rbind(
        population_totals_detroit,
        data.frame(
          SSP = ssp,
          Year = "2019",
          Total_Population = sum(population_detroit$pop_2019, na.rm = TRUE)
        ),
        data.frame(
          SSP = ssp,
          Year = "2050",
          Total_Population = sum(population_detroit$pop_2050, na.rm = TRUE)
        )
      )
      
      # Assign the dataframe to the environment dynamically
      assign(paste0("population_", ssp, "_2050_detroit"), population_detroit, envir = .GlobalEnv)
      
      # Prepare long dataframe for faceted plot
      grid_pop_long <- population_detroit %>%
        st_drop_geometry() %>%
        dplyr::select(pixel_ID, row, col, pop_2019, pop_2050) %>%
        pivot_longer(cols = c(pop_2019, pop_2050),
                     names_to = "Year",
      ...
      p_facets_sf <- ggplot() +
        geom_sf(data = grid_pop_long_sf, aes(fill = Population), color = NA) +
        geom_sf(data = cbsa_shapefile_detroit, fill = NA, color = "black", size = 0.5) +
        scale_fill_viridis_c(option = "magma") +
        facet_wrap(~Year) +
        labs(title = paste("Detroit Population per Grid Cell -", ssp),
             fill = "Population") +
        theme_minimal() +
        theme(axis.text = element_blank(),
              axis.title = element_blank(),
      ...
      #ggsave(paste0(out_folder,"Detroit/population/Detroit_pop_2019_2050_", ssp,".png"), p_facets_sf, width = 10, height = 8, dpi = 300)
      #ggsave(paste0(out_folder,"Detroit/population/Detroit_pop_2019_2050_", ssp,".svg"), p_facets_sf, width = 10, height = 8, dpi = 300)
      
      # Difference plot
      population_detroit$pop_diff <- population_detroit$pop_2050 - population_detroit$pop_2019
      
      p_diff <- ggplot() +
        geom_sf(data = population_detroit, aes(fill = pop_diff), color = NA) +
        geom_sf(data = cbsa_shapefile_detroit, fill = NA, color = "black", size = 0.5) +
        scale_fill_gradient2(
          low = "red",
          mid = "white",
        ...
        labs(title = paste("Detroit Population Difference (2050 - 2019) -", ssp)) +
        theme_minimal() +
        theme(axis.text = element_blank(),
              axis.title = element_blank(),
              panel.grid = element_blank())
      
      #ggsave(paste0(out_folder,"Detroit/population/Detroit_pop_diff_2050_2019_", ssp,".png"), p_diff, width = 10, height = 8, dpi = 300)
      #ggsave(paste0(out_folder,"Detroit/population/Detroit_pop_diff_2050_2019_", ssp,".svg"), p_diff, width = 10, height = 8, dpi = 300)
      
    }
    
    #### All in one figure
    population_SSP1_2050_detroit %>% 
      dplyr::select(-pop_2019, pop_2050_SSP1 = pop_2050) %>%
      left_join(population_SSP2_2050_detroit %>% 
                  st_drop_geometry()%>%
                  dplyr::select(-pop_2019, pop_2050_SSP2 = pop_2050)) %>%
      left_join(population_SSP3_2050_detroit %>% 
                  st_drop_geometry()%>%
                  dplyr::select(-pop_2019, pop_2050_SSP3 = pop_2050)) %>%
      left_join(population_SSP4_2050_detroit %>% 
                  st_drop_geometry()%>%
                  dplyr::select(-pop_2019, pop_2050_SSP4 = pop_2050)) %>%
      left_join(population_SSP5_2050_detroit %>% 
                  st_drop_geometry()%>%
                  dplyr::select(-pop_2019, pop_2050_SSP5 = pop_2050)) %>%
      left_join(population_detroit_2019)-> population_detroit_2019_2050_allSSPs
    
    pop_detroit_long <- population_detroit_2019_2050_allSSPs %>%
      pivot_longer(
        cols = starts_with("pop_2050_") | pop_2019,
        names_to = "Variable",
        values_to = "Population"
    ...
    p_all <- ggplot(pop_detroit_long) +
      geom_sf(aes(fill = Population), color = NA) +
      geom_sf(data = cbsa_shapefile_detroit, fill = NA, color = "black", size = 0.3) +
      scale_fill_viridis_c(option = "magma",  direction = -1) +
      facet_wrap(~Year, ncol = 3) +
      labs(
        title = "Detroit Population per 1-km Grid Cell: 2019 vs 2050 SSPs",
        fill = "Population"
      ) +
      annotation_scale() +
      theme_minimal() +
    ...
    #ggsave(paste0(out_folder,"Detroit/population/Detroit_pop_allSSPs_2050_2019.png"), p_all, width = 10, height = 8, dpi = 300)
    #ggsave(paste0(out_folder,"Detroit/population/Detroit_pop_allSSPs_2050_2019.svg"), p_all, width = 10, height = 8, dpi = 300)
    p_all
    
    #### 14.2. Bring age groups ####
    #Here we want to calculate how much population changed from 2019 to 2050, and then apply that change to each age group from 2019 to get its value in 2050. 
    #Note this is not assuming an aging population, just future population
    population_2019_data_pixel_age_final_detroit <- read.csv("output_data/BenMAP_files/1km/Detroit/Date_ 2024-04-01 1_km_population_2019_detroit.csv")
    
    # List of SSP columns in your combined Detroit table
    ssp_cols_detroit <- c("pop_2050_SSP1", "pop_2050_SSP2", "pop_2050_SSP3",
                       "pop_2050_SSP4", "pop_2050_SSP5")
    
    # Loop over SSPs
    for (ssp in ssp_cols_detroit) {
      
      cat("Processing", ssp, "...\n")
      
      output_name <- sub("pop_2050_", "", ssp)  # SSP1 → SSP1
      
      df_age <- population_detroit_2019_2050_allSSPs %>%
        st_as_sf() %>%
      ...
      # Output path
      outpath <- paste0(
        "output_data/BenMAP_files/1km/Detroit/population_2050",
        output_name,
        "_data_pixel_age_final_detroit.csv"
      )
...
#------------------------------------------------- POPULATION SENSITIVITY: 2. MORTALITY BENMAP RESULTS (1-KM) ---------------------------------------------------------------------------
    #Using the population files from above, here we analyze mortality impacts from BenMAP files
...
#### 2. LOS ANGELES ####
    ...
    #### 2.1 Mortality figures ####
    #Load original BenMAP results
    benmap_base <- st_read("BenMAP_results/1km_V2/LA/health_impacts_2050_1km/Health Impacts-Pope 2019_20240520.shp")
    ...
    #Process base
    benmap_base_proc <- process_benmap(benmap_base, "BASE")
    
    #Read SSPs
    ssp_list <- paste0("SSP", 1:5)
    benmap_ssp_paths <- paste0(
      "BenMAP_results/1km_V2/LA/2050_pop_sens/highVSlow_mortality_2050_pop_sens_",
      ssp_list,
      "/Health Impacts-Pope 2019_20251120.shp"
    )
    
    #  Process each SSP shapefile
    benmap_ssp_list <- map2(benmap_ssp_paths, ssp_list, ~{
      sf <- st_read(.x)
      process_benmap(sf, .y)
    })
    
    # Combine BASE + SSPs
    benmap_all <- bind_rows(list(benmap_base_proc, benmap_ssp_list[[1]],
                                 benmap_ssp_list[[2]], benmap_ssp_list[[3]],
                                 benmap_ssp_list[[4]], benmap_ssp_list[[5]]))
    
    #  Create faceted plot
    p_facets <- ggplot() +
      geom_sf(data = benmap_all, aes(fill = Point.Estim), color = NA, size = 0) +
      geom_sf(data = cbsa_shapefile_LA, color = "black", fill = NA) +
      scale_fill_viridis_c(option = "rocket", direction = -1) +#limits = c(0, 1)
      facet_wrap(~Scenario, ncol = 3) +
      annotation_scale() +
      theme_void() +
      labs(title = "2050 LA Mortality due to PM2.5  (BASE vs SSPs)", #High vs Low CDR scenario
           fill = "Deaths per pixel") 
    # ggsave("figures/BenMAP/sensitivity/LA/mortality_V2/1km_mortality_2050_Benmap_LA_total_SENS.png", p_facets, width = 16, height = 12, units = "in", dpi = 600)
    # ggsave("figures/BenMAP/sensitivity/LA/mortality_V2/1km_mortality_2050_Benmap_LA_total_SENS.svg", p_facets, width = 16, height = 12, units = "in", dpi = 600)
    
    #### 2.2 Mortality totals ####
    # Summarize total mortality per scenario
    population_totals_LA_clean <- population_totals_LA %>%
      filter(Year == 2050) %>%   # keep only 2050 
      dplyr::select(SSP, Total_Population) %>%
      rename(Scenario = SSP)
    
    base_population <- population_totals_LA %>%
      filter(Year == 2019, SSP == "SSP1") %>% 
      summarise(Total_Population = sum(Total_Population)) %>%
      pull(Total_Population)
    
    benmap_all_summary <- benmap_all %>%
      st_drop_geometry() %>%
      group_by(Scenario) %>%
      summarise(total_deaths = sum(Point.Estim, na.rm = TRUE), .groups = "drop") %>%
      left_join(population_totals_LA_clean,
                by = "Scenario") %>%
      mutate(
        # insert BASE population manually
        Total_Population = ifelse(
          Scenario == "BASE",
          base_population,  # <-- correct reference
          Total_Population
        ),
...
#### 11. PHILADELPHIA ####
    #### 11.1 Mortality figures ####
    population_2019_data_pixel_age_final_Phil <- read.csv(
      "output_data/BenMAP_files/1km/Philadelphia/Date_ 2024-04-08 1_km_population_2019_phil_fixed.csv"
    )
    
    cbsa_shapefile_phil <- cbsa_shapefile %>%
      filter(AFFGEOID == "310M500US37980")
    
    #Load BASE BenMAP results
    benmap_base_Phil <- st_read(
      "BenMAP_results/1km_V2/Philadelphia/health_impacts_2050_1km/Health Impacts-Pope 2019_20240520.shp"
    )
    
    #Function to process BenMAP shapefiles
    process_benmap <- function(benmap_sf, scenario_name, cbsa_shapefile) {
      benmap_sf <- benmap_sf %>%
        mutate(
          Point.Estim = if_else(is.na(Point.Estim), 0, Point.Estim),
          Population = if_else(is.na(Population), 0, Population),
      ...
      benmap_sf <- st_transform(benmap_sf, crs = st_crs(cbsa_shapefile))
      benmap_sf <- st_intersection(benmap_sf, cbsa_shapefile)
      benmap_sf$Scenario <- scenario_name
      return(benmap_sf)
    }
    
    #### Process BASE
    benmap_base_proc_Phil <- process_benmap(benmap_base_Phil, "BASE", cbsa_shapefile_phil)
    
    #### Process SSP shapefiles
    ssp_list <- paste0("SSP", 1:5)
    benmap_ssp_paths_Phil <- paste0(
      "BenMAP_results/1km_V2/Philadelphia/2050_pop_sens/highVSlow_mortality_2050_pop_sens_",
      ssp_list,
      "/Health Impacts-Pope 2019_20251120.shp"
    )
    

    benmap_ssp_list_Phil <- map2(benmap_ssp_paths_Phil, ssp_list, ~{
      sf <- st_read(.x)
      process_benmap(sf, .y, cbsa_shapefile_phil)
    })
    
    #### Combine BASE + SSPs
    benmap_all_Phil <- bind_rows(
      list(benmap_base_proc_Phil, benmap_ssp_list_Phil[[1]],
           benmap_ssp_list_Phil[[2]], benmap_ssp_list_Phil[[3]],
           benmap_ssp_list_Phil[[4]], benmap_ssp_list_Phil[[5]])
    )
    
    #### Create faceted plot
    p_facets_Phil <- ggplot() +
      geom_sf(data = benmap_all_Phil, aes(fill = Point.Estim), color = NA, size = 0) +
      geom_sf(data = cbsa_shapefile_phil, color = "black", fill = NA) +
      scale_fill_viridis_c(option = "rocket", direction = -1) +
      facet_wrap(~Scenario, ncol = 3) +
      annotation_scale() +
      theme_void() +
      labs(title = "2050 Philadelphia Mortality due to PM2.5 per pixel (BASE vs SSPs)",
           fill = "Deaths per pixel")
    
    # ggsave("figures/BenMAP/sensitivity/Philadelphia/mortality_V2/mortality_2050_Benmap_Phil_total_SENS.png", 
    #        p_facets_Phil, width = 16, height = 12, units = "in", dpi = 600)
    # ggsave("figures/BenMAP/sensitivity/Philadelphia/mortality_V2/mortality_2050_Benmap_Phil_total_SENS.svg", 
    #        p_facets_Phil, width = 16, height = 12, units = "in", dpi = 600)
    
    #### 11.2 Mortality totals ####
    # Make sure you have your Philadelphia population totals per SSP (like population_totals_LA)
    population_totals_Phil_clean <- population_totals_phil %>%
      filter(Year == "2050") %>%
      dplyr::select(SSP, Total_Population)
    
    base_population_Phil <- population_totals_phil %>%
      filter(Year == "2019", SSP == "SSP1") %>%
      summarise(Total_Population = sum(Total_Population)) %>%
      pull(Total_Population)
    
    mortality_summary_Phil <- benmap_all_Phil %>%
      st_drop_geometry() %>%
      group_by(Scenario) %>%
      summarise(total_deaths = sum(Point.Estim, na.rm = TRUE)) %>%
      left_join(
        population_totals_Phil_clean %>%
          rename(Scenario = SSP, Total_Population = Total_Population),
        by = "Scenario"
      ) %>%
      mutate(
        Total_Population = ifelse(Scenario == "BASE", base_population_Phil, Total_Population),
        deaths_per_million = (total_deaths / Total_Population) * 1e6
      )
    
#### 14. DETROIT ####  
    #### 14.1 Mortality figures ####
    population_2019_data_pixel_age_final_Detroit <- read.csv(
      "output_data/BenMAP_files/1km/Detroit/Date_ 2024-04-08 1_km_population_2019_detroit_fixed.csv"
    )
    
    cbsa_shapefile_detroit <- cbsa_shapefile %>%
      filter(AFFGEOID == "310M500US19820") # Detroit CBSA
    
    #Load original file for Detroit
    benmap_base_Detroit <- st_read(
      "BenMAP_results/1km_V2/Detroit/health_impacts_2050_1km/Health Impacts-Pope 2019_20240517.shp"
    )
    ...
    #Process base file
    benmap_base_proc_Detroit <- process_benmap(benmap_base_Detroit, "BASE", cbsa_shapefile_detroit)
    
    #Process SSPs
    ssp_list <- paste0("SSP", 1:5)
    benmap_ssp_paths_Detroit <- paste0(
      "BenMAP_results/1km_V2/Detroit/2050_pop_sens/highVSlow_mortality_2050_pop_sens_",
      ssp_list,
      "/Health Impacts-Pope 2019_20251120.shp"
    )
    
    benmap_ssp_list_Detroit <- map2(benmap_ssp_paths_Detroit, ssp_list, ~{
      sf <- st_read(.x)
      process_benmap(sf, .y, cbsa_shapefile_detroit)
    })
    
    #Join all
    benmap_all_Detroit <- bind_rows(
      list(
        benmap_base_proc_Detroit, 
        benmap_ssp_list_Detroit[[1]],
        benmap_ssp_list_Detroit[[2]],
        benmap_ssp_list_Detroit[[3]],
        benmap_ssp_list_Detroit[[4]],
        benmap_ssp_list_Detroit[[5]]
      )
    )
    
    #Plot
    p_facets_Detroit <- ggplot() +
      geom_sf(data = benmap_all_Detroit, aes(fill = Point.Estim), color = NA, size = 0) +
      geom_sf(data = cbsa_shapefile_detroit, color = "black", fill = NA) +
      scale_fill_viridis_c(option = "rocket", direction = -1) +
      facet_wrap(~Scenario, ncol = 3) +
      annotation_scale() +
      theme_void() +
      labs(
        title = "2050 Detroit Mortality due to PM2.5 per pixel (BASE vs SSPs)",
        fill = "Deaths per pixel"
      )
    #     ggsave("figures/BenMAP/sensitivity/Detroit/mortality_V2/mortality_2050_Benmap_Detroit_total_SENS.png", 
    #   p_facets_Detroit, width = 16, height = 12, units = "in", dpi = 600)
    # ggsave( "figures/BenMAP/sensitivity/Detroit/mortality_V2/mortality_2050_Benmap_Detroit_total_SENS.svg", 
    #   p_facets_Detroit, width = 16, height = 12, units = "in", dpi = 600)
    
    #### 14.2 Mortality totals ####
    population_totals_Detroit_clean <- population_totals_detroit %>%
      filter(Year == "2050") %>%
      dplyr::select(SSP, Total_Population)
    
    base_population_Detroit <- population_totals_detroit %>%
      filter(Year == "2019", SSP == "SSP1") %>%
      summarise(Total_Population = sum(Total_Population)) %>%
      pull(Total_Population)

    mortality_summary_Detroit <- benmap_all_Detroit %>%
      st_drop_geometry() %>%
      group_by(Scenario) %>%
      summarise(total_deaths = sum(Point.Estim, na.rm = TRUE)) %>%
      left_join(
        population_totals_Detroit_clean %>%
          rename(Scenario = SSP, Total_Population = Total_Population),
        by = "Scenario"
      ) %>%
      mutate(
        Total_Population = ifelse(Scenario == "BASE", base_population_Detroit, Total_Population),
        deaths_per_million = (total_deaths / Total_Population) * 1e6
      )
    
#------------------------------------------------- POPULATION SENSITIVITY: 3. DISTIRBUTION ASSESSMENT ---------------------------------------------------------------------------  
#### FUNCTIONS ####
    #1. Weighted average pollution
    weighted_average<- function(data, impact_var, population_var) {
    ...
    calculate_weighted_average_bins_income <- function(city_name, city_data) {
      ...
      # Compute actual 33rd and 66th percentiles
      ...
      # Loop over income bins
      for (range in bins) {
        
        filtered <- city_data %>%
          filter(weighted_median_income >= range$min &
                   weighted_median_income <= range$max)
        
        # Loop through SSPs
        for (ssp in ssps) {
          
          # SSP-specific population
          pop_col <- paste0("pop_2050_", ssp)
          
          # SSP-specific deaths columns
          deaths_high_col <- paste0("deaths_HIGH_", ssp)
          deaths_low_col  <- paste0("deaths_LOW_",  ssp)
          
          # PM columns (same across SSPs)
          # Weighted by SSP population
          w_PM_HIGH <- weighted_average(filtered, "PM25_HIGH", pop_col)
          w_PM_LOW  <- weighted_average(filtered, "PM25_LOW",  pop_col)
          
          # Deaths (SSP-specific)
          w_death_HIGH <- weighted_average(filtered, deaths_high_col, pop_col)
          w_death_LOW  <- weighted_average(filtered, deaths_low_col,  pop_col)
          ...
          results <- rbind(
            ...
            data.frame(
              ...
              PM25_HIGH = w_PM_HIGH,
              PM25_LOW  = w_PM_LOW,
              deaths_HIGH = w_death_HIGH,
              deaths_LOW  = w_death_LOW,
              region = city_name
            )
          )
        }
    ...
    calculate_weighted_average_bins_white <- function(city_name, city_data) {
      ...
      for (range in percentage_ranges) {
        ...
        # Filter data by percent_white bin
        filtered <- city_data %>%
          filter(percent_white >= range$min & percent_white <= range$max)
        
        # Loop through SSP1–SSP5
        for (ssp in ssps) {
          
          # SSP-specific population weight
          pop_col <- paste0("pop_2050_", ssp)
          ...
          # --- PM2.5 weighted by SSP population ---
          w_PM_HIGH <- weighted_average(filtered, "PM25_HIGH", pop_col)
          w_PM_LOW  <- weighted_average(filtered, "PM25_LOW",  pop_col)
          
          # --- Mortality weighted by SSP population ---
          w_deaths_HIGH <- weighted_average(filtered, deaths_high_col, pop_col)
          w_deaths_LOW  <- weighted_average(filtered, deaths_low_col,  pop_col)
          ...
          results <- rbind(
            ...
            data.frame(
              ...
              PM25_HIGH = w_PM_HIGH,
              PM25_LOW  = w_PM_LOW,
              deaths_HIGH = w_deaths_HIGH,
              deaths_LOW  = w_deaths_LOW,
              region = city_name
            )
          )
        }
    ...
    #4. city analysis
    city_analysis <- function(city_name, ssps = paste0("SSP", 1:5)) {
      table_name <- paste0("partitions_", city_name, "_complete")
      if (!exists(table_name, envir = .GlobalEnv)) {
        stop("Table ", table_name, " not found in the global environment.")
      }
      table <- get(table_name, envir = .GlobalEnv)
      
      pm_variables <- c("PM25_HIGH", "PM25_LOW")
      demos <- c("Total", "White", "Black", "Hispanic", "Other")
      
      # helper to safely compute weighted average if columns exist
      safe_weighted <- function(tbl, value_col, weight_col) {
        if (! (value_col %in% names(tbl)) ) return(NA_real_)
        if (! (weight_col %in% names(tbl)) ) return(NA_real_)
        weighted_average(tbl, value_col, weight_col)
      }
      
      results_list <- list()
      
      for (ssp in ssps) {
        pop_col_total <- paste0("pop_2050_", ssp)
        pop_cols_demo <- setNames(paste0(c("White", "Black", "Hispanic", "Other"), "_pop_2050_", ssp),
                                  c("White", "Black", "Hispanic", "Other"))
        mortality_cols <- c(
          "deaths_HIGH" = paste0("deaths_HIGH_", ssp),
          "deaths_LOW"  = paste0("deaths_LOW_", ssp)
        )
        ...
        res_ssp <- data.frame(
          SSP = ssp,
          demo = demos,
          PM25_HIGH = rep(NA_real_, length(demos)),
          PM25_LOW  = rep(NA_real_, length(demos)),
          deaths_HIGH = rep(NA_real_, length(demos)),
          deaths_LOW  = rep(NA_real_, length(demos)),
          stringsAsFactors = FALSE
        )
        
        for (i in seq_along(demos)) {
          demo <- demos[i]
          pop_col <- if (demo == "Total") pop_col_total else pop_cols_demo[demo]
          
          # PM25 (not SSP-specific values) weighted by SSP-population
          res_ssp$PM25_HIGH[i] <- safe_weighted(table, "PM25_HIGH", pop_col)
          res_ssp$PM25_LOW[i]  <- safe_weighted(table, "PM25_LOW",  pop_col)
          
          # deaths: use SSP-specific deaths columns if present
          res_ssp$deaths_HIGH[i] <- safe_weighted(table, mortality_cols["deaths_HIGH"], pop_col)
          res_ssp$deaths_LOW[i]  <- safe_weighted(table, mortality_cols["deaths_LOW"],  pop_col)
        }
        
        results_list[[ssp]] <- res_ssp
      }
      
      final_results <- bind_rows(results_list)
      
      # save to global env with a descriptive name
      result_name <- paste0(city_name, "_weighted_analysis")
      assign(result_name, final_results, envir = .GlobalEnv)
...
#### 2. LOS ANGELES 1km ####
    ...
    #Mortality from SSPs
    mortality_HIGH_SSP1 <- st_read("BenMAP_results/1km_V2/LA/2050_pop_sens/highVSzero_mortality_2050_pop_sens_SSP1/Health Impacts-Pope 2019_20251124.shp") %>% st_drop_geometry() %>% dplyr::select(ROW, COL, Point.Estim, Population) %>% rename(row = ROW, col = COL, benmap_population_SSP1 = Population, deaths_HIGH_SSP1 = Point.Estim) 
    
    mortality_HIGH_SSP2 <- st_read("BenMAP_results/1km_V2/LA/2050_pop_sens/highVSzero_mortality_2050_pop_sens_SSP2/Health Impacts-Pope 2019_20251124.shp") %>% st_drop_geometry() %>% dplyr::select(ROW, COL, Point.Estim, Population) %>% rename(row = ROW, col = COL, benmap_population_SSP2 = Population, deaths_HIGH_SSP2 = Point.Estim) 
    
    mortality_HIGH_SSP3 <- st_read("BenMAP_results/1km_V2/LA/2050_pop_sens/highVSzero_mortality_2050_pop_sens_SSP3/Health Impacts-Pope 2019_20251124.shp") %>% st_drop_geometry() %>% dplyr::select(ROW, COL, Point.Estim, Population) %>% rename(row = ROW, col = COL, benmap_population_SSP3 = Population, deaths_HIGH_SSP3 = Point.Estim) 
    
    mortality_HIGH_SSP4 <- st_read("BenMAP_results/1km_V2/LA/2050_pop_sens/highVSzero_mortality_2050_pop_sens_SSP4/Health Impacts-Pope 2019_20251124.shp") %>% st_drop_geometry() %>% dplyr::select(ROW, COL, Point.Estim, Population) %>% rename(row = ROW, col = COL, benmap_population_SSP4 = Population, deaths_HIGH_SSP4 = Point.Estim) 
    
    mortality_HIGH_SSP5 <- st_read("BenMAP_results/1km_V2/LA/2050_pop_sens/highVSzero_mortality_2050_pop_sens_SSP5/Health Impacts-Pope 2019_20251124.shp") %>% st_drop_geometry() %>% dplyr::select(ROW, COL, Point.Estim, Population) %>% rename(row = ROW, col = COL, benmap_population_SSP5 = Population, deaths_HIGH_SSP5 = Point.Estim) 
    
    mortality_LOW_SSP1 <- st_read("BenMAP_results/1km_V2/LA/2050_pop_sens/lowVSzero_mortality_2050_pop_sens_SSP1/Health Impacts-Pope 2019_20251124.shp") %>% st_drop_geometry() %>% dplyr::select(ROW, COL, Point.Estim) %>% rename(row = ROW, col = COL, deaths_LOW_SSP1 = Point.Estim) 
    
    mortality_LOW_SSP2 <- st_read("BenMAP_results/1km_V2/LA/2050_pop_sens/lowVSzero_mortality_2050_pop_sens_SSP2/Health Impacts-Pope 2019_20251124.shp") %>% st_drop_geometry() %>% dplyr::select(ROW, COL, Point.Estim) %>% rename(row = ROW, col = COL, deaths_LOW_SSP2 = Point.Estim) 
    
    mortality_LOW_SSP3 <- st_read("BenMAP_results/1km_V2/LA/2050_pop_sens/lowVSzero_mortality_2050_pop_sens_SSP3/Health Impacts-Pope 2019_20251124.shp") %>% st_drop_geometry() %>% dplyr::select(ROW, COL, Point.Estim) %>% rename(row = ROW, col = COL, deaths_LOW_SSP3 = Point.Estim) 
    
    mortality_LOW_SSP4 <- st_read("BenMAP_results/1km_V2/LA/2050_pop_sens/lowVSzero_mortality_2050_pop_sens_SSP4/Health Impacts-Pope 2019_20251124.shp") %>% st_drop_geometry() %>% dplyr::select(ROW, COL, Point.Estim, ) %>% rename(row = ROW, col = COL, deaths_LOW_SSP4 = Point.Estim) 
    
    mortality_LOW_SSP5 <- st_read("BenMAP_results/1km_V2/LA/2050_pop_sens/lowVSzero_mortality_2050_pop_sens_SSP5/Health Impacts-Pope 2019_20251124.shp") %>% st_drop_geometry() %>% dplyr::select(ROW, COL, Point.Estim, ) %>% rename(row = ROW, col = COL,  deaths_LOW_SSP5 = Point.Estim) 
    ...
    partitions_LA_race %>% #race
      ...
      left_join(mortality_HIGH_SSP1, by = c("row", "col")) %>% #Mortality 
      left_join(mortality_HIGH_SSP2, by = c("row", "col")) %>% #Mortality 
      left_join(mortality_HIGH_SSP3, by = c("row", "col")) %>% #Mortality 
      left_join(mortality_HIGH_SSP4, by = c("row", "col")) %>% #Mortality 
      left_join(mortality_HIGH_SSP5, by = c("row", "col")) %>% #Mortality 
      left_join(mortality_LOW_SSP1, by = c("row", "col")) %>% #Mortality 
      left_join(mortality_LOW_SSP2, by = c("row", "col")) %>% #Mortality 
      left_join(mortality_LOW_SSP3, by = c("row", "col")) %>% #Mortality 
      left_join(mortality_LOW_SSP4, by = c("row", "col")) %>% #Mortality 
      left_join(mortality_LOW_SSP5, by = c("row", "col")) %>% #Mortality 
      ...
      mutate(across(c("Population", "White", "Black", "Hispanic", "Other", 
                      ...
                      "deaths_2019", "deaths_HIGH","deaths_LOW" ,"deaths_HIGH_SSP1", "deaths_HIGH_SSP2", 
                      "deaths_HIGH_SSP3", "deaths_HIGH_SSP4", "deaths_HIGH_SSP5","deaths_LOW_SSP1", "deaths_LOW_SSP2",
                      "deaths_LOW_SSP3", "deaths_LOW_SSP4", "deaths_LOW_SSP5", "benmap_population_SSP1",
                      "benmap_population_SSP2", "benmap_population_SSP3", "benmap_population_SSP4", "benmap_population_SSP5"), 
                    ~ replace_na(., 0))) %>%
      #Here we filter out pixels with NA in pollution, which refer to some bordering pixels
      filter_all(all_vars(!is.na(.)))-> partitions_LA_complete_pre
    
    death_totals <- partitions_LA_complete_pre %>%
      dplyr::select(starts_with("deaths")) %>%  # keep only death columns
      summarise(across(everything(), ~ round(sum(.x, na.rm = TRUE)))) %>%
      pivot_longer(everything(),
                   names_to = "death_category",
                   values_to = "total_deaths")
    
    #Prepare each SSP table
    pop_LA_long %>%
      st_drop_geometry() %>%
      dplyr::select(-Year) %>%
      spread(Variable, Population)->LA_SSP_population
    
    demos <- c("White", "Black", "Hispanic", "Other")
    
    partitions_LA_complete <- partitions_LA_complete_pre %>%
      left_join(LA_SSP_population,
                by = c("pixel_ID", "row", "col")) %>%
      mutate(
        across(starts_with("pop_2050_SSP"),
               ~ .x * percent_white / 100,
               .names = "White_{.col}"),
        across(starts_with("pop_2050_SSP"),
               ~ .x * percent_black / 100,
    ...
    #### 2.3. Apply population weighted equations ####
    ...
    #### 2.4. Figures ####
      ...
      p<-ggplot(bin_white_graph, aes(x = `0to30white`, y = `61to100white`,   color = scenario)) + #label = SSP,
        geom_point() +  # Add points for each city
        # geom_text(hjust = 0, vjust = 0) +  # Add city labels
        geom_line(aes(group = SSP), color = "grey") +  # Connect the dots by city
        facet_wrap(.~ SSP) +
      ...
      ggsave("figures/BenMAP/sensitivity/LA/mortality_V2/1km_scatter_PM25_race_sensitivity.png", p, width = 16, height = 12, units = "in", dpi = 600)
      ggsave("figures/BenMAP/sensitivity/LA/mortality_V2/1km_scatter_PM25_race_sensitivity.svg", p, width = 16, height = 12, units = "in", dpi = 600)
      
      ### Deaths and race ####
      bin_white_filtered_high <- LA_white %>% #HIGH CDR
        filter(bin != "31to60white") %>%
        # Select the columns you want to spread
        dplyr::select(bin, deaths_HIGH, SSP) %>%
        # Spread the values across columns based on the 'bin' column
      ...
      bin_white_filtered_low <- LA_white %>% #HIGH CDR
        filter(bin != "31to60white") %>%
        # Select the columns you want to spread
        dplyr::select(bin, deaths_LOW, SSP) %>%
        # Spread the values across columns based on the 'bin' column
      ...
      # Create the scatterplot
      p<-ggplot(bin_white_graph, aes(x = `0to30white`, y = `61to100white`,color = scenario, label = SSP,shape = scenario)) +
        geom_point() +  # Add points for each city #label = SSP
        geom_line(aes(group = SSP), color = "grey") +  # Connect the dots by city
        # facet_wrap(.~SSP)+
        geom_text(hjust = 0, vjust = 0) +  # Add city labels
        geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "black") +  # Add the 1:1 line
      ...
      ggsave("figures/BenMAP/sensitivity/LA/mortality_V2/1km_scatter_mortality_race_sensitivity.png", p, width = 16, height = 12, units = "in", dpi = 600)    
      ggsave("figures/BenMAP/sensitivity/LA/mortality_V2/1km_scatter_mortality_race_sensitivity.svg", p, width = 16, height = 12, units = "in", dpi = 600)    
      
      ### PM2.5 and income ####
      bin_white_filtered_high <- LA_income %>% #HIGH CDR
      ...
      bin_white_filtered_low <- LA_income %>% #HIGH CDR
        filter(bin != "33to66percentile") %>%
        # Select the columns you want to spread
        dplyr::select(bin, PM25_LOW, SSP) %>%
        # Spread the values across columns based on the 'bin' column
      ...
      # Create the scatterplot
      p<-ggplot(bin_white_graph, aes(x = `0to33percentile`, y = `66to100percentile`,   color = scenario)) + #label = SSP,
        geom_point() +  
        geom_line(aes(group = SSP), color = "grey") +  # Connect the dots by city
        facet_wrap(.~ SSP) +
        geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "black") +  # Add the 1:1 line
        labs(x = "Population-weighted PM2.5 for 0-33rd income percentile",
             y = "Population-weighted PM2.5 for 66-100th income percentile",
             title = "Scatterplot of Population-weighted PM2.5")
      ggsave("figures/BenMAP/sensitivity/LA/mortality_V2/1km_scatter_PM25_income_sensitivity.png", p, width = 16, height = 12, units = "in", dpi = 600)
      ggsave("figures/BenMAP/sensitivity/LA/mortality_V2/1km_scatter_PM25_income_sensitivity.svg", p, width = 16, height = 12, units = "in", dpi = 600)
      ...
      # Create the scatterplot
      p<-ggplot(bin_white_graph, aes(x = `0to33percentile`, y = `66to100percentile`,color = scenario, shape = scenario)) +
        geom_point() +  # Add points for each city
        geom_line(aes(group = SSP), color = "grey") +  # Connect the dots by city
        facet_wrap(.~SSP)+
        geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "black") +  # Add the 1:1 line
        labs(x = "Population-weighted mortality for 0-33rd income percentile",
             y = "Population-weighted mortality for 66-100income percentile",
             title = "Scatterplot of Population-weighted deaths")
      ggsave("figures/BenMAP/sensitivity/LA/mortality_V2/1km_scatter_mortality_income_sensitivity.png", p, width = 16, height = 12, units = "in", dpi = 600)    
      ggsave("figures/BenMAP/sensitivity/LA/mortality_V2/1km_scatter_mortality_income_sensitivity.svg", p, width = 16, height = 12, units = "in", dpi = 600)    
...
#### 11. PHILADELPHIA 1km ####
    ...
    #Mortality from SSPs
    mortality_HIGH_SSP1 <- st_read("BenMAP_results/1km_V2/Philadelphia/2050_pop_sens/highVSzero_mortality_2050_pop_sens_SSP1/Health Impacts-Pope 2019_20251124.shp") %>% st_drop_geometry() %>% dplyr::select(ROW, COL, Point.Estim, Population) %>% rename(row = ROW, col = COL, benmap_population_SSP1 = Population, deaths_HIGH_SSP1 = Point.Estim) 
    
    mortality_HIGH_SSP2 <- st_read("BenMAP_results/1km_V2/Philadelphia/2050_pop_sens/highVSzero_mortality_2050_pop_sens_SSP2/Health Impacts-Pope 2019_20251124.shp") %>% st_drop_geometry() %>% dplyr::select(ROW, COL, Point.Estim, Population) %>% rename(row = ROW, col = COL, benmap_population_SSP2 = Population, deaths_HIGH_SSP2 = Point.Estim) 
    
    mortality_HIGH_SSP3 <- st_read("BenMAP_results/1km_V2/Philadelphia/2050_pop_sens/highVSzero_mortality_2050_pop_sens_SSP3/Health Impacts-Pope 2019_20251124.shp") %>% st_drop_geometry() %>% dplyr::select(ROW, COL, Point.Estim, Population) %>% rename(row = ROW, col = COL, benmap_population_SSP3 = Population, deaths_HIGH_SSP3 = Point.Estim) 
    
    mortality_HIGH_SSP4 <- st_read("BenMAP_results/1km_V2/Philadelphia/2050_pop_sens/highVSzero_mortality_2050_pop_sens_SSP4/Health Impacts-Pope 2019_20251124.shp") %>% st_drop_geometry() %>% dplyr::select(ROW, COL, Point.Estim, Population) %>% rename(row = ROW, col = COL, benmap_population_SSP4 = Population, deaths_HIGH_SSP4 = Point.Estim) 
    
    mortality_HIGH_SSP5 <- st_read("BenMAP_results/1km_V2/Philadelphia/2050_pop_sens/highVSzero_mortality_2050_pop_sens_SSP5/Health Impacts-Pope 2019_20251124.shp") %>% st_drop_geometry() %>% dplyr::select(ROW, COL, Point.Estim, Population) %>% rename(row = ROW, col = COL, benmap_population_SSP5 = Population, deaths_HIGH_SSP5 = Point.Estim) 
    
    mortality_LOW_SSP1 <- st_read("BenMAP_results/1km_V2/Philadelphia/2050_pop_sens/lowVSzero_mortality_2050_pop_sens_SSP1/Health Impacts-Pope 2019_20251124.shp") %>% st_drop_geometry() %>% dplyr::select(ROW, COL, Point.Estim) %>% rename(row = ROW, col = COL, deaths_LOW_SSP1 = Point.Estim) 
    
    mortality_LOW_SSP2 <- st_read("BenMAP_results/1km_V2/Philadelphia/2050_pop_sens/lowVSzero_mortality_2050_pop_sens_SSP2/Health Impacts-Pope 2019_20251124.shp") %>% st_drop_geometry() %>% dplyr::select(ROW, COL, Point.Estim) %>% rename(row = ROW, col = COL, deaths_LOW_SSP2 = Point.Estim) 
    
    mortality_LOW_SSP3 <- st_read("BenMAP_results/1km_V2/Philadelphia/2050_pop_sens/lowVSzero_mortality_2050_pop_sens_SSP3/Health Impacts-Pope 2019_20251124.shp") %>% st_drop_geometry() %>% dplyr::select(ROW, COL, Point.Estim) %>% rename(row = ROW, col = COL, deaths_LOW_SSP3 = Point.Estim) 
    
    mortality_LOW_SSP4 <- st_read("BenMAP_results/1km_V2/Philadelphia/2050_pop_sens/lowVSzero_mortality_2050_pop_sens_SSP4/Health Impacts-Pope 2019_20251124.shp") %>% st_drop_geometry() %>% dplyr::select(ROW, COL, Point.Estim, ) %>% rename(row = ROW, col = COL, deaths_LOW_SSP4 = Point.Estim) 
    
    mortality_LOW_SSP5 <- st_read("BenMAP_results/1km_V2/Philadelphia/2050_pop_sens/lowVSzero_mortality_2050_pop_sens_SSP5/Health Impacts-Pope 2019_20251124.shp") %>% st_drop_geometry() %>% dplyr::select(ROW, COL, Point.Estim, ) %>% rename(row = ROW, col = COL,  deaths_LOW_SSP5 = Point.Estim) 
    ...
    # Check downscaling
    check_population_sum(partitions_phil_race)
    ...
    #Join all
    partitions_phil_race %>% #race
      left_join(partitions_phil_income, by = c("pixel_ID", "row", "col")) %>% #income
      ...
      mutate(across(c("Population", "White", "Black", "Hispanic", "Other", 
                    ...
                    ~ replace_na(., 0))) %>%
      #Here we filter out pixels with NA in pollution, which refer to some bordering pixels
      filter_all(all_vars(!is.na(.)))-> partitions_phil_complete_pre
    
    death_totals <- partitions_phil_complete_pre %>%
      dplyr::select(starts_with("deaths")) %>%  # keep only death columns
      summarise(across(everything(), ~ round(sum(.x, na.rm = TRUE)))) %>%
      pivot_longer(everything(),
                   names_to = "death_category",
    ...
    pop_phil_long %>%
      st_drop_geometry() %>%
      dplyr::select(-Year) %>%
      spread(Variable, Population)->phil_SSP_population
    
    demos <- c("White", "Black", "Hispanic", "Other")
    
    partitions_Philadelphia_complete <- partitions_phil_complete_pre %>%
      left_join(phil_SSP_population,
                by = c("pixel_ID", "row", "col")) %>%
      mutate(
        across(starts_with("pop_2050_SSP"),
               ~ .x * percent_white / 100,
    ...
    #### 11.3. Apply population weighted equations ####
    phil_weighted_analysis <- city_analysis("Philadelphia")
    phil_white <- calculate_weighted_average_bins_white("Philadelphia", partitions_Philadelphia_complete)
    phil_income <- calculate_weighted_average_bins_income("Philadelphia", partitions_Philadelphia_complete)
    
    #### 11.4. Figures ####
    ### PM2.5 and race ####
    bin_white_filtered_high <- phil_white %>% #HIGH CDR
      filter(bin != "31to60white") %>%
      # Select the columns you want to spread
      dplyr::select(bin, PM25_HIGH, SSP) %>%
      # Spread the values across columns based on the 'bin' column
    ...
    bin_white_filtered_low <- phil_white %>% #HIGH CDR
      filter(bin != "31to60white") %>%
      # Select the columns you want to spread
      dplyr::select(bin, PM25_LOW, SSP) %>%
      # Spread the values across columns based on the 'bin' column
    ...
    p<-ggplot(bin_white_graph, aes(x = `0to30white`, y = `61to100white`,   color = scenario)) + #label = SSP,
      ...
      labs(x = "Population-weighted PM2.5 for 0-30% white",
           y = "Population-weighted PM2.5 for 61-100% white",
           title = "Scatterplot of Population-weighted PM2.5")
    ggsave("figures/BenMAP/sensitivity/Philadelphia/mortality_V2/1km_scatter_PM25_race_sensitivity.png", p, width = 16, height = 12, units = "in", dpi = 600)
    ggsave("figures/BenMAP/sensitivity/Philadelphia/mortality_V2/1km_scatter_PM25_race_sensitivity.svg", p, width = 16, height = 12, units = "in", dpi = 600)
    ...
    p<-ggplot(bin_white_graph, aes(x = `0to30white`, y = `61to100white`,color = scenario, label = SSP,shape = scenario)) +
      ...
      labs(x = "Population-weighted deaths for 0-30% white",
           y = "Population-weighted deaths for 61-100% white",
           title = "Scatterplot of Population-weighted deaths")
    ggsave("figures/BenMAP/sensitivity/Philadelphia/mortality_V2/1km_scatter_mortality_race_sensitivity.png", p, width = 16, height = 12, units = "in", dpi = 600)    
    ggsave("figures/BenMAP/sensitivity/Philadelphia/mortality_V2/1km_scatter_mortality_race_sensitivity.svg", p, width = 16, height = 12, units = "in", dpi = 600)    
    
    ### PM2.5 and income ####
    bin_white_filtered_high <- phil_income %>% #HIGH CDR
      filter(bin != "33to66percentile") %>%
      # Select the columns you want to spread
      dplyr::select(bin, PM25_HIGH, SSP) %>%
      # Spread the values across columns based on the 'bin' column
    ...
    bin_white_filtered_low <- phil_income %>% #HIGH CDR
      filter(bin != "33to66percentile") %>%
      # Select the columns you want to spread
      dplyr::select(bin, PM25_LOW, SSP) %>%
      # Spread the values across columns based on the 'bin' column
    ...
    p<-ggplot(bin_white_graph, aes(x = `0to33percentile`, y = `66to100percentile`,   color = scenario)) + #label = SSP,
      ...
      labs(x = "Population-weighted PM2.5 for 0-33rd income percentile",
           y = "Population-weighted PM2.5 for 66-100th income percentile",
           title = "Scatterplot of Population-weighted PM2.5")
    ggsave("figures/BenMAP/sensitivity/Philadelphia/mortality_V2/1km_scatter_PM25_income_sensitivity.png", p, width = 16, height = 12, units = "in", dpi = 600)
    ggsave("figures/BenMAP/sensitivity/Philadelphia/mortality_V2/1km_scatter_PM25_income_sensitivity.svg", p, width = 16, height = 12, units = "in", dpi = 600)
    ...
    p<-ggplot(bin_white_graph, aes(x = `0to33percentile`, y = `66to100percentile`,color = scenario, shape = scenario)) +
      ...
      labs(x = "Population-weighted mortality for 0-33rd income percentile",
           y = "Population-weighted mortality for 66-100income percentile",
           title = "Scatterplot of Population-weighted deaths")
    ggsave("figures/BenMAP/sensitivity/Philadelphia/mortality_V2/1km_scatter_mortality_income_sensitivity.png", p, width = 16, height = 12, units = "in", dpi = 600)    
    ggsave("figures/BenMAP/sensitivity/Philadelphia/mortality_V2/1km_scatter_mortality_income_sensitivity.svg", p, width = 16, height = 12, units = "in", dpi = 600)    
...
#### 14. DETROIT 1km ####
    ...
    #Mortality from SSPs
    mortality_HIGH_SSP1 <- st_read("BenMAP_results/1km_V2/Detroit/2050_pop_sens/highVSzero_mortality_2050_pop_sens_SSP1/Health Impacts-Pope 2019_20251124.shp") %>% st_drop_geometry() %>% dplyr::select(ROW, COL, Point.Estim, Population) %>% rename(row = ROW, col = COL, benmap_population_SSP1 = Population, deaths_HIGH_SSP1 = Point.Estim) 
    
    mortality_HIGH_SSP2 <- st_read("BenMAP_results/1km_V2/Detroit/2050_pop_sens/highVSzero_mortality_2050_pop_sens_SSP2/Health Impacts-Pope 2019_20251124.shp") %>% st_drop_geometry() %>% dplyr::select(ROW, COL, Point.Estim, Population) %>% rename(row = ROW, col = COL, benmap_population_SSP2 = Population, deaths_HIGH_SSP2 = Point.Estim) 
    
    mortality_HIGH_SSP3 <- st_read("BenMAP_results/1km_V2/Detroit/2050_pop_sens/highVSzero_mortality_2050_pop_sens_SSP3/Health Impacts-Pope 2019_20251124.shp") %>% st_drop_geometry() %>% dplyr::select(ROW, COL, Point.Estim, Population) %>% rename(row = ROW, col = COL, benmap_population_SSP3 = Population, deaths_HIGH_SSP3 = Point.Estim) 
    
    mortality_HIGH_SSP4 <- st_read("BenMAP_results/1km_V2/Detroit/2050_pop_sens/highVSzero_mortality_2050_pop_sens_SSP4/Health Impacts-Pope 2019_20251124.shp") %>% st_drop_geometry() %>% dplyr::select(ROW, COL, Point.Estim, Population) %>% rename(row = ROW, col = COL, benmap_population_SSP4 = Population, deaths_HIGH_SSP4 = Point.Estim) 
    
    mortality_HIGH_SSP5 <- st_read("BenMAP_results/1km_V2/Detroit/2050_pop_sens/highVSzero_mortality_2050_pop_sens_SSP5/Health Impacts-Pope 2019_20251124.shp") %>% st_drop_geometry() %>% dplyr::select(ROW, COL, Point.Estim, Population) %>% rename(row = ROW, col = COL, benmap_population_SSP5 = Population, deaths_HIGH_SSP5 = Point.Estim) 
    
    mortality_LOW_SSP1 <- st_read("BenMAP_results/1km_V2/Detroit/2050_pop_sens/lowVSzero_mortality_2050_pop_sens_SSP1/Health Impacts-Pope 2019_20251124.shp") %>% st_drop_geometry() %>% dplyr::select(ROW, COL, Point.Estim) %>% rename(row = ROW, col = COL, deaths_LOW_SSP1 = Point.Estim) 
    
    mortality_LOW_SSP2 <- st_read("BenMAP_results/1km_V2/Detroit/2050_pop_sens/lowVSzero_mortality_2050_pop_sens_SSP2/Health Impacts-Pope 2019_20251124.shp") %>% st_drop_geometry() %>% dplyr::select(ROW, COL, Point.Estim) %>% rename(row = ROW, col = COL, deaths_LOW_SSP2 = Point.Estim) 
    
    mortality_LOW_SSP3 <- st_read("BenMAP_results/1km_V2/Detroit/2050_pop_sens/lowVSzero_mortality_2050_pop_sens_SSP3/Health Impacts-Pope 2019_20251124.shp") %>% st_drop_geometry() %>% dplyr::select(ROW, COL, Point.Estim) %>% rename(row = ROW, col = COL, deaths_LOW_SSP3 = Point.Estim) 
    
    mortality_LOW_SSP4 <- st_read("BenMAP_results/1km_V2/Detroit/2050_pop_sens/lowVSzero_mortality_2050_pop_sens_SSP4/Health Impacts-Pope 2019_20251124.shp") %>% st_drop_geometry() %>% dplyr::select(ROW, COL, Point.Estim, ) %>% rename(row = ROW, col = COL, deaths_LOW_SSP4 = Point.Estim) 
    
    mortality_LOW_SSP5 <- st_read("BenMAP_results/1km_V2/Detroit/2050_pop_sens/lowVSzero_mortality_2050_pop_sens_SSP5/Health Impacts-Pope 2019_20251124.shp") %>% st_drop_geometry() %>% dplyr::select(ROW, COL, Point.Estim, ) %>% rename(row = ROW, col = COL,  deaths_LOW_SSP5 = Point.Estim) 
    ...
    partitions_detroit_race %>% #race
      ...
      mutate(across(c("Population", "White", "Black", "Hispanic", "Other", 
                    ...
                    ~ replace_na(., 0))) %>%
      #Here we filter out pixels with NA in pollution, which refer to some bordering pixels
      filter_all(all_vars(!is.na(.)))-> partitions_detroit_complete_pre
    
    death_totals <- partitions_detroit_complete_pre %>%
      dplyr::select(starts_with("deaths")) %>%  # keep only death columns
      summarise(across(everything(), ~ round(sum(.x, na.rm = TRUE)))) %>%
      pivot_longer(everything(),
                   names_to = "death_category",
    ...
    pop_detroit_long %>%
      st_drop_geometry() %>%
      dplyr::select(-Year) %>%
      spread(Variable, Population)->detroit_SSP_population
    
    demos <- c("White", "Black", "Hispanic", "Other")
    
    partitions_Detroit_complete <- partitions_detroit_complete_pre %>%
      left_join(detroit_SSP_population,
                by = c("pixel_ID", "row", "col")) %>%
      mutate(
        across(starts_with("pop_2050_SSP"),
               ~ .x * percent_white / 100,
    ...
    #### 14.3. Apply population weighted equations ####
    ...
    #### 14.4. Figures ####
    ### PM2.5 and race ####
    bin_white_filtered_high <- detroit_white %>% #HIGH CDR
      filter(bin != "31to60white") %>%
      # Select the columns you want to spread
      dplyr::select(bin, PM25_HIGH, SSP) %>%
      # Spread the values across columns based on the 'bin' column
    ...
    bin_white_filtered_low <- detroit_white %>% #HIGH CDR
      filter(bin != "31to60white") %>%
      # Select the columns you want to spread
      dplyr::select(bin, PM25_LOW, SSP) %>%
      # Spread the values across columns based on the 'bin' column
    ...
    p<-ggplot(bin_white_graph, aes(x = `0to30white`, y = `61to100white`,   color = scenario)) + #label = SSP,
      ...
      labs(x = "Population-weighted PM2.5 for 0-30% white",
           y = "Population-weighted PM2.5 for 61-100% white",
           title = "Scatterplot of Population-weighted PM2.5")
    ggsave("figures/BenMAP/sensitivity/Detroit/mortality_V2/1km_scatter_PM25_race_sensitivity.png", p, width = 16, height = 12, units = "in", dpi = 600)
    ggsave("figures/BenMAP/sensitivity/Detroit/mortality_V2/1km_scatter_PM25_race_sensitivity.svg", p, width = 16, height = 12, units = "in", dpi = 600)
    ...
    p<-ggplot(bin_white_graph, aes(x = `0to30white`, y = `61to100white`,color = scenario, label = SSP,shape = scenario)) +
      ...
      labs(x = "Population-weighted deaths for 0-30% white",
           y = "Population-weighted deaths for 61-100% white",
           title = "Scatterplot of Population-weighted deaths")
    ggsave("figures/BenMAP/sensitivity/Detroit/mortality_V2/1km_scatter_mortality_race_sensitivity.png", p, width = 16, height = 12, units = "in", dpi = 600)    
    ggsave("figures/BenMAP/sensitivity/Detroit/mortality_V2/1km_scatter_mortality_race_sensitivity.svg", p, width = 16, height = 12, units = "in", dpi = 600)    
    ...
    p<-ggplot(bin_white_graph, aes(x = `0to33percentile`, y = `66to100percentile`,   color = scenario)) + #label = SSP,
      ...
      labs(x = "Population-weighted PM2.5 for 0-33rd income percentile",
           y = "Population-weighted PM2.5 for 66-100th income percentile",
           title = "Scatterplot of Population-weighted PM2.5")
    ggsave("figures/BenMAP/sensitivity/Detroit/mortality_V2/1km_scatter_PM25_income_sensitivity.png", p, width = 16, height = 12, units = "in", dpi = 600)
    ggsave("figures/BenMAP/sensitivity/Detroit/mortality_V2/1km_scatter_PM25_income_sensitivity.svg", p, width = 16, height = 12, units = "in", dpi = 600)
    ...
    p<-ggplot(bin_white_graph, aes(x = `0to33percentile`, y = `66to100percentile`,color = scenario, shape = scenario)) +
      ...
      labs(x = "Population-weighted mortality for 0-33rd income percentile",
           y = "Population-weighted mortality for 66-100income percentile",
           title = "Scatterplot of Population-weighted deaths")
    ggsave("figures/BenMAP/sensitivity/Detroit/mortality_V2/1km_scatter_mortality_income_sensitivity.png", p, width = 16, height = 12, units = "in", dpi = 600)    
    ggsave("figures/BenMAP/sensitivity/Detroit/mortality_V2/1km_scatter_mortality_income_sensitivity.svg", p, width = 16, height = 12, units = "in", dpi = 600)    
    
    
#------------------------------------------------- BASELINE INCIDENCE SENSITIVITY ---------------------------------------------------------------------------      
#Here we want to test the hypothesis that a wealthier future society would increase its adaptive capacity to face negative impacts of pollution. 
    #To do so, we reduce the incidence rate in Philadelphia by 10% in each pixel, which could be supported by papers cite din the manuscript
    #We anticipate that overall mortality will decrease, but inequalities would remain 
    
    #We add two more sensitivities varying the incidence rate in a non-uniform way
      #SSP1 Sustainability-Taking the green road: this storyline assumes sustainable development and reduced inequalities across countries. We could extrapolate this to cities, and only modify the baseline incidence mortality in lower-income pixels, and then do the same in predominantly minority pixels (or even have 3 tiers of reductions). Then run BenMAP and compare how inequality changes. 
      #SSP4 Inequality-A Road Divided: this storyline assumes a policy focused on elites, high inequality, low economic growth... Here, we could assume that only the top decrease their baseline mortality, while the rest remain the same.
    
  #### CREATE BENMAP INPUT FILES ####
    #### Adjust incidence rate files ####
    original_incidence_Phil <- read.csv("output_data/BenMAP_files/1km/Philadelphia/Date_ 2024-04-08 Incidence_1km_2019_phil_fixed.csv")
    
    #Partitions
    partitions_phil <- read.csv("output_data/Partitions/partitions_phil.csv") %>% dplyr::select(-X)
    
    #Race (note we load census data from above)
    partitions_phil %>%
      left_join(ethnicity_data, by = "census_block_ID") %>%
      mutate(Population = total_population * area_ratio_block,
             Black = nonhisp_black * area_ratio_block,
    ...
    partitions_phil_race %>%
      select(pixel_ID, row, col, percent_white) -> partitions_phil_race_percent_nonwhite
    ...
    #Now merge original incidence with income and race
    original_incidence_Phil %>%
      left_join(partitions_phil_income, by = c("Row" = "row", "Column" = "col")) %>%
      left_join(partitions_phil_race_percent_nonwhite, by = c("Row" = "row", "Column" = "col", "pixel_ID"))-> original_incidence_Phil_complete
    
    #### Generate tables with updated incidence ####
    #Income based 
      #Cut-off points for the whole city are: 15,250 – 82,732 for 0-33rd percentile; 105,267 – 250,000 for 66th and above
      #SSP1 Sustainability-Taking the green road 
      #SSP4 Inequality-A Road Divided
    original_incidence_Phil_complete %>%
      mutate(incidence_SSP1_income = if_else(weighted_median_income <= 82732, Value * .9, Value), #decrease in lower income areas
             incidence_SSP4_income= if_else(weighted_median_income >= 105267, Value * .9, Value), #decrease in higher income areas
             incidence_SSP1_race = if_else(percent_white <= 30, Value * .9, Value), #decrease in predominantly minority areas
             incidence_SSP4_race= if_else(percent_white >= 60, Value * .9, Value)) %>%
      rename(Value_original = Value) %>%
      mutate(Type = "incidence")-> original_incidence_Phil_complete_SSPs
    
    write.csv(original_incidence_Phil_complete_SSPs %>%
                select(Row, Column, `Endpoint Group` = Endpoint.Group, Endpoint, Race, Gender, Ethnicity, `Start Age` = Start.Age, `End Age` = End.Age, Type, Value = incidence_SSP1_income), 
              "output_data/BenMAP_files/sensitivity_incidence/Incidence_1km_2019_phil_fixed_10percent_lower_SSP1_income.csv", row.names = FALSE)
    
    write.csv(original_incidence_Phil_complete_SSPs %>%
                select(Row, Column, `Endpoint Group` = Endpoint.Group, Endpoint, Race, Gender, Ethnicity, `Start Age` = Start.Age, `End Age` = End.Age, Type, Value = incidence_SSP4_income), 
              "output_data/BenMAP_files/sensitivity_incidence/Incidence_1km_2019_phil_fixed_10percent_lower_SSP4_income.csv", row.names = FALSE)
    
    write.csv(original_incidence_Phil_complete_SSPs %>%
                select(Row, Column, `Endpoint Group` = Endpoint.Group, Endpoint, Race, Gender, Ethnicity, `Start Age` = Start.Age, `End Age` = End.Age, Type, Value = incidence_SSP1_race), 
              "output_data/BenMAP_files/sensitivity_incidence/Incidence_1km_2019_phil_fixed_10percent_lower_SSP1_race.csv", row.names = FALSE)
    
    write.csv(original_incidence_Phil_complete_SSPs %>%
                select(Row, Column, `Endpoint Group` = Endpoint.Group, Endpoint, Race, Gender, Ethnicity, `Start Age` = Start.Age, `End Age` = End.Age, Type, Value = incidence_SSP4_race), 
              "output_data/BenMAP_files/sensitivity_incidence/Incidence_1km_2019_phil_fixed_10percent_lower_SSP4_race.csv", row.names = FALSE)
    ...
    #Load sensitivity BenMAP results 
      #Here we assume 10% reduction across all pixels
      benmap_highVSzero_incidence_phil <- st_read("BenMAP_results/1km_V2/Philadelphia/incidence_sensitivity/highVSzero_mortality_10per_lower_incidence_sens/Health Impacts-Pope 2019_20260304.shp")
      benmap_lowVSzero_incidence_phil <- st_read("BenMAP_results/1km_V2/Philadelphia/incidence_sensitivity/lowVSzero_mortality_10per_lower_incidence_sens/Health Impacts-Pope 2019_20260304.shp")
      
      #Here we assume 10% reduction in lower income areas following a hypothetical SSP1 and SSP4
    benmap_highVSzero_incidence_SSP1_income_phil <- st_read("BenMAP_results/1km_V2/Philadelphia/incidence_sensitivity/highVSzero_mortality_10per_lower_incidence_sens_SSP1_income/Health Impacts-Pope 2019_20260313.shp")
    benmap_highVSzero_incidence_SSP4_income_phil <- st_read("BenMAP_results/1km_V2/Philadelphia/incidence_sensitivity/highVSzero_mortality_10per_lower_incidence_sens_SSP4_income/Health Impacts-Pope 2019_20260313.shp")
    
    benmap_lowVSzero_incidence_SSP1_income_phil <- st_read("BenMAP_results/1km_V2/Philadelphia/incidence_sensitivity/lowVSzero_mortality_10per_lower_incidence_sens_SSP1_income/Health Impacts-Pope 2019_20260313.shp")
    benmap_lowVSzero_incidence_SSP4_income_phil <- st_read("BenMAP_results/1km_V2/Philadelphia/incidence_sensitivity/lowVSzero_mortality_10per_lower_incidence_sens_SSP4_income/Health Impacts-Pope 2019_20260313.shp")
    
    #Here we assume 10% reduction in minority areas following a hypothetical SSP1 and SSP4
    benmap_highVSzero_incidence_SSP1_race_phil <- st_read("BenMAP_results/1km_V2/Philadelphia/incidence_sensitivity/highVSzero_mortality_10per_lower_incidence_sens_SSP1_race/Health Impacts-Pope 2019_20260313.shp")
    benmap_highVSzero_incidence_SSP4_race_phil <- st_read("BenMAP_results/1km_V2/Philadelphia/incidence_sensitivity/highVSzero_mortality_10per_lower_incidence_sens_SSP4_race/Health Impacts-Pope 2019_20260313.shp")
    
    benmap_lowVSzero_incidence_SSP1_race_phil <- st_read("BenMAP_results/1km_V2/Philadelphia/incidence_sensitivity/lowVSzero_mortality_10per_lower_incidence_sens_SSP1_race/Health Impacts-Pope 2019_20260313.shp")
    benmap_lowVSzero_incidence_SSP4_race_phil <- st_read("BenMAP_results/1km_V2/Philadelphia/incidence_sensitivity/lowVSzero_mortality_10per_lower_incidence_sens_SSP4_race/Health Impacts-Pope 2019_20260313.shp")
    ...
    #Process reduction uniform
    benmap_highVSzero_incidence_proc_phil <- process_benmap(benmap_highVSzero_incidence_phil, "Lower incidence High CDR")
    benmap_lowVSzero_incidence_proc_phil<- process_benmap(benmap_lowVSzero_incidence_phil, "Lower incidence Low CDR")
    
    #Process reduction based on income
    benmap_highVSzero_incidence_SSP1_income_proc_phil <- process_benmap(benmap_highVSzero_incidence_SSP1_income_phil, "SSP1-income lower incidence High CDR")
    benmap_highVSzero_incidence_SSP4_income_proc_phil <- process_benmap(benmap_highVSzero_incidence_SSP4_income_phil, "SSP4-income lower incidence High CDR")
    benmap_lowVSzero_incidence_SSP1_income_proc_phil <- process_benmap(benmap_lowVSzero_incidence_SSP1_income_phil, "SSP1-income lower incidence Low CDR")
    benmap_lowVSzero_incidence_SSP4_income_proc_phil <- process_benmap(benmap_lowVSzero_incidence_SSP4_income_phil, "SSP4-income lower incidence Low CDR")
    
    #Process reduction based on race
    benmap_highVSzero_incidence_SSP1_race_proc_phil <- process_benmap(benmap_highVSzero_incidence_SSP1_race_phil, "SSP1-race lower incidence High CDR")
    benmap_highVSzero_incidence_SSP4_race_proc_phil <- process_benmap(benmap_highVSzero_incidence_SSP4_race_phil, "SSP4-race lower incidence High CDR")
    benmap_lowVSzero_incidence_SSP1_race_proc_phil <- process_benmap(benmap_lowVSzero_incidence_SSP1_race_phil, "SSP1-race lower incidence Low CDR")
    benmap_lowVSzero_incidence_SSP4_race_proc_phil <- process_benmap(benmap_lowVSzero_incidence_SSP4_race_phil, "SSP4-race lower incidence Low CDR")
    
    # Combine BASE + sensitivity
    benmap_all_phil <- bind_rows(list(benmap_base_high_proc_phil, benmap_base_low_proc_phil,
                                      benmap_highVSzero_incidence_proc_phil, benmap_lowVSzero_incidence_proc_phil,
                                      benmap_highVSzero_incidence_SSP1_income_proc_phil, benmap_highVSzero_incidence_SSP4_income_proc_phil,
                                      benmap_lowVSzero_incidence_SSP1_income_proc_phil, benmap_lowVSzero_incidence_SSP4_income_proc_phil,
                                      benmap_highVSzero_incidence_SSP1_race_proc_phil, benmap_highVSzero_incidence_SSP4_race_proc_phil,
                                      benmap_lowVSzero_incidence_SSP1_race_proc_phil, benmap_lowVSzero_incidence_SSP4_race_proc_phil))
    
    scenario_order <- c("Base High CDR", "Lower incidence High CDR", 
                        "SSP1-income lower incidence High CDR", "SSP4-income lower incidence High CDR",
                        "SSP1-race lower incidence High CDR", "SSP4-race lower incidence High CDR",
                        "Base Low CDR", "Lower incidence Low CDR",
                        "SSP1-income lower incidence Low CDR", "SSP4-income lower incidence Low CDR",
                        "SSP1-race lower incidence Low CDR", "SSP4-race lower incidence Low CDR")
    
    benmap_all_phil %>%
      mutate(Scenario = factor(Scenario, levels = scenario_order)) -> benmap_all_phil_fig
    
      #### 11.1.11 Uniform reductions ####
      #  Create faceted plot for uniform reductions
      p_facets <- ggplot() +
        geom_sf(data = benmap_all_phil_fig %>%
                  filter(Scenario %in% c("Base High CDR", "Lower incidence High CDR", 
                                         "Base Low CDR", "Lower incidence Low CDR")), 
                aes(fill = Point.Estim), color = NA, size = 0) +
        geom_sf(data = cbsa_shapefile_phil, color = "black", fill = NA) +
        scale_fill_viridis_c(option = "rocket", direction = -1) +#limits = c(0, 1)
        ...
        annotation_scale() +
        theme_void() +
        labs(title = "2050 Philadelphia Mortality due to PM2.5  (baseline incidence sensitivity)",
             fill = "Deaths per pixel") 
      ggsave("figures/BenMAP/sensitivity_incidence/mortality/Philadelphia/1km_mortality_2050_Benmap_phil_total_SENS.png", p_facets, width = 16, height = 12, units = "in", dpi = 600)
      ggsave("figures/BenMAP/sensitivity_incidence/mortality/Philadelphia/1km_mortality_2050_Benmap_phil_total_SENS.svg", p_facets, width = 16, height = 12, units = "in", dpi = 600)
      
      #Difference
      benmap_highVSzero_incidence_proc_phil %>%
        dplyr::select(ROW, COL, Point.Estim, Scenario) %>%
        dplyr::rename(mortality_sensitivity = Point.Estim) %>%
        left_join(
          benmap_base_high_proc_phil %>%
            dplyr::select(ROW, COL, Point.Estim, Scenario) %>%
            dplyr::rename(mortality_base = Point.Estim) %>%
            sf::st_drop_geometry(),
          by = c("ROW", "COL")
        ) %>%
        #If positive, it means mortality increased in sensitivity 
        #If negative, it means mortality decreased in sensitivity
        dplyr::mutate(difference = mortality_sensitivity - mortality_base) ->
        benmap_phil_highCDR_difference
      
      benmap_lowVSzero_incidence_proc_phil %>% 
        dplyr::select(ROW, COL, Point.Estim, Scenario) %>%
        dplyr::rename(mortality_sensitivity = Point.Estim) %>%
        left_join(benmap_base_low_proc_phil %>%
                    dplyr::select(ROW, COL, Point.Estim, Scenario) %>%
        ...
        #If positive, it means mortality increased in sensitivity
        #If negative, it means mortality decreased in sensitivity
        dplyr::mutate(difference = mortality_sensitivity - mortality_base)-> benmap_phil_lowCDR_difference
      ...
      p_facets <- ggplot() +
        ...
        annotation_scale() +
        theme_void() +
        labs(title = "2050 Philadelphia Mortality due to PM2.5 (baseline incidence sensitivity difference vs base)", 
             subtitle = "If positive, mortality increased in sensitivity.
             If negative, mortality decreased in sensitivity",
             fill = "Deaths per pixel") 
      ggsave("figures/BenMAP/sensitivity_incidence/mortality/Philadelphia/1km_mortality_2050_Benmap_phil_total_SENS_diff.png", p_facets, width = 16, height = 12, units = "in", dpi = 600)
      ggsave("figures/BenMAP/sensitivity_incidence/mortality/Philadelphia/1km_mortality_2050_Benmap_phil_total_SENS_diff.svg", p_facets, width = 16, height = 12, units = "in", dpi = 600)
      
      #### 11.1.2. Non-Uniform reductions: RACE ####
      ### Now for non-uniform sensitivity RACE
      #  Create faceted plot for non-uniform reductions
      p_facets <- ggplot() +
        geom_sf(data = benmap_all_phil_fig %>%
                  filter(Scenario %in% c("Base High CDR","Base Low CDR", 
                                         "SSP1-race lower incidence High CDR", "SSP4-race lower incidence High CDR",
        ...
        annotation_scale() +
        theme_void() +
        labs(title = "2050 Philadelphia Mortality due to PM2.5  (baseline incidence sensitivity, RACE)",
             fill = "Deaths per pixel") 
      ggsave("figures/BenMAP/sensitivity_incidence/mortality/Philadelphia/1km_mortality_2050_Benmap_phil_total_SENS_nonuniform_RACE.png", p_facets, width = 16, height = 12, units = "in", dpi = 600)
      ggsave("figures/BenMAP/sensitivity_incidence/mortality/Philadelphia/1km_mortality_2050_Benmap_phil_total_SENS_nonuniform_RACE.svg", p_facets, width = 16, height = 12, units = "in", dpi = 600)
      
      #Difference
      benmap_base_high_proc_phil %>%
        dplyr::select(ROW, COL, Point.Estim, Scenario) %>%
        dplyr::rename(mortality_base = Point.Estim) %>%
        left_join(benmap_highVSzero_incidence_SSP1_race_proc_phil %>%
                    dplyr::select(ROW, COL, Point.Estim, Scenario) %>%
                    dplyr::rename(mortality_SSP1_race = Point.Estim) %>%
                    sf::st_drop_geometry(),
                  by = c("ROW", "COL")) %>%
        #If positive, it means mortality increased in sensitivity 
        #If negative, it means mortality decreased in sensitivity
        dplyr::mutate(difference = mortality_SSP1_race - mortality_base) ->
        benmap_phil_highCDR_SSP1_race_difference
      
      benmap_base_high_proc_phil %>%
        dplyr::select(ROW, COL, Point.Estim, Scenario) %>%
        dplyr::rename(mortality_base = Point.Estim) %>%
        left_join(benmap_highVSzero_incidence_SSP4_race_proc_phil %>%
                    dplyr::select(ROW, COL, Point.Estim, Scenario) %>%
                    dplyr::rename(mortality_SSP4_race = Point.Estim) %>%
                    sf::st_drop_geometry(),
                  by = c("ROW", "COL")) %>%
        dplyr::mutate(difference = mortality_SSP4_race - mortality_base) ->
        benmap_phil_highCDR_SSP4_race_difference
      
      benmap_base_low_proc_phil %>%
        dplyr::select(ROW, COL, Point.Estim, Scenario) %>%
        dplyr::rename(mortality_base = Point.Estim) %>%
        left_join(benmap_lowVSzero_incidence_SSP1_race_proc_phil %>%
                    dplyr::select(ROW, COL, Point.Estim, Scenario) %>%
                    dplyr::rename(mortality_SSP1_race = Point.Estim) %>%
                    sf::st_drop_geometry(),
                  by = c("ROW", "COL")) %>%
        dplyr::mutate(difference = mortality_SSP1_race - mortality_base) ->
        benmap_phil_lowCDR_SSP1_race_difference
      
      benmap_base_low_proc_phil %>%
        dplyr::select(ROW, COL, Point.Estim, Scenario) %>%
        dplyr::rename(mortality_base = Point.Estim) %>%
        left_join(benmap_lowVSzero_incidence_SSP4_race_proc_phil %>%
                    dplyr::select(ROW, COL, Point.Estim, Scenario) %>%
                    dplyr::rename(mortality_SSP4_race = Point.Estim) %>%
                    sf::st_drop_geometry(),
                  by = c("ROW", "COL")) %>%
        ...
        #If negative, it means mortality decreased in sensitivity
        dplyr::mutate(difference = mortality_SSP4_race - mortality_base) ->
        benmap_phil_lowCDR_SSP4_race_difference
      
      benmap_all_phil_difference_race <- bind_rows(benmap_phil_highCDR_SSP1_race_difference, benmap_phil_highCDR_SSP4_race_difference,
                                                     benmap_phil_lowCDR_SSP1_race_difference, benmap_phil_lowCDR_SSP4_race_difference)
      
      #plot
      p_facets <- ggplot() +
        geom_sf(data = benmap_all_phil_difference_race %>% mutate(Scenario.y = factor(Scenario.y, levels = scenario_order)),
                aes(fill = difference), color = NA, size = 0) +
        geom_sf(data = cbsa_shapefile_phil, color = "black", fill = NA) +
        scale_fill_gradient2( low = "blue", mid = "white",   high = "red", midpoint = 0,   name = "Deaths per pixel") +
        facet_wrap(~Scenario.y, ncol = 2) +
        annotation_scale() +
        theme_void() +
        labs(title = "2050 Philadelphia Mortality due to PM2.5 (RACE sensitivity minus baseline)", 
             subtitle = "If positive, mortality increased in sensitivity.
             If negative, mortality decreased in sensitivity",
             fill = "Deaths per pixel") 
      ggsave("figures/BenMAP/sensitivity_incidence/mortality/Philadelphia/1km_mortality_2050_Benmap_phil_total_SENS_nonuniform_RACE_diff.png", p_facets, width = 16, height = 12, units = "in", dpi = 600)
      ggsave("figures/BenMAP/sensitivity_incidence/mortality/Philadelphia/1km_mortality_2050_Benmap_phil_total_SENS_nonuniform_RACE_diff.svg", p_facets, width = 16, height = 12, units = "in", dpi = 600)
      
      #### 11.1.3. Non-Uniform reductions: INCOME ####
      ###INCOME BASED PLOTS
      p_facets <- ggplot() +
        geom_sf(data = benmap_all_phil_fig %>%
                  filter(Scenario %in% c("Base High CDR","Base Low CDR", 
        ...
        annotation_scale() +
        theme_void() +
        labs(title = "2050 Philadelphia Mortality due to PM2.5  (baseline incidence sensitivity, INCOME)",
             fill = "Deaths per pixel") 
      ggsave("figures/BenMAP/sensitivity_incidence/mortality/Philadelphia/1km_mortality_2050_Benmap_phil_total_SENS_nonuniform_INCOME.png", p_facets, width = 16, height = 12, units = "in", dpi = 600)
      ggsave("figures/BenMAP/sensitivity_incidence/mortality/Philadelphia/1km_mortality_2050_Benmap_phil_total_SENS_nonuniform_INCOME.svg", p_facets, width = 16, height = 12, units = "in", dpi = 600)
      
      #Difference
      benmap_base_high_proc_phil %>%
        dplyr::select(ROW, COL, Point.Estim, Scenario) %>%
        dplyr::rename(mortality_base = Point.Estim) %>%
        left_join(benmap_highVSzero_incidence_SSP1_income_proc_phil %>%
                    dplyr::select(ROW, COL, Point.Estim, Scenario) %>%
                    dplyr::rename(mortality_SSP1_income = Point.Estim) %>%
                    sf::st_drop_geometry(),
                  by = c("ROW", "COL")) %>%
        #If positive, it means mortality increased in sensitivity 
        #If negative, it means mortality decreased in sensitivity
        dplyr::mutate(difference = mortality_SSP1_income - mortality_base) ->
        benmap_phil_highCDR_SSP1_income_difference
      
      benmap_base_high_proc_phil %>%
        dplyr::select(ROW, COL, Point.Estim, Scenario) %>%
        dplyr::rename(mortality_base = Point.Estim) %>%
        left_join(benmap_highVSzero_incidence_SSP4_income_proc_phil %>%
                    dplyr::select(ROW, COL, Point.Estim, Scenario) %>%
                    dplyr::rename(mortality_SSP4_income = Point.Estim) %>%
                    sf::st_drop_geometry(),
                  by = c("ROW", "COL")) %>%
        dplyr::mutate(difference = mortality_SSP4_income - mortality_base) ->
        benmap_phil_highCDR_SSP4_income_difference
      
      benmap_base_low_proc_phil %>%
        dplyr::select(ROW, COL, Point.Estim, Scenario) %>%
        dplyr::rename(mortality_base = Point.Estim) %>%
        left_join(benmap_lowVSzero_incidence_SSP1_income_proc_phil %>%
                    dplyr::select(ROW, COL, Point.Estim, Scenario) %>%
                    dplyr::rename(mortality_SSP1_income = Point.Estim) %>%
                    sf::st_drop_geometry(),
                  by = c("ROW", "COL")) %>%
        dplyr::mutate(difference = mortality_SSP1_income - mortality_base) ->
        benmap_phil_lowCDR_SSP1_income_difference
      
      benmap_base_low_proc_phil %>%
        dplyr::select(ROW, COL, Point.Estim, Scenario) %>%
        dplyr::rename(mortality_base = Point.Estim) %>%
        left_join(benmap_lowVSzero_incidence_SSP4_income_proc_phil %>%
                    dplyr::select(ROW, COL, Point.Estim, Scenario) %>%
                    dplyr::rename(mortality_SSP4_income = Point.Estim) %>%
                    sf::st_drop_geometry(),
                  by = c("ROW", "COL")) %>%
        ...
        #If negative, it means mortality decreased in sensitivity
        dplyr::mutate(difference = mortality_SSP4_income - mortality_base) ->
        benmap_phil_lowCDR_SSP4_income_difference
      
      benmap_all_phil_difference_income <- bind_rows(benmap_phil_highCDR_SSP1_income_difference, benmap_phil_highCDR_SSP4_income_difference,
                                                     benmap_phil_lowCDR_SSP1_income_difference, benmap_phil_lowCDR_SSP4_income_difference)
      
      #plot
      p_facets <- ggplot() +
        geom_sf(data = benmap_all_phil_difference_income %>% mutate(Scenario.y = factor(Scenario.y, levels = scenario_order)),
                aes(fill = difference), color = NA, size = 0) +
        geom_sf(data = cbsa_shapefile_phil, color = "black", fill = NA) +
        scale_fill_gradient2( low = "blue",  mid = "white",  high = "red",  midpoint = 0,   name = "Deaths per pixel"   ) +
        facet_wrap(~Scenario.y, ncol = 2) +
        annotation_scale() +
        theme_void() +
        labs(title = "2050 Philadelphia Mortality due to PM2.5 (INCOME sensitivity minus baseline)", 
             subtitle = "If positive, mortality increased in sensitivity.
             If negative, mortality decreased in sensitivity",
             fill = "Deaths per pixel") 
      ggsave("figures/BenMAP/sensitivity_incidence/mortality/Philadelphia/1km_mortality_2050_Benmap_phil_total_SENS_nonuniform_INCOME_diff.png", p_facets, width = 16, height = 12, units = "in", dpi = 600)
      ggsave("figures/BenMAP/sensitivity_incidence/mortality/Philadelphia/1km_mortality_2050_Benmap_phil_total_SENS_nonuniform_INCOME_diff.svg", p_facets, width = 16, height = 12, units = "in", dpi = 600)
    ...
    #### 11.3 DISTRIBUTION ASSESSMENT####
    #### 11.3.1. Load all relevant files ####
    #Mortality
    mortality_Philadelphia_HIGH <- st_read("BenMAP_results/1km_V2/Philadelphia/health_impacts_2050_1km_highvszero/Health Impacts-Pope 2019_20240520.shp") %>% st_drop_geometry() %>% dplyr::select(ROW, COL, Point.Estim) %>% rename(row = ROW, col = COL, deaths = Point.Estim) %>% mutate(Scenario = "high-CDR original")
    
    mortality_Philadelphia_LOW <- st_read("BenMAP_results/1km_V2/Philadelphia/health_impacts_2050_1km_lowvszero/Health Impacts-Pope 2019_20240520.shp") %>% st_drop_geometry() %>% dplyr::select(ROW, COL, Point.Estim) %>% rename(row = ROW, col = COL, deaths = Point.Estim) %>% mutate(Scenario = "low-CDR original")
    
    #Mortality from baseline incidence reduction (uniform 10% reduction)
    benmap_highVSzero_incidence_phil <- st_read("BenMAP_results/1km_V2/Philadelphia/incidence_sensitivity/highVSzero_mortality_10per_lower_incidence_sens/Health Impacts-Pope 2019_20260304.shp") %>% st_drop_geometry()%>% dplyr::select(ROW, COL, Point.Estim) %>% rename(row = ROW, col = COL, deaths = Point.Estim) %>% mutate(Scenario = "high-CDR incidence sens")
    benmap_lowVSzero_incidence_phil <- st_read("BenMAP_results/1km_V2/Philadelphia/incidence_sensitivity/lowVSzero_mortality_10per_lower_incidence_sens/Health Impacts-Pope 2019_20260304.shp") %>% st_drop_geometry()%>% dplyr::select(ROW, COL, Point.Estim) %>% rename(row = ROW, col = COL, deaths = Point.Estim) %>% mutate(Scenario = "low-CDR incidence sens")
    
    #Here we assume 10% reduction in lower income areas following a hypothetical SSP1 and SSP4
    benmap_highVSzero_incidence_SSP1_income_phil <- st_read("BenMAP_results/1km_V2/Philadelphia/incidence_sensitivity/highVSzero_mortality_10per_lower_incidence_sens_SSP1_income/Health Impacts-Pope 2019_20260313.shp")%>% dplyr::select(ROW, COL, Point.Estim) %>% rename(row = ROW, col = COL, deaths = Point.Estim) %>% mutate(Scenario = "high-CDR incidence sens SSP1 income")
    benmap_highVSzero_incidence_SSP4_income_phil <- st_read("BenMAP_results/1km_V2/Philadelphia/incidence_sensitivity/highVSzero_mortality_10per_lower_incidence_sens_SSP4_income/Health Impacts-Pope 2019_20260313.shp") %>% dplyr::select(ROW, COL, Point.Estim) %>% rename(row = ROW, col = COL, deaths = Point.Estim) %>% mutate(Scenario = "high-CDR incidence sens SSP4 income")
    
    benmap_lowVSzero_incidence_SSP1_income_phil <- st_read("BenMAP_results/1km_V2/Philadelphia/incidence_sensitivity/lowVSzero_mortality_10per_lower_incidence_sens_SSP1_income/Health Impacts-Pope 2019_20260313.shp") %>% dplyr::select(ROW, COL, Point.Estim)%>% rename(row = ROW, col = COL, deaths = Point.Estim) %>% mutate(Scenario = "low-CDR incidence sens SSP1 income")
    benmap_lowVSzero_incidence_SSP4_income_phil <- st_read("BenMAP_results/1km_V2/Philadelphia/incidence_sensitivity/lowVSzero_mortality_10per_lower_incidence_sens_SSP4_income/Health Impacts-Pope 2019_20260313.shp") %>% dplyr::select(ROW, COL, Point.Estim)%>% rename(row = ROW, col = COL, deaths = Point.Estim) %>% mutate(Scenario = "low-CDR incidence sens SSP4 income")
    
    #Here we assume 10% reduction in minority areas following a hypothetical SSP1 and SSP4
    benmap_highVSzero_incidence_SSP1_race_phil <- st_read("BenMAP_results/1km_V2/Philadelphia/incidence_sensitivity/highVSzero_mortality_10per_lower_incidence_sens_SSP1_race/Health Impacts-Pope 2019_20260313.shp")  %>% dplyr::select(ROW, COL, Point.Estim)%>% rename(row = ROW, col = COL, deaths = Point.Estim) %>% mutate(Scenario = "high-CDR incidence sens SSP1 race")
    benmap_highVSzero_incidence_SSP4_race_phil <- st_read("BenMAP_results/1km_V2/Philadelphia/incidence_sensitivity/highVSzero_mortality_10per_lower_incidence_sens_SSP4_race/Health Impacts-Pope 2019_20260313.shp") %>% dplyr::select(ROW, COL, Point.Estim)%>% rename(row = ROW, col = COL, deaths = Point.Estim) %>% mutate(Scenario = "high-CDR incidence sens SSP4 race")
    
    benmap_lowVSzero_incidence_SSP1_race_phil <- st_read("BenMAP_results/1km_V2/Philadelphia/incidence_sensitivity/lowVSzero_mortality_10per_lower_incidence_sens_SSP1_race/Health Impacts-Pope 2019_20260313.shp") %>% dplyr::select(ROW, COL, Point.Estim)%>% rename(row = ROW, col = COL, deaths = Point.Estim) %>% mutate(Scenario = "low-CDR incidence sens SSP1 race")
    benmap_lowVSzero_incidence_SSP4_race_phil <- st_read("BenMAP_results/1km_V2/Philadelphia/incidence_sensitivity/lowVSzero_mortality_10per_lower_incidence_sens_SSP4_race/Health Impacts-Pope 2019_20260313.shp")  %>% dplyr::select(ROW, COL, Point.Estim)%>% rename(row = ROW, col = COL, deaths = Point.Estim) %>% mutate(Scenario = "low-CDR incidence sens SSP4 race")
    ...
    #### 11.3.2. Bring partitions file ####
    #Join all
    mortality_Philadelphia_HIGH %>%
      bind_rows(mortality_Philadelphia_LOW,
                benmap_highVSzero_incidence_phil,
                benmap_lowVSzero_incidence_phil,
                benmap_highVSzero_incidence_SSP1_income_phil,
                benmap_highVSzero_incidence_SSP4_income_phil,
                benmap_lowVSzero_incidence_SSP1_income_phil,
                benmap_lowVSzero_incidence_SSP4_income_phil,
                benmap_highVSzero_incidence_SSP1_race_phil,
                benmap_highVSzero_incidence_SSP4_race_phil,
                benmap_lowVSzero_incidence_SSP1_race_phil,
                benmap_lowVSzero_incidence_SSP4_race_phil) -> mortlaity_sensitivity_incidence_Phil
    
    partitions_phil_race %>% #race
      left_join(partitions_phil_income, by = c("pixel_ID", "row", "col")) %>% #income
      left_join(mortlaity_sensitivity_incidence_Phil, by = c("row", "col")) %>%
      #Here we filter out pixels with NA in pollution, which refer to some bordering pixels
      filter_all(all_vars(!is.na(.)))-> partitions_phil_complete_pre
    ...
    #### 11.3. Apply population weighted equations ####
    ###NOTE USE ORIGINAL FORMULAS ON ROW ~13,000
    phil_weighted_analysis <- city_analysis("Philadelphia", partitions_phil_complete_pre)
    phil_white <- calculate_weighted_average_bins_white("Philadelphia", partitions_phil_complete_pre)
    phil_income <- calculate_weighted_average_bins_income("Philadelphia", partitions_phil_complete_pre)
    
    #### 11.4 Scatterplots ####
    library(stringr)
    ###RACE-ETHNICITY
    phil_plot <- phil_white %>%
      filter(bin %in% c("0to30white", "61to100white")) %>%
      select(Scenario, bin, deaths) %>%
      pivot_wider(names_from = bin, values_from = deaths) %>%
      rename( white_0to30 = `0to30white`,
        white_61to100 = `61to100white`) %>%
      mutate(CDR = ifelse(str_detect(Scenario, "high-CDR"), "high", "low"),
        scenario_group = str_remove(Scenario, "high-CDR |low-CDR "),
        sensitivity = case_when(
          str_detect(scenario_group, "race") ~ "race",
          str_detect(scenario_group, "income") ~ "income",
          TRUE ~ "base" ))

    phil_plot <- bind_rows(
      phil_plot %>% filter(sensitivity == "base") %>% mutate(sensitivity = "race"),
      phil_plot %>% filter(sensitivity == "base") %>% mutate(sensitivity = "income"),
      phil_plot %>% filter(sensitivity != "base")
    )
    
    lims <- range(c(phil_plot$white_0to30, phil_plot$white_61to100), na.rm = TRUE)
    
    p<-ggplot(phil_plot, aes(x = white_0to30, y = white_61to100)) +
      geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "black") +
      geom_point(aes(color = CDR, shape = scenario_group), size = 5) +
      geom_line(aes(group = scenario_group), color = "grey40") +
      coord_equal(xlim = lims, ylim = lims) +
      labs(
        x = "Deaths (0–30% white population)",
        y = "Deaths (61–100% white population)",
        color = "CDR scenario"
      ) +
      facet_wrap(~sensitivity) +
      theme_minimal()+
      theme(panel.border = element_rect(color = "black", fill = NA, linewidth = 0.8))
    ggsave("figures/BenMAP/sensitivity_incidence/mortality/Philadelphia/1km_mortality_2050_Benmap_phil_total_SENS_nonuniform_scatterplot.png", p, width = 16, height = 12, units = "in", dpi = 600)
    ggsave("figures/BenMAP/sensitivity_incidence/mortality/Philadelphia/1km_mortality_2050_Benmap_phil_total_SENS_nonuniform_scatterplot.svg", p, width = 16, height = 12, units = "in", dpi = 600)
    
    
    ###INCOME
    phil_plot_income <- phil_income %>%
      filter(bin %in% c("0to33percentile", "66to100percentile")) %>%
      select(Scenario, bin, deaths) %>%
      pivot_wider(names_from = bin, values_from = deaths) %>%
      rename( income_0to33 = `0to33percentile`,
              income_66to100 = `66to100percentile`) %>%
      mutate(CDR = ifelse(str_detect(Scenario, "high-CDR"), "high", "low"),
             scenario_group = str_remove(Scenario, "high-CDR |low-CDR "),
             sensitivity = case_when(
               str_detect(scenario_group, "race") ~ "race",
    ...
    phil_plot_income <- bind_rows(
      phil_plot_income %>% filter(sensitivity == "base") %>% mutate(sensitivity = "race"),
      phil_plot_income %>% filter(sensitivity == "base") %>% mutate(sensitivity = "income"),
      phil_plot_income %>% filter(sensitivity != "base")
    )
    
    lims <- range(c(phil_plot_income$income_0to33, phil_plot_income$income_66to100), na.rm = TRUE)
    
    p<-ggplot(phil_plot_income, aes(x = income_0to33, y = income_66to100)) +
      geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "black") +
      geom_point(aes(color = CDR, shape = scenario_group), size = 5) +
      geom_line(aes(group = scenario_group), color = "grey40") +
      coord_equal(xlim = lims, ylim = lims) +
      labs(
        x = "Deaths (0–33rd income percentile)",
        y = "Deaths (66–100th income percentile)",
        color = "CDR scenario"
      ) +
      facet_wrap(~sensitivity) +
      theme_minimal()+
      theme(panel.border = element_rect(color = "black", fill = NA, linewidth = 0.8))
    ggsave("figures/BenMAP/sensitivity_incidence/mortality/Philadelphia/1km_mortality_2050_Benmap_phil_total_SENS_nonuniform_scatterplotINCOME.png", p, width = 16, height = 12, units = "in", dpi = 600)
    ggsave("figures/BenMAP/sensitivity_incidence/mortality/Philadelphia/1km_mortality_2050_Benmap_phil_total_SENS_nonuniform_scatterplotINCOME.svg", p, width = 16, height = 12, units = "in", dpi = 600)

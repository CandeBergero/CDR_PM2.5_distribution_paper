...
#### Other####
  ...
  #Order
    ...
    PM_sector_order <- c("Electricity (BECCS)",  "Electricity (CCS)", "Electricity", 
                         "Other energy processing","Urban processes", "Refining", 
                         "Transportation","Buildings (commercial)", 
                         "Buildings (residential)", "Cement", "Industry (energy use)","Industry (processes)")
    
    industry_sector_order <- c("cement", "other industrial energy use", "industrial processes")
    
    industry_PM25_order <- c(
      "cement",                                   # sector: cement
      "gas", "gas cogen",                         # sector: other industrial energy use
      "refined liquids", "refined liquids cogen",
      "coal", "coal cogen",
      "biomass", "biomass cogen",
      "solvents", "other industrial processes"  # sector: industrial processes
    ) 
...
#### 1.3. PM2.5 Emissions ####
    ...
    air_pollution_source_PM2.5 %>%
      group_by(Units, Scenario, ghg, year, Sector_detailed) %>%
      summarise(total_USA = sum(value)) %>%
      ungroup() -> air_pollution_source_PM2.5_national
    
    #Supplementary table
    air_pollution_source_PM2.5 %>%
      filter(year == 2050) %>%
      group_by(Units, Scenario, ghg, year, Sector_detailed, technology) %>%
      summarise(total_USA = sum(value)) %>%
      ungroup() %>%
      spread(Scenario, total_USA) %>%
      mutate(across(everything(), ~replace_na(., 0)))-> air_pollution_source_PM2.5_national_tech
    ...
    air_pollution_resoruce_prod %>%
      filter(year > 2010 & year < 2055,
             ghg == "PM2.5") %>%
      left_join(scenario_mapping, by = "scenario") %>%
      group_by(Units, Scenario, subresource, ghg, year) %>%
      summarise(total_USA = sum(value)) %>%
      ungroup() %>%
      mutate(Sector_detailed = "Other energy processing")-> air_pollution_resoruce_prod_PM2.5
    
    #### 1.3.1. All scenarios ####
    air_pollution_source_PM2.5_national %>%
      bind_rows(air_pollution_resoruce_prod_PM2.5) %>%
      mutate(Sector_detailed = factor(Sector_detailed, levels = PM_sector_order)) %>%
      filter(year != 2015)-> air_pollution_2.5_final 

    
    p <- ggplot() + geom_area(data=air_pollution_2.5_final, aes(x=year, y=total_USA, fill= Sector_detailed)) +
      scale_fill_manual(values=pollutant_sector_color) +
      scale_x_continuous(breaks=seq(2010,2100,10)) +
      facet_wrap( ~ Scenario, ncol = 2, as.table = F) +
    ...
    air_pollution_2.5_final %>%
      filter(year == 2050) %>%
      left_join(total, by = c("Scenario", "year")) %>%
      mutate(percentage = round((total_USA / sum)*100, digits = 1),
             total_USA2 = round(total_USA, digits=2))-> percentage
    
    p <- ggplot(percentage %>%  filter(!is.na(percentage)), aes(x = "", y = percentage, fill = Sector_detailed) ) +
      geom_col(width = 1, color = "white") +
      coord_polar(theta = "y") +
      scale_fill_manual(values=pollutant_sector_color) +
      facet_wrap(~ Scenario) +
      labs(title = "Sectoral Share (%) by Scenario",
           fill = "Sector") +
      theme_void() +
    ...
    air_pollution_2.5_final %>%
      ...
      spread(Scenario, total_USA) %>%
      mutate(difference = `Net-zero, Low BECCS, Low DAC` - `Net-zero, High BECCS, High DAC`)%>%
      mutate(Sector_detailed = factor(Sector_detailed, levels = PM_sector_order)) -> air_pollution_2.5_final_difference
    
    p <- ggplot() + geom_area(data=air_pollution_2.5_final_difference, aes(x=year, y=difference, fill= Sector_detailed)) +
      scale_fill_manual(values=pollutant_sector_color) +
      scale_x_continuous(breaks=seq(2010,2100,10)) +
      ggtitle( "Total PM2.5 Emissions by sector (Low - High)" ) +
      xlab("Year") +
      ylab("Tg") +
      figure_theme
    #ggsave("figures/paper_figures/1.3.PM2.5_emissions_sector_USA_difference.png", dpi=600/2, width=6000/300, height=3000/300)
    #ggsave("figures/paper_figures/1.3.PM2.5_emissions_sector_USA_difference.svg", dpi=600/2, width=6000/300, height=3000/300)
    
    air_pollution_2.5_final_difference %>%
      filter(year == 2050) %>%
      mutate(
        total_difference = sum(difference, na.rm = TRUE),
        share_of_difference = round((difference / total_difference) * 100, digits = 0),
        difference = round(difference, digits = 2)
      ) -> air_pollution_2.5_final_difference_2050
    
    p <- ggplot() + 
      geom_col(data = air_pollution_2.5_final_difference_2050, aes(x = "difference", y = difference, fill = Sector_detailed), position = "stack") +
      scale_fill_manual(values=pollutant_sector_color) +
      ggtitle( "Total PM2.5 Emissions by sector (Low - High)" ) +
      xlab("Year") +
      ylab("Tg") +
      figure_theme
    #ggsave("figures/paper_figures/1.3.PM2.5_emissions_sector_USA_difference_2050.png", dpi=600/2, width=6000/300, height=3000/300)
    #ggsave("figures/paper_figures/1.3.PM2.5_emissions_sector_USA_difference_2050.svg", dpi=600/2, width=6000/300, height=3000/300)
    
    #Dive into industry
    air_pollution_source_PM2.5 %>%
      filter(grepl("Industry", Sector_detailed)) %>%
      group_by(Units, Scenario, sector, ghg,year) %>%
      summarise(total_USA = sum(value)) %>%
      ungroup()-> air_pollution_source_PM2.5_industry
    
    p <- ggplot() + geom_area(data=air_pollution_source_PM2.5_industry, aes(x=year, y=total_USA, fill= sector)) +
      #scale_fill_manual(values=pollutant_sector_color) +
      scale_x_continuous(breaks=seq(2010,2100,10)) +
      facet_wrap( ~ Scenario, ncol = 2, as.table = F) +
      ggtitle( "Total PM2.5 Emissions within Industry by sector" ) +
      xlab("Year") +
      ylab("Tg") +
      figure_theme
    #ggsave("figures/paper_figures/1.3.PM2.5_emissions_sector_USA_industry.png", dpi=600/2, width=6000/300, height=3000/300)
    #ggsave("figures/paper_figures/1.3.PM2.5_emissions_sector_USA_industry.svg", dpi=600/2, width=6000/300, height=3000/300)
    
    #By technology
    air_pollution_source_PM2.5 %>%
      filter(grepl("Industry", Sector_detailed)) %>%
      group_by(Units, Scenario, sector, technology, ghg,year) %>%
      summarise(total_USA = sum(value)) %>%
      ungroup()%>%
      mutate(technology = factor(technology, levels = industry_PM25_order))-> air_pollution_source_PM2.5_industry_tech
    
    p <- ggplot() +
      geom_area(data = air_pollution_source_PM2.5_industry_tech,
                aes(x = year, y = total_USA, fill = technology), position = "stack") +
      scale_x_continuous(breaks = seq(2010, 2100, 10)) +
      facet_wrap(~ Scenario, ncol = 2, as.table = FALSE) +
      ggtitle("Total PM2.5 Emissions within Industry by Technology") +
      xlab("Year") +
      ylab("Tg") +
      figure_theme
    #ggsave("figures/paper_figures/1.3.PM2.5_emissions_sector_USA_industry_tech.png", dpi=600/2, width=6000/300, height=3000/300)
    #ggsave("figures/paper_figures/1.3.PM2.5_emissions_sector_USA_industry_tech.svg", dpi=600/2, width=6000/300, height=3000/300)
...
#### 2.0 Prepare data ####
    ...
    pixel_ID_table <- read.csv("output_data/BenMAP_files/9km/Date_ 2024-03-13 _9km_partitions_2019.csv")  %>% dplyr::select(row, col, pixel_ID) %>% unique() #98,276 total pixels
    ...
    input_pollution_pixels <- read.csv("output_data/BenMAP_files/9km_V2/Date_ 2024-05-24 OutputTable_PM25_2050_REF.csv") %>%
      dplyr::select(Row, Column) %>% #97,433 total pixels with pollution from Jing files
      left_join(pixel_ID_table, by = c("Row" = "row", "Column" = "col"))
    
    pixels_to_keep <- input_pollution_pixels$pixel_ID
    
    input_pollution_pixels %>% dplyr::select(pixel_ID, Row, Column) -> pixel_ID
    ...
    partitions %>%
      ...
      dplyr::select(pixel_ID, row, col, pixel_area) %>%
      unique() %>%
...
#### 2.1. 2050 Reference Pollution ####
    grid_data_PM25_NAD_REF_2050_filtered_final <- st_read("BenMAP_results/9km_V2/2050_REF/air_quality_2050_ref_9km/Air Quality-PM2.5-Delta_20240526.shp") %>% 
      rename(Row = ROW, Column = COL, Values = D24HourMean) %>% dplyr::select(-QuarterlyMe)
...
#### Census  data ####
  ## Load data
    ...
    input_income_data_tract_original <-read.csv("data/BenMAP/ACS/ACSDT5Y2019.B19013_2024-04-17T151300_tract/ACSDT5Y2019.B19013-Data.csv")  #From census data table B19013 on race at the census TRACT level. Note we rename column to match census block metadata
  ...
  ## Prepare census data 
    ...
    input_income_data_tract_original %>%
      mutate(GEO_ID = base::substr(GEO_ID, 8, 21)) -> input_income_data_tract
...
#--------------------------------------------------------------- POLICY COSTS -------------------------------------------------------------------
    ...
    #Load the project
      prj_cost <- loadProject('data/GCAM/GCAM_analysis_V3_policy_cost.dat')  
...
#### SM Figure 2: GCAM 2015 values ####
    ...
    p <- ggplot() + 
      geom_col(data = air_pollution_2.5_final_2015, aes(x = year, y = total_USA, fill = Sector_detailed), position = "stack") +
      scale_fill_manual(values=pollutant_sector_color) +
      facet_wrap(~ Scenario, ncol = 2, as.table = FALSE) +
      ggtitle("Total CO2 Emissions by sector") +
      xlab("Year") +
...
#--------------------------------------------------------------- SUPPLEMENTARY FIGURE  -------------------------------------------------------------------
    ...
    air_pollution_source_all %>% #This has electricity and other sources (including agriculture)
      filter(year > 2010 & year < 2055) %>%
      left_join(scenario_mapping, by = "scenario") %>%
      #mutate(ghg = str_remove(ghg, "_.*$")) %>% #get rid of extra info in gas
      filter(!region %in% c("AK", "HI")) %>%
      group_by(Units, Scenario, year, ghg) %>%
      summarise(total_USA = sum(value))-> air_pollution_gas_USA
    
    air_pollution_resoruce_prod %>% #This has resource production
      filter(year > 2010 & year < 2055) %>%
      left_join(scenario_mapping, by = "scenario") %>%
      group_by(Units, Scenario, ghg, year) %>%
      summarise(total_USA = sum(value)) %>%
      ungroup() -> air_pollution_resoruce_prod_USA
    
    #join both
    air_pollution_gas_USA %>%
      left_join(air_pollution_resoruce_prod_USA, by = c("Scenario","year", "ghg")) %>%
      mutate(total_USA = replace_na(total_USA.x, 0) + replace_na(total_USA.y, 0))-> air_pollution_gas_USA_complete #adds electricity, sectors, resource production
    
    p <- ggplot(air_pollution_gas_USA_complete %>% filter(year > 2015), aes(x = year, y = total_USA, color = Scenario)) +
      geom_point() +
      geom_line() +
      facet_wrap(~ ghg, ncol = 3, scales = "free_y") +
      ggtitle("Air pollution emissions in the US") +
      ylab("Emissions Mt/year") +
      theme_minimal()
    #ggsave("figures/paper_figures/SM_pollution.png", p, width = 16, height = 12, units = "in", dpi = 600)
    #ggsave("figures/paper_figures/SM_pollution.svg", p, width = 16, height = 12, units = "in", dpi = 600)
...
#### SM Figure 4 and 5: PM2.5 in states ####
    ...
    air_pollution_source_PM2.5 %>%
      group_by(Units, Scenario, ghg, year, Sector_detailed, region) %>%
      summarise(total_USA = sum(value)) %>%
      ungroup() -> air_pollution_sources_complete_PM2.5_region
    ...
    air_pollution_resoruce_prod %>%
      ...
      summarise(total_USA = sum(value)) %>%
      ungroup() %>%
      mutate(Sector_detailed = "Other energy processing")-> air_pollution_resoruce_prod_PM2.5_region
    ...
    air_pollution_sources_complete_PM2.5_region_final %>%
      ...
      filter(Scenario != "Reference") %>%
      mutate(Sector_detailed = factor(Sector_detailed, levels = PM_sector_order))-> air_pollution_sources_complete_PM2.5_region_final_top3
    
    p <- ggplot() + geom_area(data=air_pollution_sources_complete_PM2.5_region_final_top3, aes(x=year, y=total_USA, fill= Sector_detailed)) +
      scale_fill_manual(values=pollutant_sector_color) +
      scale_x_continuous(breaks=seq(2010,2100,10)) +
    ...
    ####Difference in all states in 2050####
    # 1. Calculate difference for 2050
    air_pollution_sources_complete_PM2.5_region_final %>%
      filter(Scenario != "Reference", year == 2050) %>% 
      spread(Scenario, total_USA) %>%
      mutate(difference = `Net-zero, Low BECCS, Low DAC` - `Net-zero, High BECCS, High DAC`) %>%
      mutate(Sector_detailed = factor(Sector_detailed, levels = PM_sector_order))-> air_pollution_sources_complete_PM2.5_region_final_DIFF
    
    # 2. Get total difference per state (sum of sectors) and order ascending (smallest first)
    state_order <- air_pollution_sources_complete_PM2.5_region_final_DIFF %>%
      group_by(region) %>%
      summarise(total_diff = sum(difference, na.rm = TRUE), .groups = "drop") %>%
      arrange(total_diff) %>%
      pull(region)
    
    # 3. Split into 5 groups of 10 and 1 group of 11
    state_groups <- split(state_order, ceiling(seq_along(state_order) / 10))
    
    # 4. Loop over groups and make stacked column plots with reordered factor levels inside each group
    for (i in seq_along(state_groups)) {
      
      # Subset data for this group
      df_group <- air_pollution_sources_complete_PM2.5_region_final_DIFF %>%
        filter(region %in% state_groups[[i]])
      
      # Calculate total difference per region within this group for ordering
      region_order_group <- df_group %>%
        group_by(region) %>%
        summarise(total_diff = sum(difference, na.rm = TRUE), .groups = "drop") %>%
        arrange(total_diff) %>%  # or use desc(total_diff) if you want largest first
        pull(region)
      
      # Reorder factor levels for this group
      df_group <- df_group %>%
        mutate(region = factor(region, levels = region_order_group))
      
      # Plot
      p <- ggplot(df_group, aes(x = "difference", y = difference, fill = Sector_detailed)) +
      ...
      # Save plots
      #ggsave(paste0("figures/paper_figures/SM5.PM2.5_emissions_sector_difference_group", i, ".png"), plot = p, dpi = 300, width = 10, height = 6)
      #ggsave(paste0("figures/paper_figures/SM5.PM2.5_emissions_sector_difference_group", i, ".svg"), plot = p, dpi = 300, width = 10, height = 6)
    }
...
#--------------------------------------------------------------- SUPPLEMENTARY FIGURE  -------------------------------------------------------------------
    #Look at composiiton of industrial emissions 
    ##### Final energy industry by fuel #####
    ...
    p <- ggplot() + geom_col(data=industry_final_energy_USA, aes(x=year, y=sum, fill= Fuel)) +
      ...
      theme(panel.background = element_blank(),
            ...
            legend.title = element_text(size = 15),
            legend.text = element_text(size = 10))
    ggsave("figures/paper_figures/SM.industry_final_energy_fuel.png", dpi=600/2, width=6000/300, height=3000/300)
    
    ##### Final energy industry by tech and fuel #####
    industry_final_energy_tech <- getQuery(prj, "industry final energy by tech and fuel" )
    
    industry_final_energy_tech %>%
      filter(region %in% USA_states,
             year > 2010 & year < 2055) %>%
      left_join(scenario_mapping, by = "scenario") %>%
      mutate(Scenario = factor(Scenario, levels = scenario_short_facet_order))%>%
      left_join(FE_fuel_mapping, by = "input") %>%
      mutate(Fuel = factor(Fuel, levels = final_energy_fuel_order))-> industry_final_energy_tech_states
    
    industry_final_energy_tech_states %>%
      group_by(Units, Scenario, Fuel, sector, year) %>%
      summarise(sum = sum(value)) %>%
      ungroup()  -> industry_final_energy_tech_USA
    
    p <- ggplot() + geom_col(data=industry_final_energy_tech_USA, aes(x=year, y=sum, fill= Fuel)) +
      #scale_fill_manual(values=final_energy_fuel_color) +
      ...
      theme(panel.background = element_blank(),
            ...
            legend.text = element_text(size = 10))
    ggsave("figures/paper_figures/SM.industry_final_energy_fuel_sector.png", dpi=600/2, width=6000/300, height=3000/300)
    #
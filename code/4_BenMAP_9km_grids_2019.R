...
#### 4. Join our shapefiles (census data + 9km grid) ####
    ...
    system.time({joined_data_original <- st_join(grid_data_NAD83_area, census_block, join = st_intersects)}) #30 seconds
      ...
      joined_data_original <- joined_data_original[complete.cases(joined_data_original$census_block_ID), , drop = FALSE]#From 661,940 to 516,028. Note we do not care to keep intersections where no people live, meaning the ocean.
...
#### 6. Run the function ####
  ...
  #### 6.2. Save partitions ####
    ...
    #Write as csv
    #write.csv(partitions, row.names = FALSE, file = paste("output_data/BenMAP_files/9km_V2/Date_", today_date, "_9km_partitions_2019.csv"))
    partitions <- read.csv("output_data/BenMAP_files/9km_V2/Date_ 2024-03-13 _9km_partitions_2019.csv")
    partitions %>%
...
#### 7. Outputs ####
  ...
  #### 7.5. Population by age ####
    ...
    #write.csv(population_2019_data_pixel_age_final, row.names = FALSE, file = paste("output_data/BenMAP_files/9km_V2/Date_", today_date, "9_km_grid_NAD83_filtered_population_2019.csv")) #BenMAP file wiht row, col, age range, population, all races, all ethnicity, all gender
    population_2019_data_pixel_age_final<- read.csv("output_data/BenMAP_files/9km_V2/Date_ 2024-03-01 9_km_grid_NAD83_filtered_population_2019.csv")
...
#### 6. Print output table####
    ...
    write.csv(partitions_rate_final_clean, row.names = FALSE, file = paste("output_data/BenMAP_files/9km_V2/Date_",today_date,"Incidence_9km_2019.csv"))
...
#### 1. 2019 (9km resolution) ####
  ...
  #### 1.4. Prepare output table#### 
      ...
      #grid_data_PM25_NAD_filtered_final <- read.csv("output_data/BenMAP_files/9km_V2/Date_ 2024-03-01 OutputTable_PM25_2019.csv")
...
#### 2. 2050 Reference (9km resolution) ####
  ...
  #### 2.4. Prepare output table #### 
      ...
      #grid_data_PM25_NAD_REF_2050_filtered_final <- read.csv("output_data/BenMAP_files/9km_V2/Date_ 2024-03-07 OutputTable_PM25_2050_REF.csv")
...
#### 3. 2050 High CDR (9km resolution) ####
  ...
  #### 3.4. Prepare output table #### 
      ...
      #grid_data_PM25_NAD_HIGH_2050_filtered_final <- read.csv("output_data/BenMAP_files/9km_V2/Date_ 2024-03-07 OutputTable_PM25_2050_HIGH.csv")
...
#### 4. 2050 Low CDR (9km resolution) ####
  ...
  #### 4.4. Prepare output table #### 
      ...
      #grid_data_PM25_NAD_LOW_2050_filtered_final <- read.csv("output_data/BenMAP_files/9km_V2/Date_ 2024-03-07 OutputTable_PM25_2050_LOW.csv")
...
#---------------------------------------------- SECTION 2: SCATTERPLOTS
      #Here we load BenMAP shapefiles with mortality results to create plots
      #These are results from High CDR run inus a Low CDR run
      benmap_2050_REF_9km <- st_read("BenMAP_results/9km_V2/2050_REF/health_impacts_2050_ref_9km/Health Impacts-Pope 2019_20240311.shp")
  ...
  #------------------------------------------------- EXTRA: SCATTER PLOTS BENMAP RESULTS -------------------------------------------------    
